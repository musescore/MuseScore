            ...
            while (e.readNextStartElement()) {
                ...
                if (tag == "transposeChromatic") {
                    ...
                    if (!makeGap1(dstTick, dstStaffIdx, tickLen, voiceOffset)) {
                        qDebug("cannot make gap in staff %d at tick %d", dstStaffIdx,
                               dstTick.ticks());
                    ...
                    for (Element* el :
                         seg->findAnnotations(ElementType::HARMONY, e.track(), e.track())) {
...
// qDebug("pasteChordRest %s at %d, len %d/%d", cr->name(), tick, cr->ticks().numerator(), cr->ticks().denominator() );
    ...
    bool convertMeasureRest = cr->isRest()
    ...
    if (!isGrace && !cr->tuplet()
        && (tick + cr->actualTicks() > measureEnd || partialCopy || convertMeasureRest)) {
        if (cr->isChord()) {
            // split Chord
            Chord* c = toChord(cr);
            ...
            while (rest.isNotZero()) {
                ...
                std::vector<TDuration> dl = toRhythmicDurationList(len, false,
                                                                   tick - measure->tick(),
                                                                   sigmap()->timesig(
                                                                       tick).nominal(), measure,
                                                                   MAX_DOTS);
                TDuration d = dl[0];
            ...
            while (!rest.isZero()) {
                ...
                std::vector<TDuration> dl = toRhythmicDurationList(len, true,
                                                                   tick - measure->tick(),
                                                                   sigmap()->timesig(
                                                                       tick).nominal(), measure,
                                                                   MAX_DOTS);
            ...
            if (tag == "trackOffset") {
                ...
                if (tag == "Harmony" || tag == "FretDiagram") {
                    ...
                    if (destTick >= lastTick) {
                        ...
                        harmSegm
                            = meas ? meas->undoGetSegment(SegmentType::ChordRest,
                                                          destTick) : nullptr;
                    }
