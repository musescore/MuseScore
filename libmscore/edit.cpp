...
Rest* Score::setRest(const Fraction& _tick, int track, const Fraction& _l, bool useDots,
                     Tuplet* tuplet, bool useFullMeasureRest)
{
    Fraction l = _l;
    Fraction tick = _tick;
    Measure* measure = tick2measure(tick);
    ...
    while (!l.isZero()) {
        ...
        if ((measure->timesig() == measure->ticks())       // not in pickup measure
            ...
            && (useFullMeasureRest)) {
            Rest* rest
                = addRest(tick, track, TDuration(TDuration::DurationType::V_MEASURE), tuplet);
                ...
                if (cr && cr->isChord()) {
                    ...
                    if (chord->tremolo() && chord->tremolo()->twoNotes()) {
                        ...
                        if (trem->chord2() == chord)
                            continue;
                        auto newP = std::tuple<Fraction, Fraction, Tremolo*, int>(
        ...
        for (Score* score : scoreList()) {
            ...
            if (local) {
                if (score == this) {
                    ...
                    qDebug(
                        "cmdAddTimeSig: unable to write local time signature change to linked score");
                    startStaffIdx = 0;
                    endStaffIdx = 0;
                }
            } else {
            ...
            for (int si = startStaffIdx; si < endStaffIdx; ++si) {
                ...
                if (nsig == 0) {
                    ...
                    nsig->undoChangeProperty(Pid::TIMESIG_TYPE, int(ts->timeSigType()));
                    nsig->undoChangeProperty(Pid::TIMESIG_STRETCH, QVariant::fromValue(
                                                 ts->stretch()));
                    nsig->undoChangeProperty(Pid::GROUPS, QVariant::fromValue(ts->groups()));
                    nsig->setSelected(false);
        ...
        for (int i = 0; i < nstaves(); ++i) {
            ...
            if (tsig && tsig->isLocal()) {
                for (Measure* nm = m; nm; nm = nm->nextMeasure()) {
                    ...
                    if (cr && cr->isRest()
                        && cr->durationType() == TDuration::DurationType::V_MEASURE)
                        cr->undoChangeProperty(Pid::DURATION,
                                               QVariant::fromValue(nm->stretchedLen(staff(i))));
    ...
    for (Measure* msr = seg->measure(); msr && msr->tick() < endTick; msr = msr->nextMeasure()) {
        ...
        for (; seg; seg = seg->next(SegmentType::ChordRest)) {
            ...
            if (curr->isRest() && !(curr->tuplet()) && !(toRest(curr)->isGap())) {
                ...
                for (Segment* s = seg->next(SegmentType::ChordRest); s;
                     s = s->next(SegmentType::ChordRest)) {
                ...
                seg = setNoteRest(seg, curr->track(),
                                  NoteVal(), restTicks, Direction::AUTO, false, true);
            } else if (curr->isChord()) {
                // combine tied chords
                Chord* chord = toChord(curr);
                Chord* lastTiedChord = chord;
                for (Chord* next = chord->nextTiedChord();
                     next && next->tick() + next->ticks() <= maxTick; next = next->nextTiedChord())
                    lastTiedChord = next;
                if (!lastTiedChord)
                    lastTiedChord = chord;
                ...
                if (!(curr->tuplet())) {
                    ...
                    for (;;) {
                        ...
                        dl
                            = toRhythmicDurationList(dd, false, segment->rtick(),
                                                     sigmap()->timesig(
                        ...
                        if (cr == 0) {
                            if (tr % VOICES)
                                cr
                                    = addRest(segment, tr, TDuration(
                                                  TDuration::DurationType::V_MEASURE), 0);
                            else
                                break;
                        }
                    }
                    if (_is.slur()) {
                        // extend slur
                        _is.slur()->undoChangeProperty(Pid::SPANNER_TICKS,
                                                       nchord->tick() - _is.slur()->tick());
                        for (ScoreElement* e : _is.slur()->linkList()) {
                            ...
                            for (ScoreElement* ee : nchord->linkList()) {
                                ...
                                if (e1->score() == slur->score() && e1->track() == slur->track2()) {
                                    slur->score()->undo(new ChangeSpannerElements(slur,
                                                                                  slur->startElement(),
                                                                                  e1));
                                    break;
                                }
        ...
        if (noteEntryMode()) {
            ...
            if (n) {
                ...
                while (nnote) {
                    ...
                    tie->setTicks(
                        nnote->chord()->segment()->tick() - note->chord()->segment()->tick());
                    undoAddElement(tie);
                    if (!addFlag || nnote->chord()->tick() >= lastAddedChord->tick()
                        || nnote->chord()->isGrace()) {
                        break;
                    } else {
                        note = nnote;
...
void Score::cmdFlip()
{
    ...
    auto flipOnce
        = [&alreadyFlippedElements](const Element* element,
                                    std::function<void()> flipFunction) -> void {
    ...
    for (Element* e : el) {
            ...
            if (chord->beam()) {
                ...
                flipOnce(chord, [chord](){
                        ...
                        chord->undoChangeProperty(Pid::STEM_DIRECTION,
                                                  QVariant::fromValue<Direction>(dir));
                    });
            }
        }
        ...
        if (e->isBeam()) {
            ...
            flipOnce(beam, [beam](){
                    ...
                    beam->undoChangeProperty(Pid::STEM_DIRECTION,
                                             QVariant::fromValue<Direction>(dir));
                });
            ...
            flipOnce(slurTieSegment, [slurTieSegment](){
                    ...
                    slurTieSegment->undoChangeProperty(Pid::SLUR_DIRECTION,
                                                       QVariant::fromValue<Direction>(dir));
                });
...
void Score::deleteItem(Element* el)
{
    ...
    if (el->generated()
        && !(el->isBracket() || el->isBarLine() || el->isClef() || el->isMeasureNumber()))
        return;
// qDebug("%s", el->name());
        ...
        //
    {
        ...
        if (el->voice() != 0) {
            ...
            if (m->isOnlyDeletedRests(track)) {
                ...
                Element* pe = 0;
                for (Segment* ps = s->prev(SegmentType::ChordRest); ps;
                     ps = ps->prev(SegmentType::ChordRest)) {
                ...
                Segment* ns;
                Element* ne = 0;
                for (ns = s->next(SegmentType::ChordRest); ns;
    ...
    case ElementType::MEASURE_NUMBER:
    {
        ...
        case MeasureNumberMode::AUTO: {
            mea->undoChangeProperty(Pid::MEASURE_NUMBER_MODE,
                                    static_cast<int>(MeasureNumberMode::HIDE));
        }
        break;
        ...
        case MeasureNumberMode::SHOW: {
            if (mea->showsMeasureNumberInAutoMode())
                ...
                mea->undoChangeProperty(Pid::MEASURE_NUMBER_MODE,
                                        static_cast<int>(MeasureNumberMode::AUTO));
        }
        break;
            ...
            if (sts && !lastDeletedKeySig) {
                ...
                if (lastDeletedKeySig) {
                    ...
                    if (!styleB(Sid::concertPitch) && !lastDeletedKeySigEvent.isAtonal()
                        ...
                        if (!v.isZero())
                            lastDeletedKeySigEvent.setKey(transposeKey(lastDeletedKeySigEvent.key(),
                                                                       v,
                                                                       lastDeletedKeySig->part()->
                                                                       preferSharpFlat()));
                    }
                }
            }
            ...
            if (!s) {
                ...
                for (int staffIdx = 0; staffIdx < score->nstaves(); staffIdx++) {
                    ...
                    if (transposeKeySigEvent) {
                        Interval v
                            = score->staff(staffIdx)->part()->instrument(Fraction(0,
                                                                                  1))->transpose();
                        v.flip();
...
void Score::deleteSpannersFromRange(const Fraction& t1, const Fraction& t2, int track1, int track2,
...
void Score::deleteAnnotationsFromRange(Segment* s1, Segment* s2, int track1, int track2,
                                       const SelectionFilter& filter)
{
    if (!s1)
        return;
...
ChordRest* Score::deleteRange(Segment* s1, Segment* s2, int track1, int track2,
                              const SelectionFilter& filter)
{
    ChordRest* cr = nullptr;
        ...
        for (int track = track1; track < track2; ++track) {
            ...
            for (Segment* s = s1; s && (s->tick() < stick2); s = s->next1()) {
                ...
                if (!s->isChordRestType()) {
                    ...
                    if (s->segmentType() != SegmentType::TimeSig
                ...
                if (tick == Fraction(-1, 1)) {
                    ...
                    tuplet = cr1->tuplet();
                    if (tuplet && (tuplet->tick() == tick)
                        && ((tuplet->tick() + tuplet->actualTicks()) <= tick2)) {
...
void Score::cmdDeleteSelection()
{
    ...
    if (selection().isRange()) {
        ...
        cr
            = deleteRange(s1, s2, staff2track(selection().staffStart()),
                ...
                if (e->isNote())
                    ...
                    tick = toSpannerSegment(e)->spanner()->tick();
                         ...
                         && (e->parent()->isSegment() || e->parent()->isChord()
                             || e->parent()->isNote() || e->parent()->isRest()))
                    tick = e->parent()->tick();
                // else tick < 0
                track = e->track();
            }
...
Hairpin* Score::addHairpin(HairpinType t, const Fraction& tickStart, const Fraction& tickEnd,
                           int track)
{
...
bool Score::checkTimeDelete(Segment* startSegment, Segment* endSegment)
{
    ...
    if (endSegment)
        endMeasure
            = endSegment->prev() ? endSegment->measure() : endSegment->measure()->prevMeasure();
    else
...
void Score::cloneVoice(int strack, int dtrack, Segment* sf, const Fraction& lTick, bool link,
                       bool spanner)
{
            ...
            if (oe->isChordRest()) {
                ...
                if (oe->isChord()) {
                    ...
                    for (size_t i = 0; i < n; ++i) {
                        ...
                        Interval v
                            = staff(idx) ? staff(idx)->part()->instrument(tick)->transpose() :
                              Interval();
                        nn->setTpc1(on->tpc1());
                        if (v.isZero()) {
                            nn->setTpc2(on->tpc1());
                ...
                if (track == strack && track2 == strack) {
                    ...
                    if (cr1 && cr1->links()) {
                        for (ScoreElement* e : *cr1->links()) {
                            ...
                            if (cr == cr1)
                                continue;
                            if ((cr->score() == this) && (cr->tick() == ns->tick())
                                && cr->track() == dtrack) {
                    ...
                    if (cr2 && cr2->links()) {
                        for (ScoreElement* e : *cr2->links()) {
                            ...
                            if (cr == cr2)
                                continue;
                            if ((cr->score() == this) && (cr->tick() == ns->tick2())
                                && cr->track() == dtrack) {
                                ns->setEndElement(cr);
                                break;
                            }
...
void Score::updateInstrumentChangeTranspositions(KeySigEvent& key, Staff* staff,
                                                 const Fraction& tick)
{
            ...
            if (e.forInstrumentChange()) {
                ...
                if (key.isAtonal() && !e.isAtonal()) {
                    ...
                    Interval transposeInterval
                        = staff->part()->instrument(Fraction::fromTicks(nextTick))->transpose();
...
void Score::undoExchangeVoice(Measure* measure, int srcVoice, int dstVoice, int srcStaff,
                              int dstStaff)
{
        ...
        for (Staff* st : staffList) {
            ...
            if (ex) {
                ...
                for (int srcTrack2 : srcTrackList) {
                    ...
                    for (int testTrack : testTracks) {
                        if (staffTrack <= testTrack && testTrack < staffTrack + VOICES
                            && dstTrackList.contains(testTrack)) {
                            hasVoice = true;
                    ...
                    if (!hasVoice) {
                        undo(new CloneVoice(measure->first(), measure2->endTick(),
                                            measure2->first(), tempTrack, srcTrack2,
                                            tempTrack + trackDiff));
                ...
                for (int dstTrack2 : dstTrackList) {
                    ...
                    if (!hasVoice) {
                        undo(new CloneVoice(measure->first(), measure2->endTick(),
                                            measure2->first(), tempTrack, dstTrack2,
                                            tempTrack - trackDiff));
                ...
                undo(new ExchangeVoice(measure2, staffTrack + srcVoice, staffTrack + dstVoice,
                                       st->idx()));
            }
        }
    }
    ...
    if ((et == ElementType::REHEARSAL_MARK)
        ...
        for (Staff* staff : staffList) {
            ...
            if (et == ElementType::VOLTA) {
                ...
                toMeasure(element->parent())->undoChangeProperty(Pid::MEASURE_NUMBER_MODE,
                                                                 static_cast<int>(MeasureNumberMode
                                                                                  ::SHOW));
            } else {
    ...
    for (Staff* staff : ostaff->staffList()) {
        ...
        for (int ntrack : tr) {
            ...
            //
                     ...
                     || element->isFermata()
                     || element->isHarmony()) {
                Segment* segment = element->parent()->isFretDiagram() ? toSegment(
                    element->parent()->parent()) : toSegment(element->parent());
                Fraction tick = segment->tick();
                Measure* m = score->tick2measure(tick);
                ...
                if (element->isHarmony() && ne != element) {
                    ...
                    if (score->styleB(Sid::concertPitch)
                        != element->score()->styleB(Sid::concertPitch)) {
                ...
                if (score->isMaster() && part->instrument(tickStart)->transpose() != oldV) {
                    auto i = part->instruments()->upper_bound(tickStart.ticks());
                    Fraction tickEnd = i
                                       == part->instruments()->end() ? Fraction(-1,
                                                                                1) : Fraction::
                                       fromTicks(
                        i->first);
                    transpositionChanged(part, oldV, tickStart, tickEnd);
...
void Score::undoChangeSpannerElements(Spanner* spanner, Element* startElement, Element* endElement)
{
    ...
    int startDeltaTrack = startElement
                          && oldStartElement ? startElement->track() - oldStartElement->track() : 0;
    int endDeltaTrack = endElement
                        && oldEndElement ? endElement->track() - oldEndElement->track() : 0;
    ...
    for (ScoreElement* el : spanner->linkList()) {
        ...
        if (sp != spanner) {
            if (startElement) {
                ...
                int newTrack = sp->startElement() ? sp->startElement()->track()
                               + startDeltaTrack : 0;
