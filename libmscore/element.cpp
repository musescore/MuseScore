...
QColor Element::curColor(bool isVisible, QColor normalColor) const
{
    ...
    if (selected() || marked) {
        ...
        if (isVisible) {
            ...
            return QColor(red + tint * (255 - red), green + tint * (255 - green),
                          blue + tint * (255 - blue));
        }
    }
    if (!isVisible)
...
bool Element::readProperties(XmlReader& e)
{
    ...
    if (readProperty(tag, e, Pid::SIZE_SPATIUM_DEPENDENT)) {
        ...
        if (tag == "linkedMain") {
            ...
            if (!_links)
                qWarning("Element::readProperties: could not link %s at staff %d",
                         name(), mainLoc.staff() + 1);
        }
    } else if (tag == "lid") {
...
ElementType Element::readType(XmlReader& e, QPointF* dragOffset, Fraction* duration)
{
    while (e.readNextStartElement()) {
        if (e.name() == "Element") {
            while (e.readNextStartElement()) {
...
Element* Element::readMimeData(Score* score, const QByteArray& data, QPointF* dragOffset,
                               Fraction* duration)
{
    XmlReader e(data);
...
bool Element::setProperty(Pid propertyId, const QVariant& v)
{
    ...
    break;
    default:
        qDebug("%s unknown <%s>(%d), data <%s>", name(), propertyName(
                   propertyId), int(propertyId), qPrintable(v.toString()));
        return false;
    }
    triggerLayout();
    return true;
...
void Element::drawSymbols(const std::vector<SymId>& s, QPainter* p, const QPointF& o,
                          qreal scale) const
{
        ...
        if (isTextBase()) {
            ...
            for (Measure* pm = m->prevMeasure(); pm; pm = pm->prevMeasure()) {
                if (s2.left() > totalWidth) {
                    s1.add(pm->staffShape(si).translated(QPointF(-(totalWidth + pm->width()),
                                                                 0.0)));
                    totalWidth += pm->width();
                } else {
                    break;
            ...
            // if necessary
            bool cnm = (s2.right() > m->width()) && m->nextMeasure()
