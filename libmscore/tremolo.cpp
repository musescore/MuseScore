...
void Tremolo::layoutOneNoteTremolo(qreal x, qreal y, qreal _spatium)
{
    ...
    if (!placeMidStem()) {
        ...
        // prevent stroke from going out of staff at the bottom while stem direction is up
            ...
            yLine = qMin(yLine, (staff()->lines(
                                     tick()) - 1) * 2 - (2.0 * (lines() - 1)) * td - 2.0 * sw);
        ...
        const qreal stemY
            = stem ? (stem->y() + (up ? stem->bbox().bottom() : stem->bbox().top())) : noteBorder;
...
void Tremolo::layoutTwoNotesTremolo(qreal x, qreal y, qreal h, qreal _spatium)
{
    ...
    if (stem2 && stem1) {
        ...
        const std::pair<qreal, qreal> extendedLen
            = extendedStemLenWithTwoNoteTremolo(this,
                                                _chord1->defaultStemLength(),
        ...
        if (defaultStyle && _chord1->up() != _chord2->up() && !crossStaffBeamBetween()) {
            qreal meanNote1Y = .5
                               * (_chord1->upNote()->pagePos().y() - firstChordStaffY
                                  + _chord1->downNote()->pagePos().y() - firstChordStaffY);
            qreal meanNote2Y = .5
                               * (_chord2->upNote()->pagePos().y() - firstChordStaffY
                                  + _chord2->downNote()->pagePos().y() - firstChordStaffY);
            y1 = .5 * (y1 + meanNote1Y);
            y2 = .5 * (y2 + meanNote2Y);
        }
    ...
    if (_chord1->up() != _chord2->up() && defaultStyle && !crossStaffBeamBetween())
        dy = qMin(qMax(dy,
                       -1.0 * _spatium / defaultLength * dx), 1.0 * _spatium / defaultLength * dx);
    qreal ds = dy / dx;
