...
void HairpinSegment::layout()
{
    ...
    if (autoplace() && !score()->isPalette()) {
        ...
        qreal minDynamicsDistance = score()->styleP(Sid::autoplaceHairpinDynamicsDistance)
                                    * staff()->mag(tick());
        ...
        if (isSingleType() || isEndType()) {
            ...
            if (ed && ed->addToSkyline() && ed->placement() == hairpin()->placement()) {
                ...
                else if (dist >= extendThreshold && hairpin()->endText().isEmpty()
                         && minDynamicsDistance > 0.0)
...
std::unique_ptr<ElementGroup> HairpinSegment::getDragGroup(
