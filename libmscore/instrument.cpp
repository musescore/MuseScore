...
StaffName::StaffName(const QString& s, int p) : _name(s),
    _pos(p)
{
...
void Channel::switchExpressive(Synthesizer* synth, bool expressive, bool force /* = false */)
{
    ...
    if (!info.fontName.contains("MuseScore_General")) {
        qDebug().nospace() << "Soundfont '" << info.fontName
                           << "' is not MuseScore General, cannot update expressive";
        return;
    }
...
PartChannelSettingsLink::PartChannelSettingsLink(PartChannelSettingsLink&& other) :
    ChannelListener(),
    // swap() will set the notifier instead
    _main(nullptr),
    _bound(nullptr),
    _excerpt(false)
{
