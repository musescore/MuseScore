...
public:
    ...
    {
        return _updateMode == UpdateMode::Layout;
    }

    bool updateAll() const
    {
        return int(_updateMode) >= int(UpdateMode::UpdateAll);
    }

    bool updateRange() const
    {
        return _updateMode == UpdateMode::Update;
    }
...
class ScoreContentState
{
    ...
    int num;
public:
    ScoreContentState() : score(nullptr),
        num(0)
    {
    }

    ScoreContentState(const Score* s, int stateNum) : score(s),
        num(stateNum)
    {
    }
    ...
    {
        return score == s2.score && num == s2.num;
    }
...
public:
    ...
    std::vector<Text*> headersText() const
    {
        return _headersText;
    }

    std::vector<Text*> footersText() const
    {
        return _footersText;
    }

    void setHeaderText(Text* t, int index)
    {
        _headersText[index] = t;
    }

    void setFooterText(Text* t, int index)
    {
        _footersText[index] = t;
    }
};
...
private:
    ...
    void deleteSpannersFromRange(const Fraction& t1, const Fraction& t2, int trackStart,
                                 int trackEnd, const SelectionFilter& filter);
    void deleteAnnotationsFromRange(Segment* segStart, Segment* segEnd, int trackStart,
                                    int trackEnd, const SelectionFilter& filter);
...
protected:
    ...
    void createGraceNotesPlayEvents(const Fraction& tick, Chord* chord, int& ontime,
                                    int& trailtime);
    void cmdPitchUp();
    void cmdPitchDown();
...
public:
    ...
    void transposeKeys(int staffStart, int staffEnd, const Fraction& tickStart,
                       const Fraction& tickEnd, const Interval&, bool useInstrument = false,
                       bool flip = false);
    ...
    {
        return ((n >= 0) && (n < _staves.size())) ? _staves.at(n) : nullptr;
    }

    Measure* pos2measure(const QPointF&, int* staffIdx, int* pitch, Segment**,
    ...
    bool undoPropertyChanged(Element* e, Pid t, const QVariant& st,
    ...
    void updateInstrumentChangeTranspositions(Ms::KeySigEvent& key, Ms::Staff* staff,
                                              const Ms::Fraction& tick);

    Note* setGraceNote(Chord*, int pitch, NoteType type, int len);

    Segment* setNoteRest(Segment*, int track, NoteVal nval, Fraction,
                         Direction stemDirection = Direction::AUTO, bool forceAccidental = false,
                         bool rhythmic = false, InputState* externalInputState = nullptr);
    Segment* setChord(Segment*, int track, Chord* chord, Fraction,
    ...
    bool makeGap1(const Fraction& baseTick, int staffIdx, const Fraction& len,
    ...
    void cloneVoice(int strack, int dtrack, Segment* sf, const Fraction& lTick, bool link = true,
                    bool spanner = true);
    ...
    bool playNote() const
    {
        return _updateState._playNote;
    }

    void setPlayNote(bool v)
    {
        _updateState._playNote = v;
    }

    bool playChord() const
    {
        return _updateState._playChord;
    }

    void setPlayChord(bool v)
    {
        _updateState._playChord = v;
    }

    bool selectionChanged() const
    {
        return _updateState._selectionChanged;
    }

    void setSelectionChanged(bool val)
    {
        _updateState._selectionChanged = val;
    }

    void deleteLater(ScoreElement* e)
    {
        _updateState._deleteList.push_back(e);
    }
    ...
    {
        return _showInvisible;
    }

    bool showUnprintable() const
    {
        return _showUnprintable;
    }
    ...
    {
        return _markIrregularMeasures;
    }

    bool showInstrumentNames() const
    {
        return _showInstrumentNames;
    }
    ...
    bool saveCompressedFile(QIODevice*, const QFileInfo&, bool onlySelection,
    ...
    {
        return ((qint64)t * MScore::division + _fileDivision/2) / _fileDivision;
    }
    ...
    qreal spatium() const
    {
        return styleD(Sid::spatium);
    }

    void setSpatium(qreal v)
    {
        setStyleValue(Sid::spatium, v);
    }

    bool genCourtesyTimesig() const
    {
        return styleB(Sid::genCourtesyTimesig);
    }

    bool genCourtesyClef() const
    {
        return styleB(Sid::genCourtesyClef);
    }
    ...
    {
        setPos(POS::LEFT, tick);
    }

    void setLoopOutTick(const Fraction& tick)
    {
        setPos(POS::RIGHT, tick);
    }
    ...
    {
        return inputState().noteEntryMode();
    }

    void setNoteEntryMode(bool val)
    {
        inputState().setNoteEntryMode(val);
    }

    NoteEntryMethod noteEntryMethod() const
    {
        return inputState().noteEntryMethod();
    }

    void setNoteEntryMethod(NoteEntryMethod m)
    {
        inputState().setNoteEntryMethod(m);
    }

    bool usingNoteEntryMethod(NoteEntryMethod m)
    {
        return inputState().usingNoteEntryMethod(m);
    }
    ...
    Measure* searchMeasure(const QPointF& p, const System* preferredSystem = nullptr,
                           qreal spacingFactor = 0.5) const;
...
// void moveBracket(int staffIdx, int srcCol, int dstCol);
    ...
    Measure* searchLabel(const QString& s, Measure* startMeasure = nullptr,
                         Measure* endMeasure = nullptr);
    Measure* searchLabelWithinSectionFirst(const QString& s, Measure* sectionStartMeasure,
    ...
    void createRevision();
    void writeSegments(XmlWriter& xml, int strack, int etrack, Segment* sseg, Segment* eseg, bool,
                       bool);
    ...
    {
        return _mscoreVersion;
    }

    int mscoreRevision() const
    {
        return _mscoreRevision;
    }
    ...
    {
        return _layer[_currentLayer].tags;
    }
    ...
    {
        return layoutMode() == LayoutMode::FLOAT;
    }
    ...
    bool lineMode() const
    {
        return layoutMode() == LayoutMode::LINE;
    }

    bool systemMode() const
    {
        return layoutMode() == LayoutMode::SYSTEM;
    }
    ...
    void transpositionChanged(Part*, Interval, Fraction tickStart = { 0, 1 },
...
// Q_INVOKABLE void cropPage(qreal margins);
    ...
    Text* headerText(int index) const
    {
        return movements()->headersText()[index];
    }

    Text* footerText(int index) const
    {
        return movements()->footersText()[index];
    }

    void setHeaderText(Text* t, int index)
    {
        movements()->setHeaderText(t, index);
    }

    void setFooterText(Text* t, int index)
    {
        movements()->setFooterText(t, index);
    }
    ...
    constexpr static double defaultTempo()
    {
...
public:
    ...
    void setLayout(const Fraction& tick1, const Fraction& tick2, int staff1, int staff2,
                   const Element* e = nullptr);
    ...
    {
        _cmdState.layoutFlags |= val;
    }

    virtual void setInstrumentsChanged(bool val) override
    {
        _cmdState._instrumentsChanged = val;
    }

    void setExcerptsChanged(bool val)
    {
        _cmdState._excerptsChanged = val;
    }

    bool excerptsChanged() const
    {
        return _cmdState._excerptsChanged;
    }

    bool instrumentsChanged() const
    {
        return _cmdState._instrumentsChanged;
    }
    ...
    std::shared_ptr<Avs::AvsOmr> avsOmr() const
    {
        return _avsOmr;
    }

    void setAvsOmr(std::shared_ptr<Avs::AvsOmr> omr)
    {
        _avsOmr = omr;
    }
    ...
    {
        return _midiMapping[c->channel()].articulation();
    }
    ...
    {
        return _sessionStartBackupInfo;
    }
...
inline UndoStack* Score::undoStack() const
{
    return _masterScore->undoStack();
}

inline const RepeatList& Score::repeatList()  const
{
    return _masterScore->repeatList();
}

inline TempoMap* Score::tempomap() const
{
    return _masterScore->tempomap();
}

inline TimeSigMap* Score::sigmap() const
{
    return _masterScore->sigmap();
}

inline QList<Excerpt*>& Score::excerpts()
{
    return _masterScore->excerpts();
}

inline const QList<Excerpt*>& Score::excerpts() const
{
    return _masterScore->excerpts();
}

inline QQueue<MidiInputEvent>* Score::midiInputQueue()
{
    return _masterScore->midiInputQueue();
}

inline std::list<MidiInputEvent>* Score::activeMidiPitches()
{
    return _masterScore->activeMidiPitches();
}

inline void Score::setUpdateAll()
{
    _masterScore->setUpdateAll();
}

inline void Score::setLayoutAll(int staff, const Element* e)
{
    _masterScore->setLayoutAll(staff, e);
}
...
inline void Score::setLayout(const Fraction& tick1, const Fraction& tick2, int staff1, int staff2,
                             const Element* e)
{
    _masterScore->setLayout(tick1, tick2, staff1, staff2, e);
}

inline CmdState& Score::cmdState()
{
    return _masterScore->cmdState();
}

inline const CmdState& Score::cmdState() const
{
    return _masterScore->cmdState();
}

inline void Score::addLayoutFlags(LayoutFlags f)
{
    _masterScore->addLayoutFlags(f);
}

inline void Score::setInstrumentsChanged(bool v)
{
    _masterScore->setInstrumentsChanged(v);
}

inline Movements* Score::movements()
{
    return _masterScore->movements();
}

inline const Movements* Score::movements() const
{
    return _masterScore->movements();
}

inline Fraction Score::pos(POS pos) const
{
    return _masterScore->pos(pos);
}

inline void Score::setPos(POS pos, Fraction tick)
{
    _masterScore->setPos(pos, tick);
}
