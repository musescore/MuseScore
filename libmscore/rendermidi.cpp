...
//---------------------------------------------------------
// swingAdjustParams
//    chord       the chord for which to adjust swing parameters
//    ontime      IN/OUT, the chord on-time (in 1/1000 of nominal chord duration)
//    gateTime    IN/OUT, the chord off-time (in 1/1000 of nominal chord duration)
//    swingUnit   the type of notes to which swing is applied (8th, 16th, ..., in ticks)
//    swingRatio  the swing ratio (in percent of nominal beat duration)
//---------------------------------------------------------

void Score::swingAdjustParams(Chord* chord, int& ontime, int& gateTime, int swingUnit, int swingRatio)
      {
...
int totalTiedNoteTicks(Note* note, int defOnTime, int defGateTime)
      ...
      int   timeOffset  = (note->onTimeOffset() != DEFAULT_NOTE_ONTIME_OFFSET ?
                  note->onTimeOffset() * PERCENT2PERTHOUSAND : defOnTime);
      ...
      if (note) {
            timeOffset = (note->offTimeOffset() != DEFAULT_NOTE_OFFTIME_OFFSET
                        ? note->offTimeOffset()*PERCENT2PERTHOUSAND
                        : defGateTime - FULL_NOTE_RENDERING_GATE);
...
//    prefix, body, suffix    vectors defining the ornament pattern
...
bool renderNoteArticulation(NoteEventList* events, Note * note, bool chromatic, int requestedTicksPerNote,
   ...
   int fastestFreq=16, int slowestFreq=8 // 16 Hz and 8 Hz
      ...
      int   numrepeat   = 1;
//      int   sustain     = 0;
      // apply note on-time offset, if any
      int   ontime      = ((note->onTimeOffset() ? note->onTimeOffset()*PERCENT2PERTHOUSAND : defOnTime)
                  // and convert to 1/1000 of ticks
                  * note->chord()->actualTicks() * TICKS2TICK1000) / FULL_NOTE_RENDERING_GATE;
      ...
      int   prefSuffLength    = millespernote*(p + s);
      int   bodyRepLength     = millespernote * b;
      // calculate the number of times to repeat the body, and sustain the last note
      // 1000 = P + numrepeat*B + S + sustain
      if (repeatp)
            numrepeat = (space - prefSuffLength) / bodyRepLength;
//      if (sustainp)
//            sustain   = space - millespernote*(p + numrepeat * b + s);
      ...
      if (padding > 0) {
            ...
            int         pad   = padding/chord->actualTicks();
...
void renderChordArticulation(Chord *chord, QList<NoteEventList> & ell, int ontime, int & gateTime)
      ...
      for (int k = 0; k < chord->notes().size(); ++k) {
            ...
            int               noteOnTime  = note->onTimeOffset();
            int               noteOffTime = note->offTimeOffset();
            Trill             *trill;
            retVal      = false;          // assume articulation rendering false
            if (noteOnTime != DEFAULT_NOTE_ONTIME_OFFSET)
                  ontime = noteOnTime * PERCENT2PERTHOUSAND;
            if (noteOffTime != DEFAULT_NOTE_OFFTIME_OFFSET)
                  gateTime = (FULL_NOTE_TIME_GATE + noteOffTime) * PERCENT2PERTHOUSAND;
            ...
            if (noteHasGlissando(note))
                  ...
                  for (Articulation* a : chord->articulations()) {
                        ...
                        // and note has not a custom off-time,
                        // set gate time to articulation specifics
                        if (!retVal && noteOffTime == DEFAULT_NOTE_OFFTIME_OFFSET)
                              gateTime = instr->gateTime(gateTime / PERCENT2PERTHOUSAND,
                                          channel, a->subtypeName()) * PERCENT2PERTHOUSAND;
                        }
                  }
            if (events->count() < 1)
...
/* Removed 'time gating' of individual note events;
 * 'time gating' of the note whole event block is now in renderChordArticulation()
      //
      //    apply gateTime
      //
      for (int i = 0; i < notes; ++i) {
...
void Score::createPlayEvents(Chord* chord)
      ...
      if (!slur) {
            Instrument* instr = chord->part()->instrument(tick);
            gateTime = instr->gateTime(gateTime, 0, "");
            }
      ...
      if (unit) {
            swingAdjustParams(chord, ontime, gateTime, unit, ratio);
            }
      //
