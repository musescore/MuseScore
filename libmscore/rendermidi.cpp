...
struct SndConfig {
    ...
    SndConfig(bool use, int c, DynamicsRenderMethod me) : useSND(use),
        controller(c),
        method(me)
    {
    }
        ...
        for (Staff* st : staves()) {
            ...
            for (int track = strack; track < etrack; ++track) {
                ...
                if (channel >= instr->channel().size()) {
                    qDebug() << "Channel " << channel << " too high. Max "
                             << instr->channel().size();
                    channel = 0;
                }
...
static void playNote(EventMap* events, const Note* note, int channel, int pitch, int velo,
                     int onTime, int offTime, int staffIdx)
{
    if (!note->play())
        return;
...
static void collectNote(EventMap* events, int channel, const Note* note, qreal velocityMultiplier,
                        int tickOffset, Staff* staff, SndConfig config)
{
...
static void collectProgramChanges(EventMap* events, Measure* m, Staff* staff, int tickOffset)
{
    ...
    for (Segment* s = m->first(SegmentType::ChordRest); s; s = s->next(SegmentType::ChordRest)) {
        for (Element* e : s->annotations()) {
            if (!e->isStaffTextBase() || e->staffIdx() < firstStaffIdx
...
void MidiRenderer::collectMeasureEventsSimple(EventMap* events, Measure* m,
                                              const StaffContext& sctx, int tickOffset)
{
...
void MidiRenderer::collectMeasureEventsDefault(EventMap* events, Measure* m,
                                               const StaffContext& sctx, int tickOffset)
{
...
void MidiRenderer::collectMeasureEvents(EventMap* events, Measure* m, const StaffContext& sctx,
                                        int tickOffset)
{
    switch (sctx.method) {
...
void MidiRenderer::renderSpanners(const Chunk& chunk, EventMap* events)
{
    ...
    for (const auto& sp : score->spannerMap().map()) {
        ...
        if (s->isPedal() || s->isLetRing()) {
            channelPedalEvents.insert({channel, std::vector<std::pair<int, std::pair<bool,
                                                                                     int> > >()});
            std::vector<std::pair<int,
                                  std::pair<bool, int> > > pedalEventList = channelPedalEvents.at(
                channel);
            ...
            if (!pedalEventList.empty())
                ...
                lastEvent
                    = std::pair<int, std::pair<bool, int> >(0, std::pair<bool, int>(true, staff));
...
void Score::swingAdjustParams(Chord* chord, int& gateTime, int& ontime, int swingUnit,
                              int swingRatio)
{
    ...
    if (ds) {
        for (Note* n : chord->notes()) {
            DrumInstrumentVariant div = ds->findVariant(n->pitch(),
                                                        chord->articulations(), chord->tremolo());
...
void renderArpeggio(Chord* chord, QList<NoteEventList>& ell)
{
    ...
    for (int i = start; i != end; i += step) {
        ...
        auto tempoRatio = chord->score()->tempomap()->tempo(chord->tick().ticks())
                          / Score::defaultTempo();
                            ...
                            )
{
    ...
    auto makeEvent = [note, chord, chromatic, events](int pitch, int ontime, int duration) {
                         events->append(NoteEvent(chromatic ? pitch : articulationExcursion(note,
                                                                                            note,
                                                                                            pitch),
    ...
    auto graceExtend
        = [millespernote, chord, events](int notePitch, QVector<Chord*> graceNotes, int ontime) {
...
bool renderNoteArticulation(NoteEventList* events, Note* note, bool chromatic,
                            SymId articulationType, MScore::OrnamentStyle ornamentStyle)
{
...
bool renderNoteArticulation(NoteEventList* events, Note* note, bool chromatic,
                            Trill::Type trillType, MScore::OrnamentStyle ornamentStyle)
{
        ...
        if (spanner->type() == ElementType::GLISSANDO) {
            ...
            if (glissando->playGlissando() && ElementType::NOTE == ee->type()) {
                ...
                if (glissandoStyle == GlissandoStyle::DIATONIC) {         // scale obeying accidentals
                    ...
                    for (int p = pitchstart; direction ? p < pitchend : p > pitchend;
                         p += (direction ? 1 : -1)) {
                        bool choose = false;
...
void Score::createGraceNotesPlayEvents(const Fraction& tick, Chord* chord, int& ontime,
                                       int& trailtime)
{
...
void MidiRenderer::renderMetronome(EventMap* events, Measure* m, const Fraction& tickOffset)
{
    ...
    for (int tick = msrTick; tick < endTick; tick += clickTicks, rtick += clickTicks)
        events->insert(std::pair<int, NPlayEvent>(tick + tickOffset.ticks(),
                                                  NPlayEvent(timeSig.rtick2beatType(rtick))));
}
