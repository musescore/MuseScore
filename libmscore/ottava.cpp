...
Sid Ottava::getPropertyStyle(Pid pid) const
{
    ...
    case Pid::END_HOOK_HEIGHT: {
        if (isStyled(Pid::PLACEMENT))
            return score()->styleI(ss[idx])
                   == int(Placement::ABOVE) ? Sid::ottavaHookAbove : Sid::ottavaHookBelow;
        else
...
void Ottava::read(XmlReader& e)
{
    ...
    while (e.readNextStartElement())
        readProperties(e);
    if (_ottavaType != OttavaType::OTTAVA_8VA
        || _numbersOnly != propertyDefault(Pid::NUMBERS_ONLY).toBool())
        styleChanged();
}
...
{
    return QString("%1: %2").arg(Element::accessibleInfo()).arg(ottavaDefault[static_cast<int>(
                                                                                  ottavaType())].name);
}
