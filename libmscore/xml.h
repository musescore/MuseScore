...
public:
    XmlReader(QFile* f) : QXmlStreamReader(f),
        docName(f->fileName())
    {
    }

    XmlReader(const QByteArray& d, const QString& st = QString()) : QXmlStreamReader(d),
        docName(st)
    {
    }

    XmlReader(QIODevice* d, const QString& st = QString()) : QXmlStreamReader(d),
        docName(st)
    {
    }

    XmlReader(const QString& d, const QString& st = QString()) : QXmlStreamReader(d),
        docName(st)
    {
    }
    ...
    int readIntHex()
    {
        return readElementText().toInt(0, 16);
    }
    ...
    Fraction tick()  const
    {
        return _tick + _tickOffset;
    }

    Fraction rtick()  const;
    Fraction tickOffset() const
    {
        return _tickOffset;
    }
    ...
    {
        _pendingConnectors.push_back(std::move(c));
    }                                                                                                                    // add connector info to be checked after calling checkConnectors()
    ...
    {
        _transpose.chromatic = v;
    }

    void setTransposeDiatonic(int v)
    {
        _transpose.diatonic = v;
    }
    ...
    QMap<int, QList<QPair<LinkedElements*, Location> > >& staffLinkedElements()
    {
        return _staffLinkedElements;
    }

    void setOffsetLines(qint64 val)
    {
        _offsetLines = val;
    }
};
...
public:
    ...
    {
        _lidLocalIndices.insert(lid, localIndex);
    }

    int lidLocalIndex(int lid) const
    {
        Q_ASSERT(_lidLocalIndices.contains(lid));
    ...
    {
        _recordElements = record;
    }

    void sTag(const char* name, Spatium sp)
    {
        XmlWriter::tag(name, QVariant(sp.val()));
    }
