...
void ShadowNote::draw(QPainter* painter) const
{
    ...
    QPen pen(MScore::selectColor[_voice].lighter(SHADOW_NOTE_LIGHT), lw, Qt::SolidLine,
             Qt::RoundCap);
    painter->setPen(pen);
    ...
    if (flag != SymId::noSym) {
        ...
        qreal yOffset = up
                        == 1 ? symStemUpSE(_notehead).y() * magS() : symStemDownNW(_notehead).y()
                        * magS();
        if (flag != SymId::lastSym) {
            qreal flagHeight = (_duration.type() < TDuration::DurationType::V_256TH) ? symHeight(
                flag) : symHeight(flag) / 2;
            pos.ry() -= up
                        * (flagHeight
                           + (posDot.y() != 0 ? posDot.y() + spatium() : 0) + 0.5 * spatium());
            painter->drawLine(QLineF(pos.x(), yOffset, pos.x(), pos.y() - up * (yOffset + lw / 2)));
            pos.rx() -= (lw / 2);       // flag offset?
            drawSymbol(flag, painter, pos, 1);
        } else {
            painter->drawLine(QLineF(pos.x(), yOffset, pos.x(),
                                     -3 * up * spatium() * mag() + yOffset));
        }
    }
...
void ShadowNote::layout()
{
    ...
    if (flag == SymId::noSym) {
        b.setRect(noteheadBbox.x(), noteheadBbox.y(),
