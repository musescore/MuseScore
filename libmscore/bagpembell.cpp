...
void BagpipeEmbellishment::layout()
{
    ...
    foreach (int note, nl) {
        ...
        // head
        addbbox(score()->scoreFont()->bbox(headsym,
                                           dx.mags).translated(QPointF(x - dx.lw * .5 - dx.headw,
                                                                       dy.y2)));
        /*
        if (_embelType == 0 || _embelType == 8 || _embelType == 9) {
        ...
        if (drawFlag) {
            addbbox(score()->scoreFont()->bbox(flagsym,
                                               dx.mags).translated(QPointF(x - dx.lw * .5
                                                                           + dx.xcorr,
                                                                           dy.y1f + dy.ycorr)));
        ...
        if (line == -2) {
            addbbox(QRectF(x - dx.headw * 1.5 - dx.lw * .5, dy.y2 - dx.lw * 2, dx.headw * 2,
                           dx.lw));
            /*
            if (_embelType == 8) {
...
static void drawBeams(QPainter* painter, const qreal spatium, const qreal x1, const qreal x2,
                      qreal y)
{
    // draw the beams
...
void BagpipeEmbellishment::drawGraceNote(QPainter* painter, const BEDrawingDataX& dx,
                                         const BEDrawingDataY& dy, SymId flagsym, const qreal x,
