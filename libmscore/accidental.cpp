...
struct Acc {
    ...
    int centOffset;
    SymId sym;
    Acc(AccidentalVal o, int o2, SymId s) : offset(o),
        centOffset(o2),
        sym(s)
    {
    }
...
void Accidental::layout()
{
    ...
    if (_bracket != AccidentalBracket::NONE) {
        SymId id = _bracket
                   == AccidentalBracket::PARENTHESIS ? SymId::accidentalParensLeft : SymId::
    ...
    if (_bracket != AccidentalBracket::NONE) {
        SymId id = _bracket
                   == AccidentalBracket::PARENTHESIS ? SymId::accidentalParensRight : SymId::
...
bool Accidental::acceptDrop(EditData& data) const
{
    ...
    return e->isIcon()
           && (toIcon(e)->iconType() == IconType::BRACKETS
               || toIcon(e)->iconType() == IconType::PARENTHESES);
}
