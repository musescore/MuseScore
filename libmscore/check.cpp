        ...
        for (Segment* s = firstMeasure()->first(SegmentType::ChordRest); s;
            ...
            if (s->tick() != tick) {
                if (lcr) {
                    ...
                    qDebug(
                        "Chord/Rest gap at tick %d(%s+%d)-%d(%s) staffIdx %d measure %d (len = %d)",
                ...
                if (cr->tick() > tick) {
                    ...
                    while (ticks > 0) {
                        ...
                        for (const Duration& d, toDurationList(fff, true)) {
                            ...
                            qDebug("    -   Rest %d/%d", d.fraction().numerator(),
...
bool Score::sanityCheck(const QString& name)
{
    ...
    for (Measure* m = firstMeasure(); m; m = m->nextMeasure()) {
        ...
        for (int staffIdx = 0; staffIdx < endStaff; ++staffIdx) {
            ...
            if (voices[0] != mLen) {
                QString msg
                    = QObject::tr("Measure %1, staff %2 incomplete. Expected: %3; Found: %4").arg(
                          mNumber).arg(staffIdx+1).arg(mLen.print()).arg(voices[0].print());
                qDebug() << msg;
                error += QString("%1\n").arg(msg);
#ifndef NDEBUG
            ...
            for (int v = 1; v < VOICES; ++v) {
                if (voices[v] > mLen) {
                    QString msg = QObject::tr(
                        "Measure %1, staff %2, voice %3 too long. Expected: %4; Found: %5").arg(
                        mNumber).arg(staffIdx + 1).arg(v+1).arg(mLen.print()).arg(
                        voices[v].print());
                    qDebug() << msg;
                    error += QString("%1\n").arg(msg);
#ifndef NDEBUG
...
// TODO:ws   what about clefs not at measure start?
    ...
    for (Staff* staff : _staves) {
        ...
        for (Measure* m = firstMeasure(); m; m = m->nextMeasure()) {
            if (prevMeasure) {
                Segment* segment = prevMeasure->findSegmentR(
                    SegmentType::Clef | SegmentType::HeaderClef, 0);
                if (segment) {
            ...
            if (mapClefType != clefType) {
                qDebug("measure %d (tick %d) : clef %d, map %d", m->no(),
                       m->tick(), int(clefType), int(mapClefType));
                rc = false;
            }
            prevMeasure = m;
        }
...
void Measure::checkMeasure(int staffIdx)
{
    ...
    for (int track = strack; track < dtrack; track++) {
        ...
        if (f > expectedPos) {
            ...
            qDebug("measure overrun %6d, %d > %d, track %d", tick().ticks(),
