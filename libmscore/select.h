...
public:
    ...
    {
        _score = 0;
        _filtered = (int)SelectionFilterType::ALL;
    }

    SelectionFilter(SelectionFilterType f) : _score(nullptr),
        _filtered(int(f))
    {
    }
...
class Selection
{
    ...
    {
        return selectionFilter().canSelect(e);
    }
...
public:
    ...
    bool isNone() const
    {
        return _state == SelState::NONE;
    }

    bool isRange() const
    {
        return _state == SelState::RANGE;
    }

    bool isList() const
    {
        return _state == SelState::LIST;
    }
    ...
    void unlock(const QString& reason)
    {
        Q_UNUSED(reason);
        _lockReason.clear();
    }                                                                                // reason for clarity
    ...
    bool isLocked() const
    {
        return !_lockReason.isEmpty();
    }

    const QString& lockReason() const
    {
        return _lockReason;
    }
    ...
    bool isSingle() const
    {
        return (_state == SelState::LIST) && (_el.size() == 1);
    }
