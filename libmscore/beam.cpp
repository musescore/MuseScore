...
struct Bm {
    ...
    Bm(signed char a, signed char b) : l(a),
        s(b)
    {
    }
...
void Beam::computeStemLen(const std::vector<ChordRest*>& cl, qreal& py1, int beamLevels)
{
    ...
    qreal _spStaff4 = staff()->isTabStaff(Fraction(0, 1)) ? _spatium4 * staff()->lineDistance(Fraction(
                                                                                                  0,
                                                                                                  1))
                      : _spatium4;
    ...
    for (const ChordRest* cr : cl) {
        if (cr->isChord()) {
            ...
            const qreal crStemAbsLen = std::abs(
                (p2.x() - p1.x()) * slope - p2.y() + p1.y() + firstStemLenPoints);
    ...
    for (int beamLevel = 0; beamLevel < beamLevels; ++beamLevel) {
        ...
        for (size_t i = 0; i < n;) {
            ...
            if (beamLevel > baseLevel) {
                ...
                } else if (!currentChordRestIndex && (chordRestEndGroupIndex < n)
                           && (cr1->up() != cr2->up())) {
            ...
            qreal stemWidth
                = (cr1->isChord()
                   && toChord(cr1)->stem()) ? toChord(cr1)->stem()->lineWidthMag() : 0.0;
            ...
            if ((chordRestEndGroupIndex - currentChordRestIndex) > 1) {
                ...
                if (tab) {
                    ...
                    if (cr1->up())
                        x2 -= stemWidth;
                    if (!chordRest2->up())
                        x3 += (chordRest2->isChord() && toChord(chordRest2)->stem()) ? toChord(
                            chordRest2)->stem()->lineWidthMag() : 0.0;
                }
            } else {
                ...
                if (currentChordRestIndex == 0) {
                    ...
                    if (currentHooks - beamsOut > 1 && beamsIn > beamsOut
                        && currentHooks > beamsIn) {
                        ...
                        if (mod <= BEAM_TUPLET_TOLERANCE
                            || (tickMod - mod) <= BEAM_TUPLET_TOLERANCE)
                            len = -len;
                    }
                }
                if (tab) {
...
public:
    ...
    {
        return EditDataType::BeamEditData;
    }
};
