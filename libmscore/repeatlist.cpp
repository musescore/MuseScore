...
RepeatSegment::RepeatSegment(RepeatSegment* const rs, Measure* const fromMeasure,
...
int RepeatList::utime2utick(qreal t) const
{
    ...
    for (unsigned i = ii; i < n; ++i) {
        if ((t >= at(i)->utime) && ((i + 1 == n) || (t < at(i+1)->utime))) {
            idx2 = i;
            return _score->tempomap()->time2tick(t - at(i)->timeOffset)
                   + (at(i)->utick - at(i)->tick);
        }
    }
    if (MScore::debugMode)
        ...
        if ((startFrom->playbackCount() == startFromRepeatStartCount)        // means last pass through this set of repeats
            ...
            if (currentMeasure->repeatJump()) {       // found a jump, should we follow it?
                ...
                if (_jumpsTaken.find(jump) == _jumpsTaken.end()) {         // not yet processed
                    ...
                    if (jumpToMeasure && playUntilMeasure
                        && (continueAtMeasure || jump->continueAt().isEmpty())) {
                            ...
                            if (jump->playRepeats()) {
                                ...
                                for (int idx = copyFromIdx; idx <= copyUntilIdx; ++idx) {
                                    this->push_back(new RepeatSegment(
                                                        this->at(idx),
                                                        (idx
                                                         == copyFromIdx) ? copyFromMeasure : nullptr,
                                                        (idx
                                                         == copyUntilIdx) ? copyUntilMeasure :
                                                        nullptr
                                                        )
                                                    );
                                }
                                ...
                                while (idx <= copyUntilIdx) {
                                    ...
                                    while (((referenceIt != referenceSegment->measureList.cend())
                                        ...
                                        while ((mostRecentRepeatSegmentIdx > idx)
                                               && !forwardToMoreRecentPlaythrough) {
                                            RepeatSegment* mostRecentSegment = this->at(
                                        ...
                                        if (!forwardToMoreRecentPlaythrough) {
                                            ...
                                            rs->measureList.push_back(std::make_pair(referenceIt->
                                                                                     first,
                                                                                     referenceIt->
                                                                                     second));
                                        }
...
bool RepeatList::isFinalPlaythrough(Measure* const measure,
                                    QList<RepeatSegment*>::const_iterator repeatSegmentIt) const
{
