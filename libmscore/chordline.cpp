    ...
    if (!modified) {
        ...
        if (_chordLineType != ChordLineType::NOTYPE) {
            ...
            // chordlines to the left of the note
                     ...
                     || _chordLineType == ChordLineType::SCOOP) {
                if (_straight)
                    path.lineTo(x2, y2);
                else
...
void ChordLine::editDrag(EditData& ed)
{
    ...
    int slideBoundary = 5;
    if ((_chordLineType == ChordLineType::PLOP || _chordLineType == ChordLineType::FALL)
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (_straight) {
            ...
            if ((_chordLineType == ChordLineType::FALL
                ...
                || (_chordLineType == ChordLineType::DOIT
                    && (e.x + dx < startPoint.x || e.y + dy > startPoint.y))
                || (_chordLineType == ChordLineType::SCOOP
                    && (e.x + dx > startPoint.x || e.y + dy < startPoint.y))
                || (_chordLineType == ChordLineType::PLOP
                    && (e.x + dx > startPoint.x || e.y + dy > startPoint.y)))
                return;
        }
