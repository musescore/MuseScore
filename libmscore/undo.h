...
// #endif
    ...
    virtual bool isFiltered(Filter, const Element* /* target */) const
    {
        return false;
    }
...
public:
    ...
    {
        return curIdx > 0 ? list[curIdx-1] : 0;
    }

    UndoMacro* prev() const
    {
        return curIdx > 1 ? list[curIdx-2] : 0;
    }
...
public:
    CloneVoice(Segment* sf, const Fraction& lTick, Segment* d, int strack, int dtrack, int otrack,
               bool linked = true);
    virtual void undo(EditData*) override;
    virtual void redo(EditData*) override;
...
public:
    EditText(Text* t, const QString& ot, int /*l*/) : text(t),
        oldText(ot)                                                         /*, undoLevel(l)*/
    {
    }
...
public:
    ChangeStaff(Staff*, bool invisible, ClefTypeList _clefType, qreal userDist,
...
public:
    ChangeStaffType(Staff* s, const StaffType& t) : staff(s),
        staffType(t)
    {
    }
...
public:
    ChangeStyleVal(Score* s, Sid i, const QVariant& v) : score(s),
        idx(i),
        value(v)
    {
    }
...
public:
    ChangePageNumberOffset(Score* s, int po) : score(s),
        pageOffset(po)
    {
    }
...
public:
    InsertRemoveMeasures(MeasureBase* _fm, MeasureBase* _lm) : fm(_fm),
        lm(_lm)
    {
    }
...
public:
    SwapExcerpt(MasterScore* s, int p1, int p2) : score(s),
        pos1(p1),
        pos2(p2)
    {
    }
...
public:
    ChangeExcerptTitle(Excerpt* x, const QString& t) : excerpt(x),
        title(t)
    {
    }
...
public:
    ChangeBend(Bend* b, QList<PitchValue> p) : bend(b),
        points(p)
    {
    }
...
public:
    ChangeTremoloBar(TremoloBar* b, QList<PitchValue> p) : bend(b),
        points(p)
    {
    }
...
public:
    ChangeChordPlayEventType(Chord* c, Ms::PlayEventType pet) : chord(c),
        petype(pet)
    {
...
public:
    ChangeInstrument(InstrumentChange* _is, Instrument* i) : is(_is),
        instrument(i)
    {
    }
...
public:
    SwapCR(ChordRest* a, ChordRest* b) : cr1(a),
        cr2(b)
    {
    }
...
public:
    MoveStaff(Staff* s, Part* p, int idx) : staff(s),
        part(p),
        rstaff(idx)
    {
    }
...
public:
    ChangeProperty(ScoreElement* e, Pid i, const QVariant& v,
    ...
    bool isFiltered(UndoCommand::Filter f, const Element* target) const override
    {
        return f == UndoCommand::Filter::ChangePropertyLinked
               && target->linkList().contains(element);
    }
};
...
public:
    ChangeBracketProperty(Staff* s, int l, Pid i, const QVariant& v,
                          PropertyFlags ps = PropertyFlags::NOSTYLE) :
        ChangeProperty(nullptr, i, v, ps),
        staff(s),
        level(l)
    {
    }
...
public:
    ChangeMetaText(Score* s, const QString& i, const QString& t) : score(s),
        id(i),
        text(t)
    {
    }
...
public:
    ChangeSynthesizerState(Score* s, const SynthesizerState& st) : score(s),
        state(st)
    {
    }
...
public:
    RemoveBracket(Staff* s, int l, BracketType t, int sp) : staff(s),
        level(l),
        type(t),
        span(sp)
    {
...
public:
    AddBracket(Staff* s, int l, BracketType t, int sp) : staff(s),
        level(l),
        type(t),
        span(sp)
    {
...
public:
    ChangeParent(Element* e, Element* p, int si) : element(e),
        parent(p),
        staffIdx(si)
    {
    }
...
public:
    ChangeMMRest(Measure* _m, Measure* _mmr) : m(_m),
        mmrest(_mmr)
    {
    }
...
public:
    ChangeStartEndSpanner(Spanner* sp, Element* s, Element* e) : spanner(sp),
        start(s),
        end(e)
    {
    }
...
public:
    ChangeMetaTags(Score* s, const QMap<QString, QString>& m) : score(s),
        metaTags(m)
    {
    }
...
public:
    ChangeDrumset(Instrument* i, const Drumset* d) : instrument(i),
        drumset(*d)
    {
    }
...
public:
    FretMarker(FretDiagram* d, int _string, FretMarkerType _mtype) : diagram(d),
        string(_string),
        mtype(_mtype)
    {
    }
...
public:
    FretBarre(FretDiagram* d, int _string, int _fret, bool _add = false) : diagram(d),
        string(_string),
        fret(_fret),
        add(_add)
    {
...
public:
    MoveTremolo(Score* s, Fraction c1, Fraction c2, Tremolo* tr, int t) : score(s),
        chord1Tick(c1),
        chord2Tick(c2),
        trem(tr),
        track(t)
