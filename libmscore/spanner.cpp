...
public:
    SpannerWriter(XmlWriter& xml, const Element* current, const Spanner* spanner, int track,
...
void Spanner::computeEndElement()
{
    ...
    case Anchor::SEGMENT: {
        ...
        if (isLyricsLine() && toLyricsLine(this)->isEndMelisma()) {
            ...
            Fraction tick
                = (l->ticks().ticks() == Lyrics::TEMP_MELISMA_TICKS) ? l->tick() : l->endTick();
...
SpannerSegment* Spanner::getNextLayoutSystemSegment(System* system,
                                                    std::function<SpannerSegment* ()> createSegment)
{
...
void Spanner::writeSpannerStart(XmlWriter& xml, const Element* current, int track,
                                Fraction tick) const
{
...
void Spanner::writeSpannerEnd(XmlWriter& xml, const Element* current, int track,
                              Fraction tick) const
{
    Fraction frac = fraction(xml, current, tick);
...
SpannerWriter::SpannerWriter(XmlWriter& xml, const Element* current, const Spanner* sp, int track,
                             Fraction frac, bool start) :
    ConnectorInfoWriter(xml, current, sp, track, frac)
{
    ...
    if (current->isMeasure() || current->isSegment()
