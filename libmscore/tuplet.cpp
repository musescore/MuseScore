...
{
    for (auto p :
         { Pid::FONT_FACE, Pid::FONT_STYLE, Pid::FONT_SIZE, Pid::ALIGN,
...
void Tuplet::layout()
{
    ...
    if (outOfStaff && cr1->isChordRest() && cr2->isChordRest()) {
        ...
        if (toChordRest(cr1)->staffMove() == toChordRest(cr2)->staffMove() && !tuplet()
            && !nested) {
...
{
    qreal w2 = w * .5;
    setCoords(qMin(p1.x(), p2.x()) - w2, qMin(p1.y(), p2.y()) - w2, qMax(p1.x(), p2.x()) + w2,
              qMax(p1.y(), p2.y()) + w2);
}
...
void Tuplet::sanitizeTuplet()
{
    ...
    testDuration.reduce();
    if (elements().back()->tick() + elements().back()->actualTicks() - elements().front()->tick()
        > testDuration)
...
Fraction Tuplet::addMissingElement(const Fraction& startTick, const Fraction& endTick)
{
    ...
    if (!d.isValid()) {
        qDebug("Tuplet::addMissingElement(): invalid duration: %d/%d", f.numerator(),
               f.denominator());
