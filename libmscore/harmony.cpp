...
void Harmony::write(XmlWriter& xml) const
{
    ...
    if (_rootTpc != Tpc::TPC_INVALID || _baseTpc != Tpc::TPC_INVALID) {
        ...
        if (staff()) {
            ...
            Segment* segment = parent()->isSegment() ? toSegment(parent()) : toSegment(
                parent()->parent());
...
static int convertNote(const QString& s, NoteSpellingType noteSpelling, NoteCaseType& noteCase,
                       int& idx)
{
...
const ChordDescription* Harmony::fromXml(const QString& kind, const QString& kindText,
                                         const QString& symbols, const QString& parens,
...
void Harmony::calculateBoundingRect()
{
    ...
    const FretDiagram* fd
        = (parent() && parent()->isFretDiagram()) ? toFretDiagram(parent()) : nullptr;
...
QString Harmony::generateScreenReaderInfo() const
{
    ...
    default:
        if (_rootTpc != Tpc::TPC_INVALID)
            rez
                = QString("%1 %2").arg(rez).arg(tpc2name(_rootTpc, NoteSpellingType::STANDARD,
    ...
    if (_baseTpc != Tpc::TPC_INVALID)
        rez
            = QString("%1 / %2").arg(rez).arg(tpc2name(_baseTpc, NoteSpellingType::STANDARD,
                                                       NoteCaseType::AUTO, true));
