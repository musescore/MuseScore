...
Segment* LineSegment::findSegmentForGrip(Grip grip, QPointF pos) const
{
    ...
    Segment* const oldSeg = left ? l->startSegment() : score()->tick2leftSegmentMM(
        l->tick2() - Fraction::eps());
...
LineSegment* LineSegment::rebaseAnchor(Grip grip, Segment* newSeg)
{
    ...
    if (newSeg->system() == oldSystem) {
        const QPointF delta = left ? deltaRebaseLeft(oldSeg, newSeg) : deltaRebaseRight(oldSeg,
                                                                                        newSeg, track2staff(
                                                                                            l->
                                                                                            effectiveTrack2()));
        if (left) {
        ...
        if (grip == Grip::START) {
            ...
            if (cr && type() == ElementType::OTTAVA) {
                ...
                if (cr->durationType() == TDuration::DurationType::V_MEASURE
                    && !cr->measure()->hasVoices(cr->staffIdx()))
...
// x = -cr->spaceLw;  // account for accidentals, etc
            ...
            if (isOttava()) {
                    ...
                    for (int track = startTrack; track < endTrack; ++track) {
                        ...
                        if (cr1->isChord()) {
                            for (Note* n : toChord(cr1)->notes())
                                width = qMax(width,
                                             n->shape().right() + n->pos().x() + cr1->pos().x());
                        } else if (cr1->isRest()
                                   && (cr1->actualDurationType()
                ...
                if (!cr) {
                    qDebug("no end for lyricsline segment - start %d, ticks %d",
                ...
                if (cr && endElement()->parent()
                    && endElement()->parent()->type() == ElementType::SEGMENT) {
...
SpannerSegment* SLine::layoutSystem(System* system)
{
    ...
    LineSegment* lineSegm = toLineSegment(getNextLayoutSystemSegment(system, [this]() {
...
void SLine::layout()
{
    ...
    if (segmentsNeeded != segCount) {
        fixupSegments(segmentsNeeded, [this]() {
                return createLineSegment();
            });
        if (segmentsNeeded > segCount) {
...
void SLine::writeProperties(XmlWriter& xml) const
{
    ...
    for (const SpannerSegment* seg : spannerSegments()) {
        if (!seg->autoplace() || !seg->visible()
            ...
            || (!seg->isStyled(Pid::OFFSET)
                && (!seg->offset().isNull() || !seg->userOff2().isNull()))) {
            modified = true;
            break;
        }
    }
