...
void Score::cmdAddSpanner(Spanner* spanner, int staffIdx, Segment* startSegment,
                          Segment* endSegment)
{
...
Segment* Score::setNoteRest(Segment* segment, int track, NoteVal nval, Fraction sd,
                            Direction stemDirection, bool forceAccidental, bool rhythmic,
                            InputState* externalInputState)
{
    ...
    for (;;) {
        ...
        if (rhythmic)
            dl = toRhythmicDurationList(dd, isRest, segment->rtick(), sigmap()->timesig(
                                            tick).nominal(), measure, 1);
        else
        ...
        for (size_t i = 0; i < n; ++i) {
            ...
            if (isRest) {
                ...
                ncr->setDurationType(d);
                ncr->setTicks(
                    d == TDuration::DurationType::V_MEASURE ? measure->ticks() : d.fraction());
            } else {
                        ...
                        bool keepChord)
{
    ...
    for (Segment* seg = firstSegment; seg; seg = seg->next(SegmentType::ChordRest)) {
        ...
        if (sd < td) {
            ...
            if ((tuplet == 0)
                && (((measure->tick() - tick).ticks() % dList[0].ticks().ticks()) == 0)) {
...
bool Score::makeGap1(const Fraction& baseTick, int staffIdx, const Fraction& len,
...
static void upDownChromatic(bool up, int pitch, Note* n, Key key, int tpc1, int tpc2, int& newPitch,
                            int& newTpc1, int& newTpc2)
{
    if (up && pitch < 127) {
        newPitch = pitch + 1;
        if (n->concertPitch()) {
...
void Score::cmdResetBeamMode()
{
    ...
    for (Segment* seg = selection().firstChordRestSegment(); seg && seg->tick() < endTick;
        ...
        for (int track = selection().staffStart() * VOICES; track < selection().staffEnd() * VOICES;
             ++track) {
            ChordRest* cr = toChordRest(seg->element(track));
            if (cr == 0)
                continue;
        ...
        if (e->isNote()) {
            ...
            if (note->staff() && note->staff()->isTabStaff(note->chord()->tick())) {
                ...
                if (d == MScore::DirectionH::AUTO)
                    ...
                    d = d
                        == MScore::DirectionH::LEFT ? MScore::DirectionH::RIGHT : MScore::DirectionH
                        ::LEFT;
...
void Score::cmdIncDecDuration(int nSteps, bool stepDotted)
{
    ...
    TDuration initialDuration
        = (cr->durationType() == TDuration::DurationType::V_MEASURE) ? TDuration(
...
void Score::cmdSlashFill()
{
    ...
    Fraction endTick = endSegment ? endSegment->tick() : lastSegment()->tick()
                       + Fraction::fromTicks(1);
    Chord* firstSlash = 0;
    Chord* lastSlash = 0;
...
// ---------------------------------------------------------
Segment* Score::setChord(Segment* segment, int track, Chord* chordTemplate, Fraction dur,
...
void Score::cmdPitchUp()
{
    ...
    if (el && el->isLyrics())
        ...
        el->undoChangeProperty(Pid::OFFSET,
                               el->offset() + QPointF(0.0, -MScore::nudgeStep* el->spatium()),
...
void Score::cmdPitchDownOctave()
{
    ...
    if (el && (el->isArticulation() || el->isTextBase()))
        el->undoChangeProperty(Pid::OFFSET,
                               el->offset() + QPointF(0.0, MScore::nudgeStep10* el->spatium()),
                               PropertyFlags::UNSTYLED);
    else
...
void Score::cmd(const QAction* a, EditData& ed)
{
    ...
    static const std::vector<ScoreCmd> cmdList {
        { "note-c", [](Score* cs, EditData& ed){
              cs->cmdAddPitch(ed, 0, false, false);
          }},
        { "note-d", [](Score* cs, EditData& ed){
              cs->cmdAddPitch(ed, 1, false, false);
          }},
        { "note-e", [](Score* cs, EditData& ed){
              cs->cmdAddPitch(ed, 2, false, false);
          }},
        { "note-f", [](Score* cs, EditData& ed){
              cs->cmdAddPitch(ed, 3, false, false);
          }},
        { "note-g", [](Score* cs, EditData& ed){
              cs->cmdAddPitch(ed, 4, false, false);
          }},
        { "note-a", [](Score* cs, EditData& ed){
              cs->cmdAddPitch(ed, 5, false, false);
          }},
        { "note-b", [](Score* cs, EditData& ed){
              cs->cmdAddPitch(ed, 6, false, false);
          }},
        { "chord-c", [](Score* cs, EditData& ed){
              cs->cmdAddPitch(ed, 0, true, false);
          }},
        { "chord-d", [](Score* cs, EditData& ed){
              cs->cmdAddPitch(ed, 1, true, false);
          }},
        { "chord-e", [](Score* cs, EditData& ed){
              cs->cmdAddPitch(ed, 2, true, false);
          }},
        { "chord-f", [](Score* cs, EditData& ed){
              cs->cmdAddPitch(ed, 3, true, false);
          }},
        { "chord-g", [](Score* cs, EditData& ed){
              cs->cmdAddPitch(ed, 4, true, false);
          }},
        { "chord-a", [](Score* cs, EditData& ed){
              cs->cmdAddPitch(ed, 5, true, false);
          }},
        { "chord-b", [](Score* cs, EditData& ed){
              cs->cmdAddPitch(ed, 6, true, false);
          }},
        { "insert-c", [](Score* cs, EditData& ed){
              cs->cmdAddPitch(ed, 0, false, true);
          }},
        { "insert-d", [](Score* cs, EditData& ed){
              cs->cmdAddPitch(ed, 1, false, true);
          }},
        { "insert-e", [](Score* cs, EditData& ed){
              cs->cmdAddPitch(ed, 2, false, true);
          }},
        { "insert-f", [](Score* cs, EditData& ed){
              cs->cmdAddPitch(ed, 3, false, true);
          }},
        { "insert-g", [](Score* cs, EditData& ed){
              cs->cmdAddPitch(ed, 4, false, true);
          }},
        { "insert-a", [](Score* cs, EditData& ed){
              cs->cmdAddPitch(ed, 5, false, true);
          }},
        { "insert-b", [](Score* cs, EditData& ed){
              cs->cmdAddPitch(ed, 6, false, true);
          }},
        { "fret-0", [](Score* cs, EditData&){
              cs->cmdAddFret(0);
          }},
        { "fret-1", [](Score* cs, EditData&){
              cs->cmdAddFret(1);
          }},
        { "fret-2", [](Score* cs, EditData&){
              cs->cmdAddFret(2);
          }},
        { "fret-3", [](Score* cs, EditData&){
              cs->cmdAddFret(3);
          }},
        { "fret-4", [](Score* cs, EditData&){
              cs->cmdAddFret(4);
          }},
        { "fret-5", [](Score* cs, EditData&){
              cs->cmdAddFret(5);
          }},
        { "fret-6", [](Score* cs, EditData&){
              cs->cmdAddFret(6);
          }},
        { "fret-7", [](Score* cs, EditData&){
              cs->cmdAddFret(7);
          }},
        { "fret-8", [](Score* cs, EditData&){
              cs->cmdAddFret(8);
          }},
        { "fret-9", [](Score* cs, EditData&){
              cs->cmdAddFret(9);
          }},
        { "fret-10", [](Score* cs, EditData&){
              cs->cmdAddFret(10);
          }},
        { "fret-11", [](Score* cs, EditData&){
              cs->cmdAddFret(11);
          }},
        { "fret-12", [](Score* cs, EditData&){
              cs->cmdAddFret(12);
          }},
        { "fret-13", [](Score* cs, EditData&){
              cs->cmdAddFret(13);
          }},
        { "fret-14", [](Score* cs, EditData&){
              cs->cmdAddFret(14);
          }},
        { "toggle-visible", [](Score* cs, EditData&){
              cs->cmdToggleVisible();
          }},
        { "reset-stretch", [](Score* cs, EditData&){
              cs->resetUserStretch();
          }},
        { "mirror-note", [](Score* cs, EditData&){
              cs->cmdMirrorNoteHead();
          }},
        { "double-duration", [](Score* cs, EditData&){
              cs->cmdDoubleDuration();
          }},
        { "half-duration", [](Score* cs, EditData&){
              cs->cmdHalfDuration();
          }},
        { "inc-duration-dotted", [](Score* cs, EditData&){
              cs->cmdIncDurationDotted();
          }},
        { "dec-duration-dotted", [](Score* cs, EditData&){
              cs->cmdDecDurationDotted();
          }},
        { "add-staccato", [](Score* cs, EditData&){
              cs->addArticulation(SymId::articStaccatoAbove);
          }},
        { "add-tenuto", [](Score* cs, EditData&){
              cs->addArticulation(SymId::articTenutoAbove);
          }},
        { "add-marcato", [](Score* cs, EditData&){
              cs->addArticulation(SymId::articMarcatoAbove);
          }},
        { "add-sforzato", [](Score* cs, EditData&){
              cs->addArticulation(SymId::articAccentAbove);
          }},
        { "add-trill", [](Score* cs, EditData&){
              cs->addArticulation(SymId::ornamentTrill);
          }},
        { "add-up-bow", [](Score* cs, EditData&){
              cs->addArticulation(SymId::stringsUpBow);
          }},
        { "add-down-bow", [](Score* cs, EditData&){
              cs->addArticulation(SymId::stringsDownBow);
          }},
        { "add-8va", [](Score* cs, EditData&){
              cs->cmdAddOttava(OttavaType::OTTAVA_8VA);
          }},
        { "add-8vb", [](Score* cs, EditData&){
              cs->cmdAddOttava(OttavaType::OTTAVA_8VB);
          }},
        { "note-longa", [](Score* cs, EditData& ed){
              cs->padToggle(Pad::NOTE00, ed);
          }},
        { "note-longa-TAB", [](Score* cs, EditData& ed){
              cs->padToggle(Pad::NOTE00, ed);
          }},
        { "note-breve", [](Score* cs, EditData& ed){
              cs->padToggle(Pad::NOTE0, ed);
          }},
        { "note-breve-TAB", [](Score* cs, EditData& ed){
              cs->padToggle(Pad::NOTE0, ed);
          }},
        { "pad-note-1", [](Score* cs, EditData& ed){
              cs->padToggle(Pad::NOTE1, ed);
          }},
        { "pad-note-1-TAB", [](Score* cs, EditData& ed){
              cs->padToggle(Pad::NOTE1, ed);
          }},
        { "pad-note-2", [](Score* cs, EditData& ed){
              cs->padToggle(Pad::NOTE2, ed);
          }},
        { "pad-note-2-TAB", [](Score* cs, EditData& ed){
              cs->padToggle(Pad::NOTE2, ed);
          }},
        { "pad-note-4", [](Score* cs, EditData& ed){
              cs->padToggle(Pad::NOTE4, ed);
          }},
        { "pad-note-4-TAB", [](Score* cs, EditData& ed){
              cs->padToggle(Pad::NOTE4, ed);
          }},
        { "pad-note-8", [](Score* cs, EditData& ed){
              cs->padToggle(Pad::NOTE8, ed);
          }},
        { "pad-note-8-TAB", [](Score* cs, EditData& ed){
              cs->padToggle(Pad::NOTE8, ed);
          }},
        { "pad-note-16", [](Score* cs, EditData& ed){
              cs->padToggle(Pad::NOTE16, ed);
          }},
        { "pad-note-16-TAB", [](Score* cs, EditData& ed){
              cs->padToggle(Pad::NOTE16, ed);
          }},
        { "pad-note-32", [](Score* cs, EditData& ed){
              cs->padToggle(Pad::NOTE32, ed);
          }},
        { "pad-note-32-TAB", [](Score* cs, EditData& ed){
              cs->padToggle(Pad::NOTE32, ed);
          }},
        { "pad-note-64", [](Score* cs, EditData& ed){
              cs->padToggle(Pad::NOTE64, ed);
          }},
        { "pad-note-64-TAB", [](Score* cs, EditData& ed){
              cs->padToggle(Pad::NOTE64, ed);
          }},
        { "pad-note-128", [](Score* cs, EditData& ed){
              cs->padToggle(Pad::NOTE128, ed);
          }},
        { "pad-note-128-TAB", [](Score* cs, EditData& ed){
              cs->padToggle(Pad::NOTE128, ed);
          }},
        { "pad-note-256", [](Score* cs, EditData& ed){
              cs->padToggle(Pad::NOTE256, ed);
          }},
        { "pad-note-256-TAB", [](Score* cs, EditData& ed){
              cs->padToggle(Pad::NOTE256, ed);
          }},
        { "pad-note-512", [](Score* cs, EditData& ed){
              cs->padToggle(Pad::NOTE512, ed);
          }},
        { "pad-note-512-TAB", [](Score* cs, EditData& ed){
              cs->padToggle(Pad::NOTE512, ed);
          }},
        { "pad-note-1024", [](Score* cs, EditData& ed){
              cs->padToggle(Pad::NOTE1024, ed);
          }},
        { "pad-note-1024-TAB", [](Score* cs, EditData& ed){
              cs->padToggle(Pad::NOTE1024, ed);
          }},
        { "reset-style", [](Score* cs, EditData&){
              cs->cmdResetStyle();
          }},
        { "reset-beammode", [](Score* cs, EditData&){
              cs->cmdResetBeamMode();
          }},
        { "reset-groupings", [](Score* cs, EditData&){
              cs->cmdResetNoteAndRestGroupings();
          }},
        { "clef-violin", [](Score* cs, EditData&){
              cs->cmdInsertClef(ClefType::G);
          }},
        { "clef-bass", [](Score* cs, EditData&){
              cs->cmdInsertClef(ClefType::F);
          }},
        { "voice-x12", [](Score* cs, EditData&){
              cs->cmdExchangeVoice(0, 1);
          }},
        { "voice-x13", [](Score* cs, EditData&){
              cs->cmdExchangeVoice(0, 2);
          }},
        { "voice-x14", [](Score* cs, EditData&){
              cs->cmdExchangeVoice(0, 3);
          }},
        { "voice-x23", [](Score* cs, EditData&){
              cs->cmdExchangeVoice(1, 2);
          }},
        { "voice-x24", [](Score* cs, EditData&){
              cs->cmdExchangeVoice(1, 3);
          }},
        { "voice-x34", [](Score* cs, EditData&){
              cs->cmdExchangeVoice(2, 3);
          }},
        { "pad-rest", [](Score* cs, EditData& ed){
              cs->padToggle(Pad::REST, ed);
          }},
        { "pad-dot", [](Score* cs, EditData& ed){
              cs->padToggle(Pad::DOT, ed);
          }},
        { "pad-dotdot", [](Score* cs, EditData& ed){
              cs->padToggle(Pad::DOTDOT, ed);
          }},
        { "pad-dot3", [](Score* cs, EditData& ed){
              cs->padToggle(Pad::DOT3, ed);
          }},
        { "pad-dot4", [](Score* cs, EditData& ed){
              cs->padToggle(Pad::DOT4, ed);
          }},
        { "beam-start", [](Score* cs, EditData&){
              cs->cmdSetBeamMode(Beam::Mode::BEGIN);
          }},
        { "beam-mid", [](Score* cs, EditData&){
              cs->cmdSetBeamMode(Beam::Mode::MID);
          }},
        { "no-beam", [](Score* cs, EditData&){
              cs->cmdSetBeamMode(Beam::Mode::NONE);
          }},
        { "beam32", [](Score* cs, EditData&){
              cs->cmdSetBeamMode(Beam::Mode::BEGIN32);
          }},
        { "beam64", [](Score* cs, EditData&){
              cs->cmdSetBeamMode(Beam::Mode::BEGIN64);
          }},
        { "auto-beam", [](Score* cs, EditData&){
              cs->cmdSetBeamMode(Beam::Mode::AUTO);
          }},
        ...
        { "flip", [](Score* cs, EditData&){
              cs->cmdFlip();
          }},
        { "stretch+", [](Score* cs, EditData&){
              cs->cmdAddStretch(0.1);
          }},
        { "stretch-", [](Score* cs, EditData&){
              cs->cmdAddStretch(-0.1);
          }},
        { "pitch-spell", [](Score* cs, EditData&){
              cs->spell();
          }},
        { "select-all", [](Score* cs, EditData&){
              cs->cmdSelectAll();
          }},
        { "select-section", [](Score* cs, EditData&){
              cs->cmdSelectSection();
          }},
        { "add-brackets", [](Score* cs, EditData&){
              cs->cmdAddBracket();
          }},
        { "add-parentheses", [](Score* cs, EditData&){
              cs->cmdAddParentheses();
          }},
        ...
        { "explode", [](Score* cs, EditData&){
              cs->cmdExplode();
          }},
        { "implode", [](Score* cs, EditData&){
              cs->cmdImplode();
          }},
        { "slash-fill", [](Score* cs, EditData&){
              cs->cmdSlashFill();
          }},
        { "slash-rhythm", [](Score* cs, EditData&){
              cs->cmdSlashRhythm();
          }},
        { "resequence-rehearsal-marks", [](Score* cs, EditData&){
              cs->cmdResequenceRehearsalMarks();
          }},
        { "del-empty-measures", [](Score* cs, EditData&){
              cs->cmdRemoveEmptyTrailingMeasures();
          }},
        { "add-audio", [](Score* cs, EditData&){
              cs->addAudioTrack();
          }},
        { "transpose-up", [](Score* cs, EditData&){
              cs->transposeSemitone(1);
          }},
        { "transpose-down", [](Score* cs, EditData&){
              cs->transposeSemitone(-1);
          }},
        { "delete", [](Score* cs, EditData&){
              cs->cmdDeleteSelection();
          }},
        { "full-measure-rest", [](Score* cs, EditData&){
              cs->cmdFullMeasureRest();
          }},
        { "toggle-insert-mode", [](Score* cs, EditData&){
              cs->_is.setInsertMode(!cs->_is.insertMode());
          }},
        { "pitch-up", [](Score* cs, EditData&){
              cs->cmdPitchUp();
          }},
        { "pitch-down", [](Score* cs, EditData&){
              cs->cmdPitchDown();
          }},
        { "time-delete", [](Score* cs, EditData&){
              cs->cmdTimeDelete();
          }},
        { "pitch-up-octave", [](Score* cs, EditData&){
              cs->cmdPitchUpOctave();
          }},
        { "pitch-down-octave", [](Score* cs, EditData&){
              cs->cmdPitchDownOctave();
          }},
        { "pad-note-increase", [](Score* cs, EditData& ed){
              cs->cmdPadNoteIncreaseTAB(ed);
          }},
        { "pad-note-decrease", [](Score* cs, EditData& ed){
              cs->cmdPadNoteDecreaseTAB(ed);
          }},
        { "pad-note-increase-TAB", [](Score* cs, EditData& ed){
              cs->cmdPadNoteIncreaseTAB(ed);
          }},
        { "pad-note-decrease-TAB", [](Score* cs, EditData& ed){
              cs->cmdPadNoteDecreaseTAB(ed);
          }},
        { "toggle-mmrest", [](Score* cs, EditData&){
              cs->cmdToggleMmrest();
          }},
        { "toggle-hide-empty", [](Score* cs, EditData&){
              cs->cmdToggleHideEmpty();
          }},
        { "set-visible", [](Score* cs, EditData&){
              cs->cmdSetVisible();
          }},
        { "unset-visible", [](Score* cs, EditData&){
              cs->cmdUnsetVisible();
          }},
        ...
        { "relayout", [](Score* cs, EditData&){
              cs->cmdRelayout();
          }},
        { "toggle-autoplace", [](Score* cs, EditData&){
              cs->cmdToggleAutoplace(false);
          }},
        { "autoplace-enabled", [](Score* cs, EditData&){
              cs->cmdToggleAutoplace(true);
          }},
    };
