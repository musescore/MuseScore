...
void StringData::fretChords(Chord* chord) const
{
    ...
    foreach (Note* note, sortedNotes) {
        ...
        if (nString
            == STRING_NONE /*|| nFret == FRET_NONE || getPitch(nString, nFret) != note->pitch()*/) {
            // get a new fretting
...
void StringData::sortChordNotes(QMap<int, Note*>& sortedNotes, const Chord* chord, int pitchOffset,
                                int* count) const
{
    int key, string, fret;
        ...
        if (tag == "staff-lines") {
            ...
            if (0 < line && line <= stringTable.size()) {
                ...
                if (pitch >= 0)
                    ...
                    qDebug(
                        "StringData::readMusicXML invalid string %d tuning step/alter/oct %s/%d/%d",
