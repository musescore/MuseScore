...
bool StringData::convertPitch(int pitch, int pitchOffset, int* string, int* fret) const
      ...
      // if 5 string banjo
      if (strings == 5 && stringTable.at(0).startFret > 0) {
...
//---------------------------------------------------------
//   configBanjo5thString
//   Called when new StringData is set for an Instrument object.
//   This method looks at the banjo tuning and sets startFret 
//   appropriately. See adjustBanjo5thFret below.
//---------------------------------------------------------
...
void StringData::configBanjo5thString()
      ...
      // assure this is a banjo - 5th string pitch is higher 
      // than 4th (unlike guitar)
      if (strg5.pitch > stringTable[1].pitch) {
...
int StringData::adjustBanjo5thFret(int fret) const
    {
    return (fret > 0 && stringTable.size() == 5) ? fret + stringTable[0].startFret : fret;
    }
