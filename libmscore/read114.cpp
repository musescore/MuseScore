...
QString convertFromHtml(TextBase* t, const QString& ss)
{
    ...
    s.replace(QChar(0xe10b),
              QString("<sym>metAugmentationDot</sym><sym>space</sym><sym>metAugmentationDot</sym>"));                     // dotdot
    ...
    while (e.readNextStartElement()) {
        ...
        if (tag == "move") {
            ...
            if (a->markerType() == Marker::Type::SEGNO || a->markerType() == Marker::Type::CODA
                || a->markerType() == Marker::Type::VARCODA
                || a->markerType() == Marker::Type::CODETTA) {
        ...
        if (tag == "lines") {
            ...
            while (e.readNextStartElement()) {
                if (e.name() == "clef") {
                    ...
                    staff->clefList().insert(std::pair<int, ClefType>(_score->fileDivision(
                                                                          tick), ct));
                    e.readNext();
                } else {
                    e.unknown();
...
// style->setTextStyle(s);
        ...
        for (idx2 = 0; idx2 < int(ArticulationType::ARTICULATIONS); ++idx2) {
            ...
            if (QString::compare(tag, QString(ai.name).append("Anchor"), Qt::CaseInsensitive) == 0
                || QString::compare(tag, QString("U").append(ai.name).append("Anchor"),
                                    Qt::CaseInsensitive) == 0
                || QString::compare(tag, QString("D").append(ai.name).append("Anchor"),
                                    Qt::CaseInsensitive) == 0
                ) {
                ...
                if (cr) {
                    ...
                    if (cr->isRest()) {
                        ...
                        if (!r->offset().isNull()) {
                            ...
                            qreal lineDist
                                = r->staff() ? r->staff()->staffType(cr->tick())->lineDistance().val()
                                  : 1.0;
