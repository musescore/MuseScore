...
static void create_segments()
{
    ...
    for (int b = 0; b < num_sbeats; b++) {
        if (b == 0
            && (sbeat[0].time - firstbeat)
            < ((sbeat[1].time-firstbeat) - (sbeat[0].time-firstbeat))/2) {
            continue;
        }
...
static void match_profiles()
{
    ...
    for (s = 0; s <= segtotal; ++s) {
        if (scoring_mode == 0) {
            ...
            if (verbosity == 3)
                qDebug("For segment %d: average_dur = %6.3f; input_sumsq = %6.3f", s,
                       segment[s].average_dur, input_sumsq);
        }
        best_key = 0;
                ...
                */
                ...
                if (scoring_mode == 0) {
                    if (tpc < 9 || tpc > 20)
                        continue;
                    /* calculate numerator */
                    keyscore()[key][s] += key_profile[key][tpc]
                                          * (seg_prof[tpc][s]-segment[s].average_dur);
...
static void make_first_table(int seg)
{
    ...
    for (i = 0; i < 56; ++i) {
        for (j = 0; j < 56; ++j) {
            if (j != i)
                analysis[i][j][1] = ((keyscore()[i][0] + keyscore()[j][1]) * seg_factor)
                                    + mod_factor;
            else
                analysis[i][j][1] = ((keyscore()[i][0] + keyscore()[j][1]) * seg_factor)
                                    + nomod_factor;
    ...
    for (int seg = 2; seg <= segtotal; ++seg) {
        ...
        for (int j = 0; j < 56; ++j) {
            for (int i = 0; i < 56; ++i) {
                int n = best[i][seg-1];
                if (j != i)
                    analysis[i][j][seg] = analysis[n][i][seg-1]
                                          + (keyscore()[j][seg] * seg_factor) + mod_factor;
                else
                    analysis[i][j][seg] = analysis[n][i][seg-1]
                                          + (keyscore()[j][seg] * seg_factor) + nomod_factor;
            }
        }
        choose_best_i(seg);
    }
...
static void best_key_analysis()
{
    ...
    for (int j = 0; j < 56; ++j) {
        ...
        if (analysis[n][j][s] < analysis[m][k][s] + .001
            && analysis[n][j][s] > analysis[m][k][s] - .001 && j != k) {
            tie1 = j;
            tie2 = k;
        }

        if (verbosity > 1
            && !(npc_or_tpc_profile == 0 && (j < 9 || (j > 20 && j < 37) || j > 48))) {
    ...
    if (verbosity >= 2) {
        qDebug("Segment 0: key choice %d; total score %6.3f; segment score %6.3f", final[0],
               keyscore()[final[0]][0],
               keyscore()[final[0]][0]);
        qDebug("Segment 1: key choice %d; total score %6.3f; segment score %6.3f", final[1],
               analysis[final[0]][final[1]][1],
