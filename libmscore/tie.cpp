...
// shapePath.translate(staffOffset);
    ...
    int nbShapes = 15;
    const CubicBezier b(pp1, ups(Grip::BEZIER1).pos(), ups(Grip::BEZIER2).pos(), ups(
                            Grip::END).pos());
...
void Tie::slurPos(SlurPos* sp)
{
    ...
    if (!sp->system1) {
        ...
        qDebug("No system: measure is %d has %d count %d", m->isMMRest(),
               m->hasMMRest(), m->mmRestCount());
    }
    ...
    bool horizontal = startNote()->line() == endNote()->line()
                      && sc->vStaffIdx() == ec->vStaffIdx();
    ...
    if (sc->vStaffIdx() != vStaffIdx() && sp->system1) {
        qreal diff = sp->system1->staff(sc->vStaffIdx())->y()
                     - sp->system1->staff(vStaffIdx())->y();
        sp->p1.ry() += diff;
    }
    if (ec->vStaffIdx() != vStaffIdx() && sp->system2) {
        qreal diff = sp->system2->staff(ec->vStaffIdx())->y()
                     - sp->system2->staff(vStaffIdx())->y();
        sp->p2.ry() += diff;
    }
}
...
void Tie::calculateDirection()
{
    ...
    if (_slurDirection == Direction::AUTO) {
        ...
        if (m1->hasVoices(c1->staffIdx(), c1->tick(),
                          c1->actualTicks())
            || m2->hasVoices(c2->staffIdx(), c2->tick(), c2->actualTicks())) {
