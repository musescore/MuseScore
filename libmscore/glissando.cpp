...
void GlissandoSegment::draw(QPainter* painter) const
{
    ...
    if (glissando()->glissandoType() == GlissandoType::STRAIGHT) {
        ...
        score()->scoreFont()->draw(ids, painter, magS(), QPointF(x,
                                                                 -(b.y() + b.height()*0.5)), scale);
...
void Glissando::layout()
{
    ...
    int upDown
        = (0 < (anchor2->pitch() - anchor1->pitch())) - ((anchor2->pitch() - anchor1->pitch()) < 0);
    ...
    if (cr2->segment()->measure()->isFirstInSystem() && cr2->rtick().isZero()
        ...
        && !(cr2->noteType() == NoteType::GRACE8_AFTER
             || cr2->noteType() == NoteType::GRACE16_AFTER
             || cr2->noteType() == NoteType::GRACE32_AFTER)
    ...
    offs1.rx()
        += (dots
            && anchor1->dot(dots-1) ? anchor1->dot(dots-1)->pos().x() + anchor1->dot(
                dots-1)->width()
    ...
    QRectF r
        = QRectF(anchor2SystPos - segm2->pos(),
                 anchor2SystPos - segm2->pos() - segm2->pos2()).normalized();
