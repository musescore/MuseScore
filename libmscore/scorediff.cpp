...
class MscxModeDiff
{
    static constexpr const char* tagRegExpStr
        = "</?(?<name>[A-z_][A-z_0-9\\-.:]*)( [A-z_0-9\\-.:\\s=\"]*)?/?>";
    ...
    struct Tag {
        ...
        Tag(int l, TagType t, const QString& n) : line(l),
            type(t),
            name(n)
        {
        }
    ...
    bool assessShiftDiff(const std::vector<TextDiff>&, const std::vector<Tag>& extraTags, int index,
                         int lines);
...
class TextDiffParser
{
    ...
    {
        return currentDiffTagLevel != 0;
    }

public:
    TextDiffParser(int iScore);

    void makeDiffs(const QString& mscx, const std::vector<std::pair<const ScoreElement*,
                                                                    QString> >& elements,
...
bool MscxModeDiff::assessShiftDiff(const std::vector<TextDiff>& diffs,
                                   const std::vector<Tag>& origExtraTags, int index, int lines)
{
    if (lines == 0)
        return diffs.empty();
...
void MscxModeDiff::handleTag(int line, TagType type, const QString& name,
                             std::vector<Tag>& extraTags)
{
    switch (type) {
    case START_TAG: {
...
void TextDiffParser::makeDiffs(const QString& mscx, const std::vector<std::pair<const ScoreElement*,
                                                                                QString> >& elements, const std::vector<TextDiff>& textDiffs, std::vector<BaseDiff*>& diffs)
{
...
BaseDiff* TextDiffParser::handleToken(const QXmlStreamReader& r, const ScoreElement* newElement,
                                      bool saveDiff)
{
...
static void makeDiffs(const QString& mscx1, const QString& mscx2, const XmlWriter& xml1,
                      const XmlWriter& xml2, const std::vector<TextDiff>& textDiffs,
...
struct MeasureInfo {
    ...
    MeasureInfo(const Measure* m1, const Measure* m2, int staff) : m1(m1),
        m2(m2),
        staff(staff)
    {
    }

    bool operator==(const MeasureInfo& other)
    {
        return m1 == other.m1 && m2 == other.m2 && staff == other.staff;
    }
...
{
    return QString("Context change: ctx1 (%1), ctx2(%2)").arg(describeContext(ctx[0])).arg(describeContext(
                                                                                               ctx[1
                                                                                               ]));
}
...
QString ElementDiff::toString() const
{
    ...
    switch (type) {
    case DiffType::DELETE: {
        return QObject::tr("%1: removed element %2", "scorediff").arg(ctxDescr).arg(
            el[0]->userName());
    }
    case DiffType::INSERT: {
        return QObject::tr("%1: inserted element %2", "scorediff").arg(ctxDescr).arg(
            el[1]->userName());
    }
    case DiffType::REPLACE: {
    ...
    case DiffType::EQUAL:
        ...
        return QObject::tr("%1: equal element %2",
                           "scorediff").arg(ctxDescr).arg(el[0]->userName());
    }
    return ctxDescr;
}
