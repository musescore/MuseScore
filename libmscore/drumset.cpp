...
void Drumset::save(XmlWriter& xml) const
{
    for (int i = 0; i < 128; ++i) {
        ...
        const NoteHead::Group saveNHValue
            = (nh == NoteHead::Group::HEAD_CUSTOM) ? NoteHead::Group::HEAD_NORMAL : nh;
        xml.tag("head", NoteHead::group2name(saveNHValue));
        if (nh == NoteHead::Group::HEAD_CUSTOM) {
            xml.stag("noteheads");
            for (int j = 0; j < int(NoteHead::Type::HEAD_TYPES); j++)
                xml.tag(NoteHead::type2name(NoteHead::Type(j)),
                        Sym::id2name(noteHeads(i, NoteHead::Type(j))));
            xml.etag();
        }
        xml.tag("line", line(i));
...
DrumInstrumentVariant Drumset::findVariant(int p, const QVector<Articulation*> articulations,
