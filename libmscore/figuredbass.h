...
public:
    ...
    FiguredBassItem* clone() const override
    {
        return new FiguredBassItem(*this);
    }
    ...
    // read / write MusicXML
    void              writeMusicXML(XmlWriter& xml, bool isOriginalFigure, int crEndTick,
                                    int fbEndTick) const;
...
struct FiguredBassFont {
    ...
    QChar displayDigit[int(FiguredBassItem::Style::NUMOF)][10][int(FiguredBassItem::Combination::
                                                                   NUMOF)];
...
public:
    ...
    static FiguredBass* addFiguredBassToSegment(Segment* seg, int track, const Fraction& extTicks,
                                                bool* pNew);

    // static functions for font config files
    static bool       readConfigFile(const QString& fileName);
    static QList<QString> fontNames();
    static bool       fontData(int nIdx, QString* pFamily, QString* pDisplayName, qreal* pSize,
                               qreal* pLineHeight);

    // standard re-implemented virtual functions
    FiguredBass* clone() const override
    {
        return new FiguredBass(*this);
    }

    ElementType   type() const override
    {
        return ElementType::FIGURED_BASS;
    }
    ...
    // read / write MusicXML
    void              writeMusicXML(XmlWriter& xml, bool isOriginalFigure, int crEndTick,
                                    int fbEndTick, bool writeDuration, int divisions) const;
