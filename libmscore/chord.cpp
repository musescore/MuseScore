...
qreal Chord::defaultStemLength() const
{
    ...
    normalStemLen
        += hookAdjustment(score()->styleSt(Sid::MusicalSymbolFont), hookIdx, up(), small());
    if (hookIdx && tab == 0) {
...
void Chord::layoutStem1()
{
    ...
    if (durationType().hasStem()
        && !(_noStem || (measure() && measure()->stemless(staffIdx()))
             || (st && st->isTabStaff() && st->stemless()))) {
        if (!_stem) {
...
void Chord::layout2()
{
    ...
    if (!gna.empty()) {
        qreal minNoteDist = score()->styleP(Sid::minNoteDistance) * mag * score()->styleD(
            Sid::graceNoteMag);
        ...
        Chord* last = gna.last();
        qreal xOff = s ? (s->pos().x() - s->staffShape(
                              last->vStaffIdx()).left())
                ...
                if (sc->notes().size() > 1 || (sc->stem() && sc->up() == tie->up())) {
                    ...
                    if (sc->width() > sn->width()) {
                        ...
                        for (unsigned i = 0; i < sc->notes().size(); ++i)
                            scEnd = qMax(scEnd, sc->notes().at(i)->x() + sc->notes().at(
                                             i)->bboxRightPos());
                        overlap += scEnd - snEnd;
                    } else {
...
void Chord::layoutTablature()
{
    ...
    for (size_t i = 0; i < numOfNotes; ++i) {
        ...
        qreal y = note->fixed() ? note->line() * lineDist / 2 : tab->physStringToYOffset(
            note->string()) * _spatium;
        note->setPos(x, y);
        if (y < minY)
            minY = y;
        ...
        if (tie) {
            ...
            if (startChord && startChord->measure() == measure()
                && startChord == prevChordRest(this)) {
...
// qreal extraLen    = score()->styleS(Sid::ledgerLineLength).val() * _spatium;
    ...
    if (tab->stemless() || _noStem || measure()->stemless(staffIdx()) || durationType().type()
        < (tab->minimStyle()
           != TablatureMinimStyle::NONE ? TDuration::DurationType::V_HALF : TDuration::DurationType
           ::
           V_QUARTER)) {
        if (_stem)
...
// _tabDur->setMag(mag());           // useless to set grace mag: graces have no dur. symbol
    ...
    for (size_t i = 0; i < numOfNotes; ++i)
        _notes.at(i)->layout2();
    QRectF bb;
    processSiblings([&bb](Element* e) {
            bb |= e->bbox().translated(e->pos());
        });
    if (_tabDur)
...
void Chord::layoutArpeggio2()
{
    ...
    Note* dnote
        = (bchord
           && bchord->type() == ElementType::CHORD) ? toChord(bchord)->downNote() : downNote();
            ...
            if (staff()->isDrumStaff(tick())) {
                ...
                if (ds && ds->isValid(pitch)) {
                    undoChangeProperty(Pid::STEM_DIRECTION,
                                       QVariant::fromValue<Direction>(ds->stemDirection(pitch)));
...
TremoloChordType Chord::tremoloChordType() const
{
    if (_tremolo && _tremolo->twoNotes()) {
        if (_tremolo->chord1() == this)
            ...
            qFatal("Chord::tremoloChordType(): inconsistency %p - %p, this is %p",
                   _tremolo->chord1(), _tremolo->chord2(), this);
    }
...
void Chord::layoutArticulations()
{
    ...
    qreal mag = (staffType->small() ? score()->styleD(Sid::smallStaffMag) : 1.0)
                * staffType->userMag();
    ...
    for (Articulation* a : _articulations) {
        if (a->anchor() == ArticulationAnchor::CHORD) {
            ...
            a->setUp(
                ...
                || a->anchor() == ArticulationAnchor::TOP_CHORD);
        }
