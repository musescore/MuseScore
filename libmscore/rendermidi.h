...
public:
    ...
    {
        setOccupied(range.first, range.second);
    }
...
public:
    ...
    public:
        ...
        {
            return last ? last->nextMeasure() : nullptr;
        }
        ...
        int tick1() const
        {
            return first->tick().ticks();
        }

        int tick2() const
        {
            return last ? last->endTick().ticks() : tick1();
        }
...
private:
    ...
    void collectMeasureEvents(EventMap* events, Measure* m, const MidiRenderer::StaffContext& sctx,
                              int tickOffset);
    void collectMeasureEventsSimple(EventMap* events, Measure* m, const StaffContext& sctx,
                                    int tickOffset);
    void collectMeasureEventsDefault(EventMap* events, Measure* m, const StaffContext& sctx,
                                     int tickOffset);
