...
// s->setStaffType(0, *staff->staffType(0));              // TODO
            ...
            for (auto le : *ls) {
                ...
                if (ps->primaryStaff()) {
                    for (int i = 0; i < VOICES; i++)
                        tracks.insert(ps->idx() * VOICES + i % VOICES,
                                      s->idx() * VOICES + i % VOICES);
                    break;
                }
            }
        }
    ...
    if (oscore->styleB(Sid::concertPitch) != score->styleB(Sid::concertPitch)) {
        for (const Staff* staff : score->staves()) {
            ...
            score->transposeKeys(staffIdx, staffIdx+1, Fraction(0, 1), endTick, interval, true,
                                 flip);
...
static void cloneTuplets(ChordRest* ocr, ChordRest* ncr, Tuplet* ot, TupletMap& tupletMap,
                         Measure* m, int track)
{
...
void Excerpt::cloneStaves(Score* oscore, Score* score, const QList<int>& map, QMultiMap<int,
                                                                                        int>& trackList)
{
    TieMap tieMap;
                    ...
                    for (int track : t) {
                        ...
                        if (oe && !oe->generated()) {
                            ...
                            if (oe->type() == ElementType::BAR_LINE && adjustedBarlineSpan) {
                                ...
                                if (ocr->beam() && !ocr->beam()->empty()
                                    && ocr->beam()->elements().front() == ocr) {
                                    ...
                                    for (size_t i = 0; i < n; ++i) {
                                        ...
                                        for (Spanner* oldSp : on->spannerBack()) {
                                            if (oldSp->startElement() && oldSp->endElement()
                                                && oldSp->startElement()->track()
                                                > oldSp->endElement()->track())
                                                continue;
                                            Note* newStart = Spanner::startElementFromSpanner(oldSp,
                                                                                              nn);
                                            if (newStart != nullptr) {
                                        ...
                                        for (Spanner* oldSp : on->spannerFor()) {
                                            if (oldSp->startElement() && oldSp->endElement()
                                                ...
                                                continue;
                                            Note* newEnd
                                                = Spanner::endElementFromSpanner(oldSp, nn);
                                            if (newEnd != nullptr) {
                                                Spanner* newSp = toSpanner(oldSp->linkedClone());
                        ...
                        if (srcTrack % VOICES && !(track % VOICES)
                            && (!tst || (!tst->element(track)))) {
        ...
        for (int srcTrack = sTrack; srcTrack < eTrack; ++srcTrack) {
            ...
            for (Segment* seg = m->first(); seg; seg = seg->next()) {
                ...
                if (oe->isClef()) {
                    ...
                    if (ClefInfo::staffGroup(clef->concertClef())
                        == dstStaff->constStaffType(Fraction(0, 1))->group()
...
void Excerpt::cloneStaff2(Staff* srcStaff, Staff* dstStaff, const Fraction& stick,
