...
public:
    ...
    {
        return numerator() / dUnitsPerBeat();
    }
    ...
    }                                                                                    // note: always test isTriple() first
    ...
    {
        return rtick % dUnitTicks();
    }                                                                                            // returns 0 if rtick is exactly on a dUnit

    int ticksToNextDUnit(int rtick)     const
    {
        return dUnitTicks() - ticksPastDUnit(rtick);
    }                                                                                            // returns dUnitTicks() if rtick is on a dUnit

    int ticksPastBeat(int rtick)        const
    {
        return rtick % beatTicks();
    }                                                                                            // returns 0 if rtick is exactly on a beat

    int ticksToNextBeat(int rtick)      const
    {
        return beatTicks() - ticksPastBeat(rtick);
    }                                                                                            // returns beatTicks() if rtick is on a beat
...
public:
    ...
    bool valid() const
    {
        return _timesig.isValid();
    }

    QString print() const
    {
        return _timesig.print();
    }
