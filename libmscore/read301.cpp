...
bool Score::read(XmlReader& e)
{
    ...
    if (e.error() != QXmlStreamReader::NoError) {
        ...
        if (e.error() == QXmlStreamReader::CustomError)
            ...
            MScore::lastError = QObject::tr("XML read error at line %1, column %2: %3").arg(
                e.lineNumber()).arg(e.columnNumber()).arg(e.name().toString());
        return false;
    }
    ...
    for (Staff* st : staves()) {
        ...
        // check spanFrom
        int minBarLineFrom = st->lines(0)
                             == 1 ? BARLINE_SPAN_1LINESTAFF_FROM : MIN_BARLINE_SPAN_FROMTO;
