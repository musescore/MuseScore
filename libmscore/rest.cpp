...
bool Rest::acceptDrop(EditData& data) const
{
    ...
    if (
        ...
        || (type == ElementType::REPEAT_MEASURE
            && durationType().type() == TDuration::DurationType::V_MEASURE)
        )
        return true;
    return false;
}
...
QString Rest::accessibleInfo() const
{
    ...
    return QObject::tr("%1; Duration: %2; %3").arg(Element::accessibleInfo()).arg(durationUserName())
           .arg(voice);
}
...
QString Rest::screenReaderInfo() const
{
    ...
    QString voice
        = voices ? QObject::tr("Voice: %1").arg(QString::number(track() % VOICES + 1)) : "";
...
void Rest::write(XmlWriter& xml) const
{
    ...
    for (NoteDot* dot : _dots)
        if (!dot->offset().isNull() || !dot->visible() || dot->color() != Qt::black
            || dot->visible() != visible()) {
            write_dots = true;
            break;
        }
