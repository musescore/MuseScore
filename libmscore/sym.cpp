...
const std::array<const char*, int(SymId::lastSym)+1> Sym::symUserNames = { {
    ...
    QT_TRANSLATE_NOOP("symUserNames",
...
bool GlyphKey::operator==(const GlyphKey& k) const
{
    return (face == k.face) && (id == k.id)
           && (magX == k.magX) && (magY == k.magY) && (worldScale == k.worldScale)
           && (color == k.color);
}
...
void ScoreFont::draw(SymId id, QPainter* painter, qreal mag, const QPointF& pos,
                     qreal worldScale) const
{
    draw(id, painter, QSizeF(mag, mag), pos, worldScale);
}

void ScoreFont::draw(SymId id, QPainter* painter, const QSizeF& mag, const QPointF& pos,
                     qreal worldScale) const
{
...
void ScoreFont::draw(const std::vector<SymId>& ids, QPainter* p, qreal mag, const QPointF& _pos,
                     qreal scale) const
{
    QPointF pos(_pos);
    for (SymId id : ids) {
...
void ScoreFont::draw(const std::vector<SymId>& ids, QPainter* p, const QSizeF& mag,
...
void ScoreFont::draw(const std::vector<SymId>& ids, QPainter* p, const QSizeF& mag,
                     const QPointF& _pos, qreal scale) const
{
    QPointF pos(_pos);
    for (SymId id : ids) {
        draw(id, p, mag, pos, scale);
...
void ScoreFont::draw(const std::vector<SymId>& ids, QPainter* p, qreal mag,
                     const QPointF& _pos) const
{
    qreal scale = p->worldTransform().m11();
    draw(ids, p, mag, _pos, scale);
...
void initScoreFonts()
{
    ...
    for (size_t i = 0; i < Sym::symNames.size(); ++i) {
        ...
        bool ok;
        uint code
            = glyphNamesJson.value(name).toObject().value("codepoint").toString().mid(2).toUInt(&ok,
                                                                                                16);
        if (ok)
...
void ScoreFont::load()
{
    ...
    for (auto i : oo.keys()) {
        for (auto mapping : engravingDefaultsMapping) {
            if (i == mapping.first)
                _engravingDefaults.push_back(std::make_pair(mapping.second,
                                                            oo.value(i).toDouble()));
            else if (i == "textEnclosureThickness")
                _textEnclosureThickness = oo.value(i).toDouble();
        }
    }
    _engravingDefaults.push_back(std::make_pair(Sid::MusicalTextFont,
                                                QString("%1 Text").arg(_family)));
...
const QRectF ScoreFont::bbox(SymId id, const QSizeF& mag) const
{
    ...
    return QRectF(r.x() * mag.width(), r.y() * mag.height(), r.width() * mag.width(),
                  r.height() * mag.height());
}
