                         ...
                         bool forceTimeSig)
{
    ...
    if (m->score()->styleB(Sid::createMultiMeasureRests) && m->isMeasure()
        && toMeasure(m)->mmRest())
...
void Score::writeMovement(XmlWriter& xml, bool selectionOnly)
{
    ...
    if (excerpt()) {
        ...
        if (!(trackList.size() == e->parts().size() * VOICES) && !trackList.isEmpty()) {
            while (i.hasNext()) {
                i.next();
                xml.tagE(QString("Tracklist sTrack=\"%1\" dstTrack=\"%2\"").arg(i.key()).arg(
                             i.value()));
            }
        }
    }
    ...
    while (i.hasNext()) {
        ...
        if ((!MScore::testMode && !MScore::saveTemplateMode)
            || (i.key() != "platform" && i.key() != "creationDate"))
    ...
    for (const Part* part : _parts) {
        if (!selectionOnly
            || ((staffIdx(part) >= staffStart) && (staffEnd >= staffIdx(part) + part->nstaves())))
            part->write(xml);
    }
...
bool MasterScore::saveFile(bool generateBackup)
{
    ...
    if (!saved() && generateBackup) {
        ...
        const QString backupDirString = info.path() + QString(QDir::separator())
                                        + backupSubdirString;
...
// tr("Removing old backup file %1 failed").arg(backupName));
        ...
        if (dir.exists(basename)) {
            if (!QFile::rename(name,
                               backupDirString + (backupDirString.endsWith("/") ? "" : "/")
                               + backupName)) {
// if (!MScore::noGui)
...
bool Score::saveCompressedFile(QIODevice* f, const QFileInfo& info, bool onlySelection,
                               bool doCreateThumbnail)
{
    MQZipWriter uz(f);
    ...
    if (masterScore()->omr()) {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            if (!image.save(&cbuf1, "PNG")) {
                MScore::lastError = tr("Save file: cannot save image (%1x%2)").arg(
...
static bool writeVoiceMove(XmlWriter& xml, Segment* seg, const Fraction& startTick, int track,
...
void Score::writeSegments(XmlWriter& xml, int strack, int etrack, Segment* sseg, Segment* eseg,
    ...
    for (int track = strack; track < etrack; ++track) {
        ...
        for (Segment* segment = sseg; segment && segment != eseg; segment = segment->next1()) {
            ...
            for (Element* e1 : segment->annotations()) {
                if (e1->track() != track || e1->generated()
                    || (e1->systemFlag() && !writeSystemElements))
                    continue;
                if (needMove) {
                    voiceTagWritten |= writeVoiceMove(xml, segment, startTick, track,
                                                      &lastTrackWritten);
                    needMove = false;
                }
                e1->write(xml);
            ...
            if (forceTimeSig && track2voice(track) == 0
                ...
                voiceTagWritten
                    |= writeVoiceMove(xml, segment, startTick, track, &lastTrackWritten);
