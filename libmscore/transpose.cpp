...
int transposeTpcDiatonicByKey(int tpc, int steps, Key key, bool keepAlteredDegrees,
                              bool useDoubleSharpsFlats)
{
...
bool Score::transpose(Note* n, Interval interval, bool useDoubleSharpsFlats)
{
    ...
    if (n->transposition()) {
        int p;
        transposeInterval(n->pitch() - n->transposition(),
                          n->tpc2(), &p, &ntpc2, interval, useDoubleSharpsFlats);
    } else {
        ntpc2 = ntpc1;
    }
    if (npitch > 127)
        ...
        foreach (Element* e, _selection.uniqueElements()) {
            ...
            if (e->isNote()) {
                ...
                if (mode == TransposeMode::DIATONICALLY) {
                    ...
                    rootTpc = transposeTpcDiatonicByKey(
                        h->rootTpc(),
                        transposeInterval, key, trKeys,
                        useDoubleSharpsFlats);
                    baseTpc = transposeTpcDiatonicByKey(
                        h->baseTpc(),
                        transposeInterval, key, trKeys,
                        useDoubleSharpsFlats);
                } else {
...
void Score::transpositionChanged(Part* part, Interval oldV, Fraction tickStart, Fraction tickEnd)
{
    ...
    for (Staff* ls : part->staff(0)->staffList()) {
        ...
        if (!score->styleB(Sid::concertPitch))
            score->transposeKeys(lp->startTrack() / VOICES,
                                 lp->endTrack() / VOICES, tickStart, tickEnd, diffV);
    }
