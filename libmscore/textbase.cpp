...
// QString family = st.family();
    ...
    return QString("<html><body style=\"font-family:'%1'; font-size:%2pt;\">%3</body></html>").arg(
        family).arg(size).arg(s);
}
...
bool TextBase::validateText(QString& s)
{
    ...
    for (int i = 0; i < s.size(); ++i) {
        ...
        if (c == '&') {
            ...
            const char* ok[] { "b>", "/b>", "i>", "/i>", "u>", "/u", "font ", "/font>", "sym>",
                               "/sym>" };
            QString t = s.mid(i+1);
            bool found = false;
            for (auto k : ok) {
...
void TextBase::draw(QPainter* p) const
{
    if (hasFrame()) {
        ...
        if (frameWidth().val() != 0.0) {
            ...
            qreal frameWidthVal = frameWidth().val()
                                  * (sizeIsSpatiumDependent() ? spatium() : baseSpatium);
        ...
        if (circle()) {
            ...
            qreal frameRoundFactor
                = (sizeIsSpatiumDependent() ? (spatium()/baseSpatium) / 2 : 0.5f);
