...
// otherwise, parentheses will automatically cause contents to be interpreted as extension or modifier
    ...
    for (j = 0, tok1 = "", tok1L = "", initial = ""; i < len; ++i, ++j) {
        ...
        tok1[j] = s[i];
        tok1L[j] = s[i].toLower();
        if (tok1L == "m" || major.contains(tok1L) || minor.contains(tok1L)
            || diminished.contains(tok1L) || augmented.contains(tok1L))
            initial = tok1;
    }
...
// enable this code to export 7sus4 as suspended-fourth,add7 rather than dominant,sub3,add4 (similar for 9sus4, 13sus4)
                ...
                if (tok2L == "4")
                    ...
                    _xmlKind = "suspended-second";
                _xmlText = tok1 + tok2;
                if (_extension == "7" || _extension == "9" || _extension == "11"
                    || _extension == "13") {
    ...
    if (MScore::debugMode) {
        ...
        if (!syntaxOnly) {
            ...
            qDebug("parse: xmlKind = <%s>, text = <%s>", qPrintable(_xmlKind),
                   qPrintable(_xmlText));
            qDebug("parse: xmlSymbols = %s, xmlParens = %s", qPrintable(_xmlSymbols),
                   qPrintable(_xmlParens));
...
QString ParsedChord::fromXml(const QString& rawKind, const QString& rawKindText,
                             const QString& useSymbols, const QString& useParens,
        ...
        for (ChordSymbol s : symbols) {
            if (s.fontIdx == fontIdx) {
                if (s.code.isNull())
                    ...
                    xml.tagE(QString("sym name=\"%1\" code=\"0x%2\"").arg(s.name).arg(s.code.unicode(),
                                                                                      0, 16));
            }
        }
        xml.etag();
