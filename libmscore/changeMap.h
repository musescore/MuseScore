...
public:
    ...
    ChangeEvent(int vel) : value(vel),
        type(ChangeEventType::FIX)
    {
    }

    ChangeEvent(Fraction s, Fraction e, int diff, ChangeMethod m, ChangeDirection d) :
        value(diff),
        type(ChangeEventType::RAMP),
        length(e - s),
        method(m),
        direction(d)
    {
...
class ChangeMap : public QMultiMap<Fraction, ChangeEvent>
{
    ...
    static bool compareRampEvents(ChangeEvent& a, ChangeEvent& b)
    {
...
public:
    ...
    void addRamp(Fraction stick, Fraction etick, int change, ChangeMethod method,
