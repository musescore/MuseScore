...
void FretDiagram::setStrings(int n)
{
    ...
    for (int fret = 1; fret <= _frets; ++fret) {
        if (barre(fret).exists()) {
            ...
            _barres[fret].endString = _barres[fret].endString
                                      == -1 ? -1 : _barres[fret].endString + difference;
        }
    }
...
void FretDiagram::init(StringData* stringData, Chord* chord)
{
    ...
    if (stringData) {
        ...
        for (const Note* note : chord->notes()) {
            int string;
            int fret;
            if (stringData->convertPitch(note->pitch(), chord->staff(), chord->segment()->tick(),
                                         &string, &fret))
                setDot(string, fret);
        }
...
void FretDiagram::draw(QPainter* painter) const
{
    ...
    if (_fretOffset > 0) {
        ...
        scaledFont.setPointSizeF(
            font.pointSize() * _userMag * (spatium() / SPATIUM20)
...
void FretDiagram::layout()
{
    ...
    if (pSeg->isChordRestType()) {
        int idx = staff()->idx();
        for (Element* e = pSeg->firstElementOfSegment(pSeg, idx); e;
             e = pSeg->nextElementOfSegment(pSeg, e, idx)) {
            if (e->isRest()) {
                Rest* r = toRest(e);
    ...
    if (_harmony && _harmony->autoplace() && _harmony->parent()) {
        ...
        QRectF r = _harmony->bbox().translated(
            m->pos() + s->pos() + pos() + _harmony->pos()
            ...
            if (d.exists()) {
                if (d.fret < lowestDotFret || lowestDotFret == -1) {
                    ...
                    furthestLeftLowestDot = i;
                           ...
                           && (i < furthestLeftLowestDot || furthestLeftLowestDot == -1)) {
                    furthestLeftLowestDot = i;
                }
            }
        }
        ...
        // Then read the rest if there is no new format diagram (compatibility read)
            ...
            while (e.readNextStartElement()) {
                ...
                if (t == "dot")
                    ...
                    setMarker(no, QChar(
                                  e.readInt())
                              == 'X' ? FretMarkerType::CROSS : FretMarkerType::CIRCLE);
                /*else if (t == "fingering")
        ...
        if (marker(i).exists() && marker(i).mtype == FretMarkerType::CIRCLE) {
            ...
            for (auto const& d : dot(i)) {
                ...
                if (std::find(bStarts.begin(), bStarts.end(), d.fret) != bStarts.end()) {
                    ...
                    bStarts.erase(std::remove(bStarts.begin(), bStarts.end(), d.fret),
                                  bStarts.end());
                }
...
{
    QString chordName
        = _harmony ? QObject::tr("with chord symbol %1").arg(_harmony->harmonyName()) : QObject::tr(
...
QString FretDiagram::screenReaderInfo() const
{
    ...
    for (int i = 0; i < _strings; i++) {
        ...
        detailedInfo
            = QString("%1 %2 %3 %4").arg(detailedInfo).arg(stringIdent).arg(markerName).arg(dotsInfo);
    }
    ...
    for (auto const& iter : _barres) {
        ...
        if (b.startString == 0 && (b.endString == -1 || b.endString == _strings - 1)) {
            ...
            newBarreInfo = QObject::tr("partial barré %1 %2 %3").arg(fretInfo).arg(startPart).arg(
                endPart);
        }
    ...
    QString chordName = _harmony ? QObject::tr("with chord symbol %1").arg(
        _harmony->generateScreenReaderInfo()) : QObject::tr("without chord symbol");
