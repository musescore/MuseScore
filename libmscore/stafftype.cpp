...
StaffType::StaffType(StaffGroup sg, const QString& xml, const QString& name, int lines, int stpOff,
                     qreal lineDist, bool genClef, bool showBarLines, bool stemless,
                     bool genTimeSig, bool genKeySig, bool showLedgerLines) :
    _group(sg),
    _xmlName(xml),
    _name(name),
    _lines(lines),
...
StaffType::StaffType(StaffGroup sg, const QString& xml, const QString& name, int lines, int stpOff,
                     qreal lineDist, bool genClef, bool showBarLines, bool stemless,
                     bool genTimesig, const QString& durFontName, qreal durFontSize,
                     qreal durFontUserY, qreal genDur, const QString& fretFontName,
                     qreal fretFontSize, qreal fretFontUserY, TablatureSymbolRepeat symRepeat,
                     bool linesThrough, TablatureMinimStyle minimStyle, bool onLines,
...
// QFontMetricsF fm(durationFont());
    ...
    _durationGridYOffset = (TAB_DEFAULT_DUR_YOFFS - (_onLines ? 0.0 : lineDistance().val()*0.5))
                           * SPATIUM20;
...
QString StaffType::fretString(int fret, int string, bool ghost) const
{
    ...
    if (ghost) {
        ...
        return text
               +(_useNumbers
                 ? (fret
                    >= NUM_OF_DIGITFRETS ? unknownFret : _fretFonts[_fretFontIdx].displayDigit[fret])
                 : (fret
                    >= NUM_OF_LETTERFRETS ? unknownFret : _fretFonts[_fretFontIdx].displayLetter[
                        fret]));
    }
}
...
TabDurationSymbol::TabDurationSymbol(Score* s, const StaffType* tab, TDuration::DurationType type,
                                     int dots) :
    Element(s, ElementFlag::NOT_SELECTABLE)
{
    setGenerated(true);
...
void TabDurationSymbol::layout()
{
    ...
    // if on a chord with special beam mode, layout an 'English'-style duration grid
        ...
        if (chord->beamMode() == Beam::Mode::BEGIN) {
            ...
            _beamLevel = static_cast<int>(chord->durationType().type())
...
void TabDurationSymbol::layout2()
{
    ...
    bbox().setWidth(
        _tab->_durationFonts[_tab->_durationFontIdx].gridStemWidth * spatium() * 0.5 * mags
        - beamLen);
}
