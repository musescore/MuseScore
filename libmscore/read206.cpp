...
void readTextStyle206(MStyle* style, XmlReader& e, std::map<QString, std::map<Sid,
                                                                              QVariant> >& excessStyles)
{
...
static void adjustPlacement(Element* e)
{
    ...
    qreal normalize;
    if (defaultPlacement == Placement::BELOW
        && ee->propertyFlags(Pid::OFFSET) == PropertyFlags::STYLED)
    ...
    if (spanner) {
        ...
        for (auto a : spanner->spannerSegments()) {
            ...
            // just adjust offset
            if (defaultPlacement == Placement::BELOW
                && a->propertyFlags(Pid::OFFSET) == PropertyFlags::STYLED)
                normalize = staffHeight;
            else
                normalize = 0.0;
    ...
    if (tag == "durationType") {
        ...
        if (ch->actualDurationType().type() != TDuration::DurationType::V_MEASURE) {
            if (ch->score()->mscVersion() < 112 && (ch->type() == ElementType::REST)
                ...
                (ch->actualDurationType() == TDuration::DurationType::V_WHOLE
    ...
    while (e.readNextStartElement()) {
        ...
        if (tag == "move") {
            ...
            if (!segment) {
                ...
                for (Segment* s = m->segments().first(); s && s->tick() == e.tick();
                     s = s->next()) {
...
static void readStyle(MStyle* style, XmlReader& e)
{
    ...
    if (newChordDescriptionFile != oldChordDescriptionFile && !chordListTag) {
        if (!newChordDescriptionFile.startsWith("chords_")
            && style->value(Sid::chordStyle).toString() == "std") {
            // should not normally happen,
            // but treat as "old" (114) score just in case
            style->set(Sid::chordStyle, QVariant(QString("custom")));
            style->set(Sid::chordsXmlFile, QVariant(true));
            qDebug("StyleData::load: custom chord description file %s with chordStyle == std", qPrintable(
                       newChordDescriptionFile));
        }
...
static bool readScore(Score* score, XmlReader& e)
{
    while (e.readNextStartElement()) {
        ...
        if (tag == "Staff") {
            ...
            score->setScoreFont(ScoreFont::fontFactory(score->style().value(Sid::MusicalSymbolFont).
                                                       toString()));
    ...
    for (Staff* st : score->staves()) {
        ...
        int defaultBarLineTo = stTo->lines(0)
                               == 1 ? BARLINE_SPAN_1LINESTAFF_TO : (stTo->lines(0) - 1) * 2;
