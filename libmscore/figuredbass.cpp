...
bool FiguredBassItem::parse(QString& str)
{
    ...
    if ((_prefix != Modifier::NONE && _suffix != Modifier::NONE)
        || (_prefix == Modifier::NONE && _digit == FBIDigitNone && _suffix == Modifier::NONE
            && _contLine == ContLine::NONE)
        || ((_suffix == Modifier::CROSS || _suffix == Modifier::BACKSLASH
...
void FiguredBassItem::write(XmlWriter& xml) const
{
    ...
    xml.tagE(QString("brackets b0=\"%1\" b1=\"%2\" b2=\"%3\" b3=\"%4\" b4=\"%5\"")
             .arg(int(parenth[0])).arg(int(parenth[1])).arg(int(parenth[2])).arg(
                 int(parenth[3])).arg(int(parenth[4])));
    if (_prefix != Modifier::NONE)
...
void FiguredBassItem::layout()
{
    ...
    if (_digit != FBIDigitNone) {
        ...
        if ((_digit < 10)
            ...
            str.append(g_FBFonts.at(font).displayDigit[style][_digit][int(_suffix)
                                                                      -(int(Modifier::CROSS)-1)]);
        }
...
void FiguredBassItem::draw(QPainter* painter) const
{
    ...
    QPen pen(figuredBass()->curColor(), FB_CONTLINE_THICKNESS * _spatium, Qt::SolidLine,
             Qt::RoundCap);
    painter->setPen(pen);
    ...
    if (parenth[4] != Parenthesis::NONE) {
        ...
        painter->drawText(QRectF(x, 0, bbox().width(),
                                 bbox().height()), Qt::AlignLeft | Qt::AlignTop,
...
void FiguredBassItem::writeMusicXML(XmlWriter& xml, bool isOriginalFigure, int crEndTick,
                                    int fbEndTick) const
{
    xml.stag("figure");
...
FiguredBass* FiguredBass::addFiguredBassToSegment(Segment* seg, int track, const Fraction& extTicks,
                                                  bool* pNew)
{
...
bool FiguredBass::readConfigFile(const QString& fileName)
{
    ...
    if (!fi.open(QIODevice::ReadOnly)) {
        MScore::lastError = QObject::tr("Cannot open figured bass description:\n%1\n%2").arg(
            fi.fileName()).arg(fi.errorString());
...
bool FiguredBass::fontData(int nIdx, QString* pFamily, QString* pDisplayName, qreal* pSize,
                           qreal* pLineHeight)
{
...
void FiguredBass::writeMusicXML(XmlWriter& xml, bool isOriginalFigure, int crEndTick, int fbEndTick,
