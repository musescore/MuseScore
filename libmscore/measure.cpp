...
void Measure::updatePitches(Segment* segment, int staffIdx, int pitch, int tcp, int line, int newAccType)
      {
      Staff* staff            = score()->staff(staffIdx);
      int startTrack          = staffIdx * VOICES;
      int endTrack            = startTrack + VOICES;
                  ...
                  switch(staffGroup) {
                        case PITCHED_STAFF:
                              if (note->tieBack()) {
                                    ...
                                    if(note->accidental()) {    // ise neu
                                          if(note->ACCIDENTAL == newAccType)
                                                score()->undoRemoveElement(note->accidental());
                                          else
                                                return;
                                          }
