...
{
    return _mstaves[staffIdx]->hasVoices();
}

void Measure::setHasVoices(int staffIdx, bool v)
{
    return _mstaves[staffIdx]->setHasVoices(v);
}
...
{
    return _mstaves[staffIdx]->vspacerDown();
}

Spacer* Measure::vspacerUp(int staffIdx) const
{
    return _mstaves[staffIdx]->vspacerUp();
}

void Measure::setStaffVisible(int staffIdx, bool visible)
{
    _mstaves[staffIdx]->setVisible(visible);
}

void Measure::setStaffStemless(int staffIdx, bool stemless)
{
    _mstaves[staffIdx]->setStemless(stemless);
}

#ifndef NDEBUG
bool Measure::corrupted(int staffIdx) const
{
    return _mstaves[staffIdx]->corrupted();
}

void Measure::setCorrupted(int staffIdx, bool val)
{
    _mstaves[staffIdx]->setCorrupted(val);
}

#endif

void Measure::setNoText(int staffIdx, MeasureNumber* t)
{
    _mstaves[staffIdx]->setNoText(t);
}

MeasureNumber* Measure::noText(int staffIdx) const
{
    return _mstaves[staffIdx]->noText();
}
    ...
    for (Score* s : score()->scoreList()) {
        ...
        if (nl > ol) {
            ...
            for (Segment* seg = m->first(); seg; seg = seg->next()) {
                if (seg->segmentType()
                    & (SegmentType::EndBarLine|SegmentType::TimeSigAnnounce
                       |SegmentType::KeySigAnnounce))
                    seg->setRtick(nl);
            }
        }
    }
        ...
        if (rests == 1 && chords == 0) {
            ...
            if (_timesig == nf) {
                rest->undoChangeProperty(Pid::DURATION,
                                         QVariant::fromValue<Fraction>(nf * stretch));
                rest->undoChangeProperty(Pid::DURATION_TYPE,
                                         QVariant::fromValue<TDuration>(TDuration::DurationType::
                                                                        V_MEASURE));
                ...
                for (ScoreElement* e : rest->linkList()) {
                    e->undoChangeProperty(Pid::DURATION,
                                          QVariant::fromValue<Fraction>(durList[0].fraction()));
                    e->undoChangeProperty(Pid::DURATION_TYPE,
                                          QVariant::fromValue<TDuration>(durList[0]));
                }
...
void Measure::write(XmlWriter& xml, int staff, bool writeSystemElements, bool forceTimeSig) const
{
    ...
    for (const Element* e : el()) {
        if (!e->generated()
            && ((e->staffIdx() == staff) || (e->systemFlag() && writeSystemElements)))
            e->write(xml);
    }
    Q_ASSERT(first());
    Q_ASSERT(last());
    if (first() && last())
        score()->writeSegments(xml, strack, etrack, first(),
                               last()->next1(), writeSystemElements, forceTimeSig);
...
void Measure::read(XmlReader& e, int staffIdx)
{
    ...
    if (e.hasAttribute("len")) {
        ...
        if (_len.numerator() <= 0 || _len.denominator() <= 0) {
            e.raiseError(QObject::tr("MSCX error at line %1: invalid measure length: %2").arg(e.
                                                                                              lineNumber()).arg(
                             _len.toString()));
            return;
        }
...
// chord->readAddTuplet(tuplet);
    ...
    if (tuplet) {
        qDebug("Measure:readVoice: measure index=%d, <endTuplet/> not found",
               e.currentMeasureIndex());
...
bool Measure::stemless(int staffIdx) const
{
    ...
    return staff->stemless(tick()) || _mstaves[staffIdx]->stemless()
...
Segment* Measure::searchSegment(qreal x, SegmentType st, int strack, int etrack,
                                const Segment* preferredSegment, qreal spacingFactor) const
{
...
// force = true;
    ...
    if (clefSeg) {
        ...
        if (visibleInt == 2)           // there is at least one visible clef in the clef segment
            ...
            qDebug("Clef Segment without Clef elements at tick %d/%d",
...
// }
            ...
            if (kSegment && staff->isPitchedStaff(tick())) {
                ...
                for (int i = 0; i < score()->nstaves(); ++i) {
                    ...
                    if ((e && !e->generated()) || (key != keyIdx.key())) {
                        disable = false;
                    } else if (e && e->generated() && key == keyIdx.key()
                               && keyIdx.key() == Key::C) {
    ...
    if ((n > 1 && score()->styleB(Sid::startBarlineMultiple))
        || (n == 1 && score()->styleB(Sid::startBarlineSingle))) {
        if (!s) {
...
{
    qreal minWidth = isMMRest() ? score()->styleP(Sid::minMMRestWidth) : score()->styleP(
        Sid::minMeasureWidth);
    if (w < minWidth)
        w = minWidth;
...
void Measure::computeMinWidth(Segment* s, qreal x, bool isSystemHeader)
{
    ...
    const Shape ls(first ? QRectF(0.0, -1000000.0, 0.0, 2000000.0) : QRectF(0.0, 0.0, 0.0,
                                                                            spatium() * 4));
...
void Measure::computeMinWidth()
{
    ...
    //
    Shape ls(first ? QRectF(0.0, -1000000.0, 0.0, 2000000.0) : QRectF(0.0, 0.0, 0.0,
                                                                      spatium() * 4));
