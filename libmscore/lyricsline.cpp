...
SpannerSegment* LyricsLine::layoutSystem(System* system)
{
    ...
    // will be too short).
    const bool tempMelismaTicks
        = (lyrics()->ticks() == Fraction::fromTicks(Lyrics::TEMP_MELISMA_TICKS));
...
void LyricsLineSegment::layout()
{
    ...
    if (!isEndType()) {
        ...
        Lyrics* nextLyr1 = searchNextLyrics(lyr->segment(), lyr->staffIdx(),
                                            lyr->no(), lyr->placement());
    ...
    if (isEndMelisma) {                   // melisma
        ...
        rypos() -= MScore::pixelRatio * lyr->fontMetrics().xHeight() * score()->styleD(
            Sid::lyricsDashYposRatio);
