...
void UndoStack::push(UndoCommand* cmd, EditData* ed)
{
    ...
    if (!strcmp(cmd->name(), "ChangeProperty")) {
        ...
        qCDebug(undoRedo, "<%s> id %d %s", cmd->name(), int(cp->getId()),
                propertyName(cp->getId()));
    } else {
...
CloneVoice::CloneVoice(Segment* _sf, const Fraction& _lTick, Segment* _d, int _strack, int _dtrack,
                       int _otrack, bool _linked)
{
...
const char* AddElement::name() const
{
    ...
    if (element->isTextBase())
        ...
        snprintf(buffer, 64, "Add:    <%s-%s> %p", element->name(), toSegment(
...
const char* RemoveElement::name() const
{
    ...
    if (element->isTextBase())
        ...
        snprintf(buffer, 64, "Remove: <%s-%s> %p", element->name(), toSegment(
                     element)->subTypeName(), element);
    else
...
ChangeFretting::ChangeFretting(Note* _note, int _pitch, int _string, int _fret, int _tpc1,
                               int _tpc2)
{
    note = _note;
    pitch = _pitch;
...
ChangeStaff::ChangeStaff(Staff* _staff, bool _invisible, ClefTypeList _clefType, qreal _userDist,
                         Staff::HideMode _hideMode, bool _showIfEmpty, bool _cutaway, bool hide)
{
    staff = _staff;
...
{
    qCDebug(undoRedo) << element->name() << int(id) << "(" << propertyName(id) << ")"
