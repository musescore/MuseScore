...
void ChordRest::writeProperties(XmlWriter& xml) const
{
    ...
    //
    if ((isRest() && _beamMode != Beam::Mode::NONE)
        || (isChord() && _beamMode != Beam::Mode::AUTO)) {
        QString s;
        switch (_beamMode) {
        case Beam::Mode::AUTO: {
            s = "auto";
    ...
    if (tag == "durationType") {
        ...
        if (actualDurationType().type() != TDuration::DurationType::V_MEASURE) {
            if (score()->mscVersion() < 112 && (type() == ElementType::REST)
                ...
                (actualDurationType() == TDuration::DurationType::V_WHOLE
                 && ticks() <= Fraction(4, 4))) {
        ...
        if (vStaffIdx() < part()->staves()->first()->idx()
            || vStaffIdx() > part()->staves()->last()->idx())
            _staffMove = 0;
    } else if (tag == "Spanner") {
...
QString ChordRest::durationUserName() const
{
    ...
    case 2: {
        dotString
            += QObject::tr("Double dotted %1").arg(durationType().durationTypeUserName()).trimmed();
    }
    break;
    case 3: {
        dotString
            += QObject::tr("Triple dotted %1").arg(durationType().durationTypeUserName()).trimmed();
    }
    break;
    case 4: {
        dotString
            += QObject::tr("Quadruple dotted %1").arg(durationType().durationTypeUserName()).trimmed();
    }
    break;
    default:
