...
ConnectorInfo::ConnectorInfo(const Element* current, int track, Fraction frac) :
    _current(current),
    _score(current->score()),
    _currentLoc(Location::absolute())
{
    if (!current)
...
static int distance(const Location& l1, const Location& l2)
{
    ...
    return 1000 * dpos + 100 * qAbs(l2.track() - l1.track()) + 10 * qAbs(l2.note() - l1.note())
           + qAbs(l2.graceIndex() - l1.graceIndex());
}
...
ConnectorInfoReader::ConnectorInfoReader(XmlReader& e, Element* current, int track) :
    ConnectorInfo(current, track),
    _reader(&e),
    _connector(nullptr),
    _connectorReceiver(current)
{
}
...
ConnectorInfoReader::ConnectorInfoReader(XmlReader& e, Score* current, int track) :
    ConnectorInfo(current, readPositionInfo(e, track)),
    _reader(&e),
    _connector(nullptr),
    _connectorReceiver(current)
{
...
ConnectorInfoWriter::ConnectorInfoWriter(XmlWriter& xml, const Element* current,
                                         const Element* connector, int track, Fraction frac) :
    ConnectorInfo(current, track, frac),
    _xml(&xml),
    _connector(connector)
{
    if (!connector) {
        qFatal("ConnectorInfoWriter::ConnectorInfoWriter(): invalid arguments: %p, %p", connector,
               current);
        return;
    }
