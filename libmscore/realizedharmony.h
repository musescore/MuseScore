...
public:
    RealizedHarmony() : _harmony(0),
        _notes(PitchMap()),
        _dirty(1)
    {
    }

    RealizedHarmony(Harmony* h) : _harmony(h),
        _notes(PitchMap()),
        _dirty(1)
    {
    }
    ...
    {
        cascadeDirty(dirty);
    }                                                    // set dirty flag and cascade

    void setHarmony(Harmony* h)
    {
        _harmony = h;
    }
    ...
    bool valid() const
    {
        return !_dirty && _harmony;
    }

    const QList<int> pitches() const
    {
        return notes().keys();
    }

    const QList<int> tpcs() const
    {
        return notes().values();
    }
