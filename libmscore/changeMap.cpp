...
void ChangeMap::addRamp(Fraction stick, Fraction etick, int change, ChangeMethod method,
...
void ChangeMap::cleanupStage3()
{
    for (auto i = begin(); i != end(); i++) {
        ...
        if ((event.cachedStartVal > event.cachedEndVal
             && event.direction == ChangeDirection::INCREASING)
            || (event.cachedStartVal < event.cachedEndVal
                && event.direction == ChangeDirection::DECREASING))
            event.cachedEndVal = event.cachedStartVal;
    }
}
...
std::vector<std::pair<Fraction, Fraction> > ChangeMap::changesInRange(Fraction stick,
                                                                      Fraction etick)
{
    if (!cleanedUp)
        cleanup();
...
void ChangeMap::dump()
{
    ...
    for (auto i = begin(); i != end(); i++) {
        ...
        if (event.type == ChangeEventType::FIX)
            ...
            qDebug().nospace() << "===" << tick.ticks() << " to "
                               << (tick + event.length).ticks() << " : RAMP diff " << event.value
                               << " "
                               << ChangeMap::changeMethodToName(event.method) << " ("
