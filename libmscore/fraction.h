...
public:
    ...
    constexpr Fraction(int z = 0, int n = 1) : _numerator(z),
        _denominator(n)
    {
    }
    ...
    constexpr Fraction(int z, int n) : _numerator{n < 0 ? -z : z},
        _denominator{n < 0 ? -n : n}
    {
    }
    ...
    static Fraction eps()
    {
        return Fraction(1, MScore::division* 4);
    }
    ...
    static Fraction fromString(const QString& str)
    {
        ...
        return (i == -1) ? Fraction(str.toInt(), 1) : Fraction(str.leftRef(i).toInt(),
                                                               str.midRef(i+1).toInt());
    }
...
inline Fraction operator*(const Fraction& f, int v)
{
    return Fraction(f) *= v;
}

inline Fraction operator*(int v, const Fraction& f)
{
    return Fraction(f) *= v;
}
}     // namespace Ms
