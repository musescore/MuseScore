...
void TextLineBaseSegment::draw(QPainter* painter) const
{
    ...
    if ((npoints == 0)
        || (score() && (score()->printing() || !score()->showInvisible()) && !tl->lineVisible()))
        return;
    ...
    if (twoLines) {     // hairpins
        ...
        if (tl->lineStyle() == Qt::CustomDashLine) {
            ...
            if (tl->beginHookType() == HookType::HOOK_45
                || tl->beginHookType() == HookType::HOOK_90) {
                qreal absD
    ...
    if (twoLines) {     // hairpins
        ...
        for (int i = 0; i < npoints; ++i) {
            shape.add(QRectF(points[i].x() - lw2, points[i].y() - lw2,
                             points[i+1].x() - points[i].x() + lw,
                             points[i+1].y() - points[i].y() + lw));
        }
    }
    return shape;
}
...
void TextLineBaseSegment::layout()
{
    ...
    if (!_text->empty()) {
        ...
        if (((isSingleType() || isBeginType())
             && (tl->beginTextPlace() == PlaceText::LEFT
                 || tl->beginTextPlace() == PlaceText::AUTO))
