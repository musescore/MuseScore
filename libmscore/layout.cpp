...
static bool vUp(Chord* chord)
{
    if (!chord) {
        ...
        if (chord->stemDirection() != Direction::AUTO)
            ...
            up = true;
        else if (chord->measure()->hasVoices(chord->staffIdx(), chord->tick(),
                                             chord->actualTicks()))
            up = !(chord->track() % 2);
        else
    ...
    for (int track = startTrack; track < endTrack; ++track) {
        ...
        if (e && e->isChord()) {
            ...
            if (chord->up()) {
                ...
                ++downVoices;
                downStemNotes.insert(downStemNotes.end(), chord->notes().begin(),
                                     chord->notes().end());
            ...
            if (separation == 1) {
                ...
                for (int i = 0, count = overlapNotes.size(); i < count; ++i) {
                    ...
                    case 0: {
                        ...
                        nHeadType
                            = (n->headType()
                               == NoteHead::Type::HEAD_AUTO) ? n->chord()->durationType().headType()
                              : n->headType();
                        pHeadType
                            = (p->headType()
                               == NoteHead::Type::HEAD_AUTO) ? p->chord()->durationType().headType()
                              : p->headType();
                        ...
                        if (n->headGroup() != p->headGroup() || n->tpc() != p->tpc()
                            || n->mirror() || p->mirror()
                            ...
                            if ((nchord->dots() != pchord->dots() || !nchord->stem()
                                 || !pchord->stem() || nHeadType != pHeadType || n->small()
                                 || p->small())
                                && ((n->headType() == NoteHead::Type::HEAD_AUTO
                                     && p->headType() == NoteHead::Type::HEAD_AUTO)
                                    || nHeadType != pHeadType)
                                && (n->visible() == p->visible()))
                                shareHeads = false;
                        }
                ...
                if (shareHeads) {
                    ...
                    if (bottomUpNote->chord()->stem())
                        clearRight = bottomUpNote->chord()->stem()->lineWidth() + qMax(
                            maxDownWidth - maxUpWidth, 0.0) + 0.3 * sp;
                    else
...
static qreal layoutAccidental(AcEl* me, AcEl* above, AcEl* below, qreal colOffset,
                              QVector<Note*>& leftNotes, qreal pnd, qreal pd, qreal sp)
{
    qreal lx = colOffset;
...
void Score::layoutChords3(std::vector<Note*>& notes, const Staff* staff, Segment* segment)
{
    ...
    for (int i = nNotes-1; i >= 0; --i) {
        ...
        if (chord->dots()) {
            ...
            if (dotPosition == Direction::AUTO && nNotes > 1 && note->visible()
                && !note->dotsHidden()) {
...
#define beamModeMid(a) (a == Beam::Mode::MID || a == Beam::Mode::BEGIN32 \
                        || a == Beam::Mode::BEGIN64)
...
void Score::beamGraceNotes(Chord* mainNote, bool after)
{
    ...
    for (ChordRest* cr : graceNotes) {
        ...
        if ((cr->durationType().type() <= TDuration::DurationType::V_QUARTER)
            || (bm == Beam::Mode::NONE)) {
            if (beam) {
...
void Score::layoutSpanner()
{
    ...
    for (int track = 0; track < tracks; ++track) {
        for (Segment* segment = firstSegment(SegmentType::All); segment;
            ...
            if (hideStaff && (n > 1)) {
                ...
                for (int i = 0; i < part->nstaves(); ++i) {
                    ...
                    for (MeasureBase* mb : system->measures()) {
                        ...
                        if (staff->hideWhenEmpty() == Staff::HideMode::INSTRUMENT
                            && !m->isEmpty(st)) {
                            hideStaff = false;
                            break;
                        }
...
struct Spring {
    ...
    qreal stretch;
    qreal fix;
    Spring(int i, qreal s, qreal f) : seg(i),
        stretch(s),
        fix(f)
    {
    }
            ...
            if (e) {
                ...
                if (!ds->element(staffIdx * VOICES)) {
                    ...
                    if (bd->barLineType() != bs->barLineType()) {
                        ...
                        undo(new ChangeProperty(bd, Pid::BARLINE_TYPE,
                                                QVariant::fromValue(bs->barLineType()),
                                                PropertyFlags::NOSTYLE));
                        undo(new ChangeProperty(bd, Pid::GENERATED, generated,
                                                PropertyFlags::NOSTYLE));
                    }
                }
            }
    ...
    if (cs) {
        ...
        for (Element* e : cs->annotations()) {
            ...
            if (!(e->isRehearsalMark() || e->isTempoText() || e->isHarmony() || e->isStaffText()
                  || e->isSystemText() || e->isInstrumentChange()))
                continue;
...
static bool breakMultiMeasureRest(Measure* m)
{
    ...
    for (auto i : sl) {
        ...
        if ((s->isVolta() || s->isTextLine())
            && (s->tick() == m->tick() || s->tick2() == m->tick()))
            return true;
    }
    ...
    for (Segment* s = m->first(); s; s = s->next()) {
        for (Element* e : s->annotations()) {
            ...
            if (e->isRehearsalMark()
                || e->isTempoText()
                || ((e->isHarmony() || e->isStaffText() || e->isSystemText()
            ...
            if (bm == Beam::Mode::AUTO) {
                ...
                if (checkBeats && cr->rtick().isNotZero()) {
                    ...
                    if ((tick.ticks() % MScore::division) == 0) {
                        ...
                        TDuration minDuration = qMin(beatSubdivision[beat],
                                                     beatSubdivision[beat - 1]);
...
std::pair<qreal, qreal> extendedStemLenWithTwoNoteTremolo(Tremolo* tremolo, qreal stemLen1,
                                                          qreal stemLen2)
{
...
// TODO: cross-staff two-note tremolo. Currently doesn't generate the right result in some cases.
    ...
    // cross-staff & beam between staves: extend both stems by the same length
        ...
        const qreal dy = c1->up() ? tremoloMinHeight - stemTipDistance : tremoloMinHeight
                         + stemTipDistance;
...
void Score::getNextMeasure(LayoutContext& lc)
{
    ...
    for (int staffIdx = 0; staffIdx < score()->nstaves(); ++staffIdx) {
        ...
        const Drumset* drumset
            = staff->part()->instrument()->useDrumset() ? staff->part()->instrument()->drumset() : 0;
                ...
                for (int t = track; t < endTrack; ++t) {
                    ...
                    if (cr->isChord()) {
                        ...
                        if (chord->tremolo() && chord == chord->tremolo()->chord2()) {
                            ...
                            if (stem1 && stem2) {
                                std::pair<qreal,
                                          qreal> extendedLen = extendedStemLenWithTwoNoteTremolo(
                                    chord->tremolo(),
                                    stem1->p2().y(),
                                    stem2->p2().y());
        ...
        if (cr && !cr->lyrics().empty()) {
            ...
            for (Lyrics* l : cr->lyrics()) {
                if (l->autoplace() && l->placeAbove()) {
                    qreal yOff = l->offset().y();
                    QRectF r = l->bbox().translated(
                        l->pos() + cr->pos() + s.pos() + s.measure()->pos());
                    r.translate(0.0, -yOff);
        ...
        if (cr && !cr->lyrics().empty()) {
            ...
            for (Lyrics* l : cr->lyrics()) {
                if (l->autoplace() && l->placeBelow()) {
                    ...
                    if (l->addToSkyline()) {
                        QPointF offset = l->pos() + cr->pos() + s.pos() + s.measure()->pos();
                        sk.add(l->bbox().translated(offset).adjusted(0.0, 0.0, 0.0,
...
static void applyLyricsMin(ChordRest* cr, int staffIdx, qreal yMin)
{
    ...
    for (Lyrics* l : cr->lyrics()) {
        if (l->autoplace() && l->placeAbove()) {
            ...
            if (l->addToSkyline()) {
                QPointF offset = l->pos() + cr->pos() + cr->segment()->pos()
                                 + cr->segment()->measure()->pos();
...
System* Score::collectSystem(LayoutContext& lc)
{
    ...
    if (measure) {
        ...
        lc.startWithLongNames = lc.firstSystem
                                && measure->sectionBreakElement()->startWithLongNames();
    }
    ...
    if (lc.endTick < lc.prevMeasure->tick()) {
        ...
        if (lc.prevMeasure == lc.systemOldMeasure) {
            ...
            if (lc.curMeasure && lc.curMeasure->isMeasure()) {
                ...
                bool firstSystem = lc.prevMeasure->sectionBreak()
                                   && _layoutMode != LayoutMode::FLOAT;
                if (curHeader)
    ...
    if (MScore::noHorizontalStretch) {
        ...
        if (!qFuzzyCompare(mw, minWidth))
            qDebug("==layoutSystem %6d old %.1f new %.1f",
                   system->measures().front()->tick().ticks(), minWidth, mw);
#endif
    ...
    for (int staffIdx = 0; staffIdx < nstaves(); ++staffIdx) {
        ...
        for (MeasureBase* mb : system->measures()) {
            ...
            for (Segment& s : m->segments()) {
                ...
                if (s.segmentType()
                    & (SegmentType::BarLine | SegmentType::EndBarLine
                    ...
                    for (Element* e : s.elist()) {
                        ...
                        if (e->isChord() && toChord(e)->tremolo()) {
                            ...
                            Chord* c2 = t->chord2();
                            if (!t->twoNotes()
                                || (c1 && !c1->staffMove() && c2 && !c2->staffMove())) {
...
void LayoutContext::collectPage()
{
    ...
    qreal y = page->systems().isEmpty() ? page->tm() : page->system(0)->y()
              + page->system(0)->height();
            ...
            if (!nextSystem && score->isMaster()) {
                ...
                if (ms) {
                    ...
                    if (systems.empty() || systems.front()->measures().empty()) {
                        ...
                        ms->setNoteHeadWidth(ms->scoreFont()->width(SymId::noteheadBlack,
                                                                    ms->spatium() / SPATIUM20));
                    } else {
                        rangeDone = true;
                        systemIdx = 0;
                        nextSystem = score->systems().value(systemIdx++);
...
class CmdStateLocker
{
    Score* score;
public:
    CmdStateLocker(Score* s) : score(s)
    {
        score->cmdState().lock();
    }
...
// if (!_systems.isEmpty())
// return;
    bool layoutAll = stick
                     <= Fraction(0,
                                 1)
                     && (etick < Fraction(0, 1) || etick >= masterScore()->last()->endTick());
