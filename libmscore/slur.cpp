...
bool SlurSegment::edit(EditData& ed)
{
    ...
    if (ed.key == Qt::Key_X) {
        sl->undoChangeProperty(Pid::SLUR_DIRECTION,
                               QVariant::fromValue<Direction>(sl->up() ? Direction::DOWN : Direction
                                                              ::UP));
        sl->layout();
        return true;
    }
...
void SlurSegment::changeAnchor(EditData& ed, Element* element)
{
    ...
    for (ScoreElement* e : spanner()->linkList()) {
        ...
        score()->undoStack()->push1(new ChangeStartEndSpanner(sp, sp->startElement(),
                                                              sp->endElement()));
    }
...
void SlurSegment::computeBezier(QPointF p6o)
{
    ...
    if ((p2.x() == 0.0) && (p2.y() == 0.0)) {
        ...
        qDebug("zero slur at tick %d(%d) track %d in measure %d-%d  tick %d ticks %d",
               m1->tick().ticks(), tick().ticks(), track(), m1->no(), m2->no(),
               slur()->tick().ticks(), slur()->ticks().ticks());
        slur()->setBroken(true);
        return;
    }
    ...
    if (autoplace() && system()) {
        ...
        for (int tries = 1; true; ++tries) {
            for (Segment* s = fs; s && s != ls; s = s->next1()) {
                ...
                if (pp2.x() < x1)
                    break;
                const Shape& segShape = s->staffShape(staffIdx()).translated(
                    s->pos() + s->measure()->pos());
                if (!intersection)
...
void Slur::slurPos(SlurPos* sp)
{
    ...
    if (scr->vStaffIdx() != vStaffIdx() && sp->system1) {
        qreal diff = sp->system1->staff(scr->vStaffIdx())->y()
                     - sp->system1->staff(vStaffIdx())->y();
        sp->p1.ry() += diff;
    }
    if (ecr->vStaffIdx() != vStaffIdx() && sp->system2) {
        qreal diff = sp->system2->staff(ecr->vStaffIdx())->y()
...
SpannerSegment* Slur::layoutSystem(System* system)
{
    ...
    if (tick() >= stick) {
        ...
        case Direction::AUTO:
        {
            ...
            if (c1 && c2 && isDirectionMixture(c1, c2) && !c1->isGrace()) {
                ...
                _up = true;
            } else if (m1->hasVoices(startCR()->staffIdx(), tick(),
                                     ticks()) && c1 && !c1->isGrace()) {
    ...
    case SpannerSegmentType::MIDDLE: {
        ...
        qreal y = staffIdx()
                  > system->staves()->size() ? system->y() : system->staff(staffIdx())->y();
    ...
    case Direction::AUTO:
    {
        ...
        if (c1 && c2 && isDirectionMixture(c1, c2) && (c1->noteType() == NoteType::NORMAL)) {
            ...
            _up = true;
                                 ...
                                 ticks()) && c1 && c1->noteType() == NoteType::NORMAL) {
            // in polyphonic passage, slurs go on the stem side
            _up = startCR()->up();
        } else if (c1 && c2 && chordsHaveTie(c1, c2)) {
            // could confuse slur with tie, put slur on stem side
