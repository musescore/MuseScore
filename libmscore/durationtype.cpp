...
{
    if (_val == DurationType::V_MEASURE || _val == DurationType::V_INVALID
                          ...
                          const Fraction& rtickStart, const TimeSigFrac& nominal, int maxDots)
{
    ...
    if (!needToSplit && strongestLevelCrossed == 0) {
        ...
        BeatType strongestBeatCrossed = nominal.strongestBeatInRange(
            rtickStart.ticks(), rtickEnd.ticks(), &dUnitsCrossed, &rtickSplit, useLast);
...
bool forceRhythmicSplit(bool isRest, BeatType startBeat, BeatType endBeat, int dUnitsCrossed,
...
QString TDuration::durationTypeUserName() const
{
    ...
    break;
    case DurationType::V_MEASURE: {
        s = QObject::tr("Measure");
    }
    break;
    case DurationType::V_INVALID: {
        s = QObject::tr("Invalid");
    }
    break;
    }
    return s;
