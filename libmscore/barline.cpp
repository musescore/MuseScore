    ...
    case BarLineType::DOTTED: {
        ...
        if (segmentType == SegmentType::EndBarLine) {
            ...
            const std::vector<Element*>& elist
                = allStaves ? segment->elist() : std::vector<Element*> { bl };
            for (Element* e : elist) {
            ...
            for (Element* e : segment1->elist()) {
                if (e) {
                    e->score()->undo(new ChangeProperty(e, Pid::GENERATED, false,
                                                        PropertyFlags::NOSTYLE));
                    e->score()->undo(new ChangeProperty(e, Pid::BARLINE_TYPE,
                                                        QVariant::fromValue(barType),
                                                        PropertyFlags::NOSTYLE));
                    // set generated flag before and after so it sticks on type change and also works on undo/redo
                    e->score()->undo(new ChangeProperty(e, Pid::GENERATED, false,
...
public:
    ...
    {
        return EditDataType::BarLineEditData;
    }
};
...
const std::vector<BarLineTableItem> BarLine::barLineTable {
    ...
    { BarLineType::START_REPEAT, QT_TRANSLATE_NOOP("Palette", "Start repeat barline"),
      "start-repeat" },
    ...
    { BarLineType::END_START_REPEAT, QT_TRANSLATE_NOOP("Palette", "End-start repeat barline"),
      "end-start-repeat" },
...
void BarLine::drawDots(QPainter* painter, qreal x) const
{
    ...
    if (parent() == 0) {      // for use in palette (always Bravura)
        ...
        qreal offset = score()->scoreFont()->name()
                       == "Bravura" ? 0 : 0.5 * score()->spatium() * mag();
...
void BarLine::drawTips(QPainter* painter, bool reversed, qreal x) const
{
    if (reversed) {
        if (isTop())
            drawSymbol(SymId::reversedBracketTop, painter,
                       QPointF(x - symWidth(SymId::reversedBracketTop), y1));
        if (isBottom())
            drawSymbol(SymId::reversedBracketBottom, painter,
                       QPointF(x - symWidth(SymId::reversedBracketBottom), y2));
    } else {
        if (isTop())
        ...
        if (_span > staff()->barLineSpan()) {
            ...
            for (int idx = idx1 + 1; idx < idx2; ++idx) {
                ...
                if (_spanTo > 0 || !(idx == idx2-1 && idx != score()->nstaves()-1)) {
                    ...
                    staff->undoChangeProperty(Pid::STAFF_BARLINE_SPAN_TO,
                                              (staff->lines(tick())-1)*2);
                }
            }
        }
    ...
    if (ed.curGrip == Grip::START) {
        ...
        const qreal max
            = (system
               && st
               != -1) ? (system->height() - y2
                         - system->staff(st)->y()) : std::numeric_limits<qreal>::max();
...
// int staff2lines   = staff2->lines(tick());
    ...
    if (shiftDrag) {                      // if precision dragging
        ...
        if (yoff2 != 0.0) {
            ...
            qreal staff2TopY = systTopY + syst->staff(staffIdx2)->y();
            newSpanTo
                = ((int)floor((ay2 - staff2TopY) / (staff2->lineDistance(tick()) * spatium())
                              + 0.5)) * 2;
            // min = 1 line dist above 1st staff line | max = 1 line dist below last staff line
...
void BarLine::layout()
{
    ...
    if (staff() && segment()) {
        if ((!staff()->staffTypeForElement(this)->showBarlines()
             && segment()->segmentType() == SegmentType::EndBarLine)
            || (staff()->hideSystemBarLine()
                && segment()->segmentType() == SegmentType::BeginBarLine)) {
            setbbox(QRectF());
            return;
        }
    }
