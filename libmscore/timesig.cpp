...
void TimeSig::layout()
      ...
      if (sigType ==  TimeSigType::FOUR_FOUR) {
            ...
            if (!_parserString.isEmpty()) {
                  ...
                  qDebug("pString before %s", qPrintable(pString));
                  ...
                  for (int i=0; i < sList.size(); i++) {
                        ...
                        if (numDem.size() == 1) {
                              ns.push_back(toTimeSigString(numDem[0], largeTimeSig(), true)); // if no denominator, use large '()' and '+' (true)
                              ds.push_back(toTimeSigString("", largeTimeSig(), false));       // empty denominator timeSigString
                              }
                        else if (numDem.size() >= 2) {
                              ns.push_back(toTimeSigString(numDem[0], largeTimeSig(), false));
                              ds.push_back(toTimeSigString(numDem[1], largeTimeSig(), false));
                              }
                        else
                  ...
                  pns = { QPointF() };
                  pds = { QPointF() };
                  ns = { toTimeSigString(QString::number(_sig.numerator()), largeTimeSig(), false) };
                  ds = { toTimeSigString(QString::number(_sig.denominator()), largeTimeSig(), false) };
                  // (old num/den)
                  //if (_numeratorString.isEmpty()) {
                  //      ns = { toTimeSigString(QString::number(_sig.numerator()), largeTimeSig(), false) };
                  //      ds = { toTimeSigString((_denominatorString.isEmpty() ? QString::number(_sig.denominator()) : _denominatorString), largeTimeSig(), false) };
                  //      }
                  //else {
