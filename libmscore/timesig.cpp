...
void TimeSig::layout()
{
    ...
    if (sigType == TimeSigType::FOUR_FOUR) {
        ...
        if (_numeratorString.isEmpty()) {
            ns = toTimeSigString(_numeratorString.isEmpty() ? QString::number(
                                     _sig.numerator()) : _numeratorString);
            ds = toTimeSigString(_denominatorString.isEmpty() ? QString::number(
        ...
        if (_largeParentheses) {
            addbbox(QRect(pointLargeLeftParen.x(), pointLargeLeftParen.y() - denRect.height(),
                          _spatium / 2, numRect.height() + denRect.height()));
            addbbox(QRect(pointLargeRightParen.x(), pointLargeRightParen.y() - denRect.height(),
                          _spatium / 2, numRect.height() + denRect.height()));
        }
    }
}
...
QString TimeSig::accessibleInfo() const
{
    ...
    break;
    default:
        timeSigString = QObject::tr("%1/%2 time").arg(QString::number(numerator())).arg(QString::number(
                                                                                            denominator()));
    }
