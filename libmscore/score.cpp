...
Measure* Score::pos2measure(const QPointF& p, int* rst, int* pitch, Segment** seg,
...
QList<System*> Score::searchSystem(const QPointF& pos, const System* preferredSystem,
...
Measure* Score::searchMeasure(const QPointF& p, const System* preferredSystem,
                              qreal spacingFactor) const
{
...
bool Score::getPosition(Position* pos, const QPointF& p, int voice) const
{
    ...
    qreal lineDist = s->staffType(tick)->lineDistance().val()
                     * (s->isTabStaff(measure->tick()) ? 1 : .5) * mag * spatium();
                                    ...
                                    const Fraction& endTick)
{
    ...
    for (Measure* m = firstAppendedMeasure; m; m = m->nextMeasure())
        for (int staffIdx = 0; staffIdx < nstaves(); ++staffIdx) {
            ...
            if (f.isZero())
                addRest(m->tick(), staffIdx*VOICES, TDuration(TDuration::DurationType::V_MEASURE),
                        0);
        }
...
void Score::cmdConcertPitchChanged(bool flag, bool /*useDoubleSharpsFlats*/)
{
    ...
    for (Staff* staff : _staves) {
        ...
        transposeKeys(staffIdx, staffIdx + 1, Fraction(0, 1),
                      lastSegment()->tick(), interval, true, !flag);
    ...
    if (selection().isSingle()) {
        ...
        for (Element* e : elements) {
            ...
            if (cr->isRepeatMeasure()
                || (cr->isRest() && toRest(cr)->measure() && toRest(cr)->measure()->isMMRest())) {
...
void Score::selectRange(Element* e, int staffIdx)
{
    ...
    if (e->isMeasure()) {
        ...
        if (_selection.isNone() || (_selection.isList() && !_selection.isSingle())) {
            ...
            if (oe->isNote() || oe->isChordRest()) {
                ...
                if (tick < oetick) {
                    ...
                    if (etick <= oetick) {
                        SegmentType st = SegmentType::ChordRest | SegmentType::EndBarLine
                                         | SegmentType::Clef;
        ...
        if (_selection.isNone() || (_selection.isList() && !_selection.isSingle())) {
            ...
            _selection.setRange(cr->segment(), cr->nextSegmentAfterCR(st),
                                e->staffIdx(), e->staffIdx() + 1);
        ...
        for (const RepeatSegment* rs : rlist) {
            ...
            for (Measure* m = tick2measure(startTick); m; m = m->nextMeasure()) {
                ...
                for (Segment* seg = m->first(st); seg; seg = seg->next(st)) {
                    ...
                    if (l->syllabic() == Lyrics::Syllabic::SINGLE
                        || l->syllabic() == Lyrics::Syllabic::END)
                        result += lyric + " ";
...
QString Score::nextRehearsalMarkText(RehearsalMark* previous, RehearsalMark* current) const
{
    ...
    } else if (previousText.length() == 2 && previousText[0].isLetter()
               && previousText[1].isLetter()) {
...
void Score::cropPage(qreal margins)
{
    if (npages() == 1) {
        ...
        if (page) {
            ...
            f.setSize(QSizeF((ttbox.width() / DPI) + 2 * margin,
                             (ttbox.height()/ DPI) + 2 * margin));
...
void MasterScore::setLayout(const Fraction& tick1, const Fraction& tick2, int staff1, int staff2,
                            const Element* e)
{
