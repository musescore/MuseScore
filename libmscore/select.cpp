...
bool SelectionFilter::canSelect(const Element* e) const
{
    ...
    if ((e->isArticulation() && !toArticulation(
             e)->isOrnament()) || e->isVibrato() || e->isFermata())
...
void Selection::updateSelectedElements()
{
    ...
    if (_state == SelState::RANGE
        && _plannedTick1 != Fraction(-1, 1) && _plannedTick2 != Fraction(-1, 1)) {
    ...
    if (_staffStart < 0 || _staffStart >= staves || _staffEnd < 0 || _staffEnd > staves
        ...
        qDebug("updateSelectedElements: bad staff selection %d - %d, staves %d", _staffStart,
               _staffEnd, staves);
        _staffStart = 0;
        _staffEnd = 0;
    }
...
{
    Q_ASSERT(
        staffEnd > staffStart && staffStart >= 0 && staffEnd >= 0
...
void Selection::setRangeTicks(const Fraction& tick1, const Fraction& tick2, int staffStart,
                              int staffEnd)
{
    Q_ASSERT(
        staffEnd > staffStart && staffStart >= 0 && staffEnd >= 0
...
QByteArray Selection::staffMimeData() const
{
    ...
    if (!MScore::testMode)
        xml.stag(QString("StaffList version=\"" MSC_VERSION
                         "\" tick=\"%1\" len=\"%2\" staff=\"%3\" staves=\"%4\"").arg(tickStart().
                                                                                     ticks()).arg(
                     ticks.ticks()).arg(staffStart()).arg(staves));
    else
        xml.stag(QString(
                     "StaffList version=\"2.00\" tick=\"%1\" len=\"%2\" staff=\"%3\" staves=\"%4\"").arg(
...
QByteArray Selection::symbolListMimeData() const
{
    ...
    foreach (Element* e, _el) {
        ...
        */
        ...
        mapData.e = e;
        mapData.s = seg;
        map.insert(std::pair<qint64, MapData>(((qint64)track << 32) + seg->tick().ticks(),
                                              mapData));
    }
...
bool Selection::canCopy() const
{
    ...
    for (int staffIdx = _staffStart; staffIdx != _staffEnd; ++staffIdx) {
        ...
        for (Measure* m = _startSegment->measure(); m && m->tick() < endTick;
             m = m->nextMeasure()) {
...
void Selection::extendRangeSelection(Segment* seg, Segment* segAfter, int staffIdx,
