...
int MasterScore::getNextFreeMidiMapping(int p, int ch)
{
    ...
    for (;; searchMidiMappingFrom++) {
        if (searchMidiMappingFrom % 16 != 9
            && !occupiedMidiChannels.contains(searchMidiMappingFrom)) {
    ...
    for (Excerpt* ex : excerpts()) {
        for (Part* p : ex->partScore()->parts()) {
            ...
            if (!masterPart->score()->isMaster()) {
                qWarning()
                    << "rebuildExcerptsMidiMapping: no part in master score is linked with "
                    << p->partName();
                continue;
            }
    ...
    for (int index = 0; index < mappingSize; index++) {
        ...
        for (auto i = il->begin(); i != il->end() && !channelExists; ++i) {
            ...
            channelExists = (_midiMapping[index].articulation()->channel() != -1
                             ...
                             && !(_midiMapping[index].port() == -1
                                  && _midiMapping[index].channel() == -1));
            if (channelExists)
                break;
        }
        if (!channelExists) {
            ...
            for (Channel* channel : instr->channel()) {
                ...
                if (channelExists) {
                    if (_midiMapping[channel->channel()].port() == -1) {
                        const int nm = getNextFreeMidiMapping(-1,
                        ...
                        if (drum) {
                            _midiMapping[channel->channel()]._port = getNextFreeDrumMidiMapping()
                                                                     / 16;
