...
Rectangle {
    ...
    ColumnLayout {
        ...
        // Rhythm pattern display with proportional spacing
        Item {
            id: rhythmContainer
            Layout.fillWidth: true
            ...
            property var cellData: root.rhythmCells.length > 0 ? root.rhythmCells : root.rhythmPattern
            property real totalBeats: {
                var total = 0;
                for (var i = 0; i < cellData.length; i++) {
                    var dur = cellData[i].duration || cellData[i];
                    total += durationToBeats(dur);
                }
            ...
            Repeater {
                model: rhythmContainer.cellData
                
                Item {
                    // Calculate position and width based on beats
                    property real cellBeats: {
                        var dur = modelData.duration || modelData;
                        return rhythmContainer.durationToBeats(dur);
                    }
                    property real startBeat: {
                        var start = 0;
                        for (var i = 0; i < index; i++) {
                            var dur = rhythmContainer.cellData[i].duration || rhythmContainer.cellData[i];
                            start += rhythmContainer.durationToBeats(dur);
                        }
                        return start;
                    }
                    
                    x: 10 + startBeat * rhythmContainer.beatWidth
                    width: cellBeats * rhythmContainer.beatWidth
                    height: rhythmContainer.height
                    
                    // Note/Rest symbol at the left of its proportional space
                    Row {
