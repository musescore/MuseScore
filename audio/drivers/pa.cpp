...
bool Portaudio::init(bool)
{
    ...
    if (idx < 0) {
        ...
        qDebug("No device selected.  PortAudio detected %d devices.  Will use the default device (index %d).",
               Pa_GetDeviceCount(), idx);
    }
...
void Portaudio::putEvent(const NPlayEvent& e, unsigned framePos)
{
    ...
    case ME_CONTROLLER:
        ...
        if (e.dataA() == CTRL_PROGRAM) {
            ...
            PmError error = Pm_WriteShort(
                portMidiDriver->getOutputStream(), seq->getCurrentMillisecondTimestampWithLatency(framePos), msg);
            if (error != pmNoError) {
    ...
    case ME_PITCHBEND:
    {
        ...
        PmError error
            = Pm_WriteShort(portMidiDriver->getOutputStream(), seq->getCurrentMillisecondTimestampWithLatency(
                                framePos), msg);
        if (error != pmNoError) {
            qDebug("Portaudio: error %d", error);
            return;
