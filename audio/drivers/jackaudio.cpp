...
void JackAudio::timebase(jack_transport_state_t state, jack_nframes_t /*nframes*/, jack_position_t* pos,
                         int /*new_pos*/, void* arg)
{
    ...
    if (!audio->seq->score()) {
        ...
        if (audio->timeSigTempoChanged) {
            ...
            qDebug() << "Time signature changed: " << pos->beats_per_minute << ", bar: " << pos->bar << ",beat: "
                     << pos->beat << ", tick:" << pos->tick << ", time sig: " << pos->beats_per_bar << "/"
                     << pos->beat_type;
        }
    }
    // TODO: Handle new_pos
...
void JackAudio::checkTransportSeek(int cur_frame, int nframes, bool inCountIn)
{
    ...
    if (preferences.getBool(PREF_IO_JACK_USEJACKTRANSPORT)) {
        ...
        if (labs((long int)cur_frame - (long int)pos.frame) > nframes + 1
            && abs(utick - cur_utick) > seq->score()->utime2utick((qreal)nframes / MScore::sampleRate) + 1) {
            if (MScore::debugMode) {
                qDebug() << "JACK Transport position changed, cur_frame: " << cur_frame << ",pos.frame: "
                         << pos.frame << ", frame diff: " << labs((long int)cur_frame - (long int)pos.frame)
                         << "cur utick:" << cur_utick << ",seek to utick: " << utick << ", tick diff: " << abs(
                    utick - cur_utick);
            }
            seq->seekRT(utick);
        }
    ...
    if (!preferences.getBool(PREF_IO_JACK_TIMEBASEMASTER) && (pos.valid & JackPositionBBT)) {
        ...
        if (int(pos.beats_per_minute) != int(60 * seq->curTempo() * seq->score()->tempomap()->relTempo())) {
            if (MScore::debugMode) {
                qDebug() << "JACK Transport tempo changed! JACK bpm: " << (int)pos.beats_per_minute
                         << ", current bpm: " << int(60 * seq->curTempo() * seq->score()->tempomap()->relTempo());
            }
...
{
    if (MScore::debugMode) {
        qDebug() << "jack locate to utick: " << utick << ", frame: "
                 << int(seq->score()->utick2utime(utick) * MScore::sampleRate);
    }
...
void JackAudio::restoreAudioConnections()
{
    ...
    int n
        = settings.value(QString("audio-0-connections"),
                         0).toInt() + settings.value(QString("audio-1-connections"), 0).toInt();
