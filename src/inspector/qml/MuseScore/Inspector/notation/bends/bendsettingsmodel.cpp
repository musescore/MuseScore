...
void BendSettingsModel::loadBendCurve()
{
    ...
    } else if (bend->bendType() == GuitarBendType::DIP) {
        m_bendCurve = {
            CurvePoint(0, 0, true),
            CurvePoint(starTime, endPitch, { CurvePoint::MoveDirection::Both }, false, QString(), false),
            CurvePoint(endTime, 0, { CurvePoint::MoveDirection::Horizontal }),
...
void BendSettingsModel::setBendCurve(const QVariantList& newBendCurve)
{
    ...
    int pitchPointIdx = bend->bendType() == GuitarBendType::DIP ? START_POINT_INDEX : END_POINT_INDEX;
    int newPitch = points.at(pitchPointIdx).pitch;
    bool pitchChanged = newPitch != m_bendCurve.at(pitchPointIdx).pitch;

    beginCommand(muse::TranslatableString("undoableAction", "Edit bend curve"));

    if (pitchChanged) {
        bend->changeBendAmount(curvePitchToBendAmount(newPitch));
    }
