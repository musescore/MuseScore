...
QList<EngravingItem*> ElementRepositoryService::findPlayCountText() const
{
    QList<mu::engraving::EngravingItem*> resultList;

    for (mu::engraving::EngravingItem* element : m_exposedElementList) {
        if (element->isPlayCountText()) {
            resultList << element;
        } else if (element->isBarLine()) {
            const mu::engraving::BarLine* bl = toBarLine(element);
            if (!bl->playCountText()) {
                continue;
            }
            resultList << bl->playCountText();
        }
    }
