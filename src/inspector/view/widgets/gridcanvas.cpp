...
void GridCanvas::mousePressEvent(QMouseEvent* event)
{
    ...
    engraving::PitchValue value = pitchValue(frameRect, coord.first, coord.second);

    m_currentPointIndex = this->pointIndex(value);
}
...
void GridCanvas::mouseMoveEvent(QMouseEvent* event)
{
    ...
    if (canMoveHorizontally) {
        bool canMove = false;
        bool moveToLeft = m_points[index].time > value.time;
        if (moveToLeft) {
            canMove = (index - 1 >= 0) && m_points[index - 1].time < value.time;
        } else {
            canMove = (index + 1 < m_points.size()) && m_points[index + 1].time > value.time;
        }

        if (canMove) {
            m_points[index].time = value.time;
        }
    }
...
void GridCanvas::mouseReleaseEvent(QMouseEvent*)
{
    m_currentPointIndex = std::nullopt;
}

void GridCanvas::hoverEnterEvent(QHoverEvent*)
{
    m_hoverPointIndex = std::nullopt;
}

void GridCanvas::hoverMoveEvent(QHoverEvent* event)
{
    ...
    m_hoverPointIndex = this->pointIndex(value);
...
void GridCanvas::hoverLeaveEvent(QHoverEvent*)
{
    m_hoverPointIndex = std::nullopt;
}

QRectF GridCanvas::frameRect() const
{
...
qreal GridCanvas::columnWidth(const QRectF& frameRect) const
{
    return frameRect.width() / (m_columns - 1);
}

qreal GridCanvas::rowHeight(const QRectF& frameRect) const
{
    return frameRect.height() / (m_rows - 1);
}

std::pair<int, int> GridCanvas::frameCoord(const QRectF& frameRect, int x, int y) const
{
...
void GridCanvas::drawBackground(QPainter* painter, const QRectF& frameRect)
{
...
void GridCanvas::drawCurve(QPainter* painter, const QRectF& frameRect)
{
    const qreal rowHeight = this->rowHeight(frameRect);
    const ThemeInfo& currentTheme = uiConfig()->currentTheme();
    QColor backgroundColor(currentTheme.values[BACKGROUND_PRIMARY_COLOR].toString());
    ...
    auto getPosition = [this, rowHeight, &frameRect](const CurvePoint& v) -> QPointF {
        const qreal x = round(qreal(v.time) * (frameRect.width() / PitchValue::MAX_TIME)) + frameRect.left();
        qreal y = 0;
...
std::optional<int> GridCanvas::pointIndex(const engraving::PitchValue& pitchValue, bool movable) const
{
    ...
    for (int i = 0; i < numberOfPoints; ++i) {
        ...
        if (std::pow((pitchValue.time - point.time), 2)
            + std::pow((pitchValue.pitch - point.pitch), 2) < std::pow(GRIP_CENTER_RADIUS, 2)) {
            return i;
        }
    }
...
PitchValue GridCanvas::pitchValue(const QRectF& frameRect, int frameX, int frameY) const
{
    PitchValue value;
    value.time = qreal(frameX) / (frameRect.width() / PitchValue::MAX_TIME);
    ...
    value.pitch = row * 100 / m_primaryRowsInterval;
