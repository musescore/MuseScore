...
QString EditStyle::pageCodeForElement(const EngravingItem* element)
{
    ...
    case ElementType::TEXTLINE_SEGMENT: {
        ...
        if (element->isTextLineSegment()) {
            systemFlag = toTextLineSegment(element)->systemFlag();
        } else {
            systemFlag = toTextLine(element)->systemFlag();
        }
        return systemFlag ? "system-text-line" : "text-line";
    }
