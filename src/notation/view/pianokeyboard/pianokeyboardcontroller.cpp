...
void PianoKeyboardController::init()
{
    ...
    playback()->player()->playingNotesRevoked().onReceive(this, [this](audio::TrackId trackId) {
        removePlayedTrackKeys(trackId);
    });

    playback()->audioOutput()->outputParamsChanged().onReceive(this, [this](audio::TrackSequenceId, audio::TrackId trackId,
...
void PianoKeyboardController::onNoteMidiEventReceived(audio::TrackId trackId, const Event& event)
{
    ...
    midi::Event::Opcode opcode = event.opcode();
    uint8_t note = event.note();

    auto handleEvent = [this, notationPlayback, trackId, instrumentTrackId, opcode, note](audio::AudioOutputParams params) {
        if (params.muted) {
            return;
        }
...
void PianoKeyboardController::addPlayedKey(audio::TrackId trackId, std::optional<piano_key_t> key)
{
...
void PianoKeyboardController::removePlayedKey(audio::TrackId trackId, std::optional<piano_key_t> key)
{
...
void PianoKeyboardController::removePlayedTrackKeys(audio::TrackId trackId)
{
    bool changed = false;
...
mu::audio::TrackId PianoKeyboardController::instrumentTrackIdToTrackId(InstrumentTrackId trackId) const
{
    return playbackController()->instrumentTrackIdMap().at(trackId);
}

InstrumentTrackId PianoKeyboardController::trackIdToInstrumentTrackId(audio::TrackId trackId) const
{
