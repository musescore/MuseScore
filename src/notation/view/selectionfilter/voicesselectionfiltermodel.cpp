...
bool VoicesSelectionFilterModel::isIndeterminate(const SelectionFilterTypesVariant& variant) const
{
    const VoicesSelectionFilterTypes type = std::get<VoicesSelectionFilterTypes>(variant);

    if (type == VoicesSelectionFilterTypes::ALL) {
        return !isFiltered(VoicesSelectionFilterTypes::ALL) && !isFiltered(VoicesSelectionFilterTypes::NONE);
    }

    return false;
}

void VoicesSelectionFilterModel::notifyAboutDataChanged(const QModelIndex& index, const SelectionFilterTypesVariant& variant)
{
    const VoicesSelectionFilterTypes type = std::get<VoicesSelectionFilterTypes>(variant);
