...
bool ElementsSelectionFilterModel::isIndeterminate(const SelectionFilterTypesVariant& variant) const
{
    const ElementsSelectionFilterTypes type = std::get<ElementsSelectionFilterTypes>(variant);

    if (type == ElementsSelectionFilterTypes::ALL) {
        return !isFiltered(ElementsSelectionFilterTypes::ALL) && !isFiltered(ElementsSelectionFilterTypes::NONE);
    }

    return false;
}

void ElementsSelectionFilterModel::notifyAboutDataChanged(const QModelIndex& index, const SelectionFilterTypesVariant& variant)
{
    const ElementsSelectionFilterTypes type = std::get<ElementsSelectionFilterTypes>(variant);
