...
static mu::playback::IPlaybackController::PlayParams makeNoteOnParams(bool infiniteDuration)
{
    ...
    if (infiniteDuration) {
        params.duration = muse::mpe::INFINITE_DURATON; // play note on only
    }
...
void NotationMidiInput::doProcessEvents()
{
    ...
    const bool playDurationAndVelocity = isSoundPreview || configuration()->playMidiNotesWithVelocityAndDurationDuringNoteInput();
    ...
    for (size_t i = 0; i < m_eventsQueue.size(); ++i) {
        ...
        if (playDurationAndVelocity && noteOff) {
    ...
    if (!notesOn.empty()) {
        ...
        playbackController()->playElements(elements, makeNoteOnParams(playDurationAndVelocity), true);
...
void NotationMidiInput::addNoteEventsToInputState()
{
    ...
    const bool playWithVelocityAndDuration = playPreviewNotes && configuration()->playMidiNotesWithVelocityAndDurationDuringNoteInput();
    ...
    for (const muse::midi::Event& event : m_eventsQueue) {
        ...
        if (opcode == muse::midi::Event::Opcode::NoteOn) {
            ...
            if (playWithVelocityAndDuration) {
    ...
    if (!notesOn.empty()) {
        ...
        if (playPreviewNotes) {
            playbackController()->playNotes(notesOn, staffIdx, state.segment(), makeNoteOnParams(playWithVelocityAndDuration));
        }
    }
}
...
void NotationMidiInput::triggerControllers(const ControllerEventMap& events)
{
    ...
    for (const auto& pair : events) {
        ...
        if (cc.type != muse::mpe::ControllerChangeEvent::Undefined) {
            cc.layerIdx = muse::mpe::makeLayerIdx(static_cast<muse::mpe::staff_layer_idx_t>(is.staffIdx()),
                                                  static_cast<muse::mpe::voice_layer_idx_t>(is.voice()));
