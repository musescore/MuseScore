...
TranslatableString name(NoteInputMethod method)
{
    switch (method) {
...
void NotationNoteInput::startNoteInput(NoteInputMethod method, bool focusNotation)
{
    ...
    accessibilityController.get()->announce(name(method).translated());
}
...
void NotationNoteInput::endNoteInput(bool resetState)
{
    ...
    updateInputState();
    accessibilityController.get()->announce(TranslatableString("noteInputMethod", "Normal mode").translated());
}
