...
inline SystemObjectGroupsByStaff collectSystemObjectGroups(const std::vector<mu::engraving::Staff*>& staves)
{
    ...
    std::vector<engraving::EngravingItem*> systemObjects = engraving::collectSystemObjects(staves.front()->score(), staves);
...
{
    std::vector<mu::engraving::Staff*> staves {
...
inline bool hasSystemObjectStaff(const mu::engraving::Part* part)
{
    for (mu::engraving::Staff* staff : part->staves()) {
        if (part->score()->isSystemObjectStaff(staff)) {
            return true;
        }
    }
