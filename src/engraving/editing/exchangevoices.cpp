...
void ExchangeVoicesInMeasure::doExchange(track_idx_t srcTrack, track_idx_t dstTrack)
{
    ...
    for (auto i = spanners.begin(); i < spanners.end(); i++) {
        ...
        LOGD("Start %d End %d Diff %d \n Measure Start %d End %d", spStart.ticks(),
             spEnd.ticks(), (spEnd - spStart).ticks(),
...
void ExchangeVoices::exchangeVoices(Score* score, Measure* measure, voice_idx_t srcVoice, voice_idx_t dstVoice,
                                    staff_idx_t srcStaff,
        ...
        for (Staff* st : score->staff(staffIdx)->staffList()) {
            ...
            if (ex) {
                ...
                for (track_idx_t srcTrack2 : srcTrackList) {
                    ...
                    if (!hasVoice) {
                        CloneVoice::cloneVoice(measure->first(), measure2->endTick(),
                                               measure2->first(), tempTrack, srcTrack2, false, true);
                ...
                for (track_idx_t dstTrack2 : dstTrackList) {
                    ...
                    if (!hasVoice) {
                        CloneVoice::cloneVoice(measure->first(), measure2->endTick(),
                                               measure2->first(), tempTrack, dstTrack2, false, true);
                ...
                score->undo(new ExchangeVoicesInMeasure(measure2, staffTrack + srcVoice, staffTrack + dstVoice,
                                                        st->idx()));
            }
        }
    }
