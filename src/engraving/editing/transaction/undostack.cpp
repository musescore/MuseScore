...
void UndoableTransaction::fillSelectionInfo(SelectionInfo& info, const Selection& sel)
{
    ...
    info.startHBox = info.endHBox = nullptr;
    info.elements.clear();
    ...
    if (sel.isList()) {
        ...
        info.startHBox = sel.startHBox();
        info.endHBox = sel.endHBox();
    }
}
...
void UndoableTransaction::applySelectionInfo(const SelectionInfo& info, Selection& sel)
{
    if (!info.elements.empty()) {
        ...
        sel.setRangeTicks(info.tickStart, info.tickEnd, info.staffStart, info.staffEnd, info.startHBox, info.endHBox);
    }
}
