...
{
    return _text->ldata()->textBlock(static_cast<int>(_row)).columns();
}
...
{
    TextBase::LayoutData* ldata = _text->mutldata();
...
bool TextCursor::set(const PointF& p, TextCursor::MoveMode mode)
{
    ...
    if (!_text->ldata()->bbox().contains(pt)) {
        return false;
    }
    size_t oldRow    = _row;
