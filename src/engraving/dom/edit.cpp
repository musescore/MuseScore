...
std::vector<ChordRest*> Score::deleteRangeAtTrack(const track_idx_t track, Segment* startSeg, const Fraction& endTick,
                                                  Tuplet* currentTuplet, std::unordered_set<Tuplet*>& handledTuplets)
{
    ...
    const auto foundDeselected = [&](const DurationElement* deselectedElement) {
        ...
        crsForSelection.insert(crsForSelection.end(), rests.begin(), rests.end());
    ...
    for (Segment* s = startSeg; s && (s->tick() < endTick); s = s->next1()) {
        ...
        if (nextTuplet) {
            //! NOTE: handledTuplets is a slight hack, ideally we'd just skip to the first segment after nextTuplet (tick2segment wasn't reliable for this purpose)
            if (handledTuplets.find(nextTuplet) != handledTuplets.end()) {
                continue;
            }
            handledTuplets.emplace(nextTuplet);

            if (nextTuplet->selected()) {
                restDuration += nextTuplet->ticks();
                cmdDeleteTuplet(nextTuplet, /*replaceWithRest*/ false);
            } else {
                const std::vector<ChordRest*> crs = deleteRangeAtTrack(track, cr1->segment(), nextTuplet->tick() + nextTuplet->ticks(),
                                                                       nextTuplet, handledTuplets);
                crsForSelection.insert(crsForSelection.end(), crs.begin(), crs.end());
...
std::vector<ChordRest*> Score::deleteRange(Segment* s1, Segment* s2, track_idx_t track1, track_idx_t track2, const SelectionFilter& filter)
{
    ...
    for (track_idx_t track = track1; track < track2; ++track) {
        ...
        std::unordered_set<Tuplet*> handledTuplets;
        const std::vector<ChordRest*> forSelection = deleteRangeAtTrack(track, s1, endTick, /*currentTuplet*/ nullptr, handledTuplets);
        crs.insert(crs.end(), forSelection.begin(), forSelection.end());
    }
