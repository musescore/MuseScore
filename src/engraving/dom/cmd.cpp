...
void Score::addInterval(int val, const std::vector<Note*>& nl)
{
    ...
    for (Note* n : _nl) {
        if (muse::contains(tmpnl, n)) {
            continue;
        }
        tmpnl.push_back(n);
        if (n->tieFor() && n->tieFor()->endNote() && !muse::contains(tmpnl, n->tieFor()->endNote())) {
    ...
    for (Note* on : tmpnl) {
        ...
        if (on->staff() && on->staff()->isDrumStaff(tick)) {
            ...
            nval.pitch = ds->defaultPitchForLine(on->line() - valTmp);
            nval.headGroup = ds->noteHead(nval.pitch);
            if (!ds->isValid(nval.pitch) || nval.headGroup == NoteHeadGroup::HEAD_INVALID) {
            ...
            if (std::abs(valTmp) != 7 || accidental) {
                ...
                if (v.isZero()) {
                    ...
                    if (style().styleB(Sid::concertPitch)) {
                        v.flip();
                        nval.tpc1 = ntpc;
                        nval.tpc2 = mu::engraving::transposeTpc(ntpc, v, true);
                    } else {
                        nval.pitch += v.chromatic;
                        nval.tpc2 = ntpc;
                        nval.tpc1 = mu::engraving::transposeTpc(ntpc, v, true);
                    }
                }
            ...
            if (nval.pitch < 0 || nval.pitch > 127) {
                notesToSelect.push_back(toEngravingItem(on));
                continue;
            }
        }
