...
SystemLockIndicator::SystemLockIndicator(System* parent)
    : EngravingItem(ElementType::SYSTEM_LOCK_INDICATOR, parent, ElementFlag::SYSTEM | ElementFlag::GENERATED) {}
...
const SystemLock* SystemLockIndicator::systemLock() const
{
    const SystemLock* lock = system()->first()->systemLock();
    assert(lock);
    return lock;
}
