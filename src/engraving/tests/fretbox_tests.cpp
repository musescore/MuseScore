...
TEST_F(Engraving_FretBoxTests, AddChords)
{
    ...
    insertChord(score, u"Dd", 5);

    // -------- Chords in the score: A, B, Dd, C, Dd, D, E, F --------

    // [THEN] Check fret box and chords
    //        The second "Dd" should be ignored
    checkFretBox(score, { u"A", u"B", u"Dd", u"C", u"D", u"E", u"F" });

    // [WHEN] Add the same chord before the first one
    insertChord(score, u"Dd", 1);

    // -------- Chords in the score: A, Dd, B, Dd, C, D, E, F --------

    // [THEN] Check fret box and chords
    //        The "Dd" should be moved to the first position
    checkFretBox(score, { u"A", u"Dd", u"B", u"C", u"D", u"E", u"F" });
...
TEST_F(Engraving_FretBoxTests, RemoveChords)
{
    ...
    insertChord(score, u"B", 2); // Reinsert "B"
    ...
    // [GIVEN] There is again a few same chords in the score
    insertChord(score, u"B", 2); // Reinsert first "B"

    // [WHEN] Remove the second "B" chord
    removeChord(score, 5); // Remove the second "B"
