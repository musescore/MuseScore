...
TEST_F(Engraving_PlaybackEventsRendererTests, Glissando_on_tied_notes)
{
    ...
    mpe::NoteEvent noteEvent0 = std::get<mpe::NoteEvent>(result.begin()->second.at(0));
    EXPECT_EQ(noteEvent0.expressionCtx().articulations.size(), 1);
    EXPECT_TRUE(noteEvent0.expressionCtx().articulations.contains(ArticulationType::Standard));
    EXPECT_EQ(noteEvent0.pitchCtx().nominalPitchLevel, nominalPitchLevel);
    EXPECT_EQ(noteEvent0.arrangementCtx().nominalDuration, QUARTER_NOTE_DURATION + glissandoNoteDuration); // A4 + 1st glissando note
...
TEST_F(Engraving_PlaybackEventsRendererTests, Single_Note_Tremolo_OnTiedNote)
{
    ...
    EXPECT_EQ(noteEvent0.expressionCtx().articulations.size(), 1);
    EXPECT_TRUE(noteEvent0.expressionCtx().articulations.contains(ArticulationType::Standard));
    EXPECT_EQ(noteEvent0.pitchCtx().nominalPitchLevel, pitchLevel(PitchClass::A, 4));
    EXPECT_EQ(noteEvent0.arrangementCtx().nominalDuration, HALF_NOTE_DURATION);
