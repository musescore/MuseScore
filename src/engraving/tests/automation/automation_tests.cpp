...
TEST_F(Automation_Tests, MoveTicks_AcrossMultipleCurves)
{
    ...
    expected2[700] = p2;
    checkCurvesMatch(data.curve(key1), expected1);
    checkCurvesMatch(data.curve(key2), expected2);
}
...
TEST_F(Automation_Tests, Notify_MoveTicks_OpenEndedRange)
{
    ...
    AutomationChanges lastChanges;
    data.changed().onReceive(this, [&lastChanges](const AutomationChanges& ch) {
        lastChanges = ch;
    });

    // [WHEN] Ticks shifted from 100 by +50
    data.moveTicks(100, 50);
