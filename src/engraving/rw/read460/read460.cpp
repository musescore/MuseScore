...
void Read460::preparePasteDurationElement(Score* score, const Fraction& tick, const Fraction& ticks, const track_idx_t track)
{
        ...
        while (e.readNextStartElement()) {
            ...
            while (e.readNextStartElement()) {
                ...
                if (tag == "transposeChromatic") {
                    ...
                    if (!tuplet->tuplet()) {
                        preparePasteDurationElement(score, tick, tuplet->actualTicksAt(tick), tuplet->track());
                    }
                    if (oldTuplet) {
                    ...
                    if (cr->isGrace()) {
                        ...
                        if (!cr->tuplet()) {
                            preparePasteDurationElement(score, tick, cr->actualTicksAt(tick), cr->track());
                        }
