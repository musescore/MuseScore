...
Ret Read206::readScoreFile(Score* score, XmlReader& e, ReadInOutData* out)
{
    ...
    for (Segment* s = score->firstSegment(st); s; s = s->next1(st)) {
        ...
        for (staff_idx_t staffIdx = 0; staffIdx < numStaves; ++staffIdx) {
            const size_t maxSpan = numStaves - staffIdx;
            const track_idx_t trackIdx = staff2track(staffIdx);
            BarLine* barLine = toBarLine(s->element(trackIdx));
            if (barLine) {
                const std::optional<size_t> span = ctx.getBarLineSpan(barLine);
                if (span) {
                    if (*span > maxSpan) {
                    ...
                    barLineSpan = std::max(barLineSpan, std::min(*span, maxSpan));
                }
            }

            if (barLineSpan == 0) {
                if (barLine) {
                    barLine->setSpanStaff(false);
                }
                continue;
            }
            ...
            if (!barLine) {
                // clone previous bar line. This is safe because we always have a previous barline
                // when barLineSpan > 0 because it's either the one that was read or the one created in the previous iteration
