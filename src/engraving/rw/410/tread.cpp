...
PropertyValue TRead::readPropertyValue(Pid id, XmlReader& e)
{
    ...
    case P_TYPE::GROUPS: {
        Groups g;
        rw410::TRead::read(&g, e, *e.context());
...
void TRead::read(ChordLine* l, XmlReader& e, ReadContext& ctx)
{
    ...
    while (e.readNextStartElement()) {
        ...
        } else if (tag == "offset" && l->score()->mscVersion() < 410) { // default positions has changed in 4.0 so ignore previous offset
            e.skipCurrentElement();
...
void TRead::read(SlurTieSegment* s, XmlReader& e, ReadContext& ctx)
{
    ...
    while (e.readNextStartElement()) {
        const AsciiStringView tag(e.name());
        if (s->score()->mscVersion() < 410 && (tag == "o1" || tag == "o2" || tag == "o3" || tag == "o4")) {
...
bool TRead::readProperties(Staff* s, XmlReader& e, ReadContext& ctx)
{
    ...
    if (tag == "StaffType") {
        ...
        rw410::TRead::read(s->keyList(), e, ctx);
    } else if (tag == "bracket") {
