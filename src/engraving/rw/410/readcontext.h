...
public:
    ...
    void addConnectorInfoLater(std::unique_ptr<rw410::ConnectorInfoReader> c) { _pendingConnectors.push_back(std::move(c)); }   // add connector info to be checked after calling checkConnectors()
...
private:
    ...
    void addConnectorInfo(std::unique_ptr<rw410::ConnectorInfoReader>);
    void removeConnector(const rw410::ConnectorInfoReader*);   // Removes the whole ConnectorInfo chain from the connectors list.
    ...
    std::vector<std::unique_ptr<rw410::ConnectorInfoReader> > _connectors;
    std::vector<std::unique_ptr<rw410::ConnectorInfoReader> > _pendingConnectors;  // connectors that are pending to be updated and added to _connectors. That will happen when checkConnectors() is called.
