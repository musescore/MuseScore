...
using namespace mu::engraving::rw410;
...
//                        chord->readAddTuplet(tuplet);
        ...
        //----------------------------------------------------
            ...
            rw410::TRead::read(segment, e, ctx);
        } else if (tag == "Ambitus") {
...
void MeasureRW::writeMeasure(const Measure* measure, XmlWriter& xml, staff_idx_t staff, bool writeSystemElements, bool forceTimeSig)
{
    ...
    if (writeSystemElements) {
        ...
        rw410::TWrite::writeProperty(measure, xml, Pid::IRREGULAR);
        rw410::TWrite::writeProperty(measure, xml, Pid::BREAK_MMR);
        rw410::TWrite::writeProperty(measure, xml, Pid::USER_STRETCH);
        rw410::TWrite::writeProperty(measure, xml, Pid::NO_OFFSET);
        rw410::TWrite::writeProperty(measure, xml, Pid::MEASURE_NUMBER_MODE);
    }
    double _spatium = measure->spatium();
    MStaff* mstaff = measure->m_mstaves[staff];
    if (mstaff->noText() && !mstaff->noText()->generated()) {
        rw410::TWrite::write(mstaff->noText(), xml, ctx);
    }

    if (mstaff->mmRangeText() && !mstaff->mmRangeText()->generated()) {
        rw410::TWrite::write(mstaff->mmRangeText(), xml, ctx);
    }
    ...
    for (const EngravingItem* e : measure->el()) {
        ...
        rw410::TWrite::writeItem(e, xml, ctx);
    }
    assert(measure->first());
    assert(measure->last());
    if (measure->first() && measure->last()) {
        rw410::TWrite::writeSegments(xml, ctx, strack, etrack, measure->first(), measure->last()->next1(), writeSystemElements,
                                     forceTimeSig);
    }
