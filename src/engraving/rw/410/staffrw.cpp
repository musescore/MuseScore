...
static void writeMeasure(XmlWriter& xml, MeasureBase* m, staff_idx_t staffIdx, bool writeSystemElements, bool forceTimeSig)
{
    ...
    if (m->isMeasure() || staffIdx == 0) {
        if (Measure::classof(m)) {
            rw410::MeasureRW::writeMeasure(static_cast<const Measure*>(m), xml, staffIdx, writeSystemElements, forceTimeSig);
        } else {
            rw410::TWrite::writeItem(m, xml, *xml.context());
        }
    }

    if (m->score()->styleB(Sid::createMultiMeasureRests) && m->isMeasure() && toMeasure(m)->mmRest()) {
        rw410::MeasureRW::writeMeasure(toMeasure(m)->mmRest(), xml, staffIdx, writeSystemElements, forceTimeSig);
    }
