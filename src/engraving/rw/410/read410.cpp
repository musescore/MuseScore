...
Err Read410::read(Score* score, XmlReader& e, ReadInOutData* data)
{
...
bool Read410::readScore410(Score* score, XmlReader& e, ReadContext& ctx)
{
    ...
    while (e.readNextStartElement()) {
        ...
        if (tag == "Staff") {
            ...
            rw410::TRead::read(score->_audio, e, ctx);
        } else if (tag == "showOmr") {
            e.skipCurrentElement();
            ...
            // Since version 410, the style is stored in a separate file
            e.skipCurrentElement();
            ...
            rw410::TRead::readItem(s, e, ctx);
            score->addSpanner(s);
        } else if (tag == "Excerpt") {
            // Since version 410, the Excerpts are stored in a separate file
            e.skipCurrentElement();
            ...
            // Since version 410, the Excerpts is stored in a separate file
            e.skipCurrentElement();
        } else if (tag == "name") {
            String n = e.readText();
