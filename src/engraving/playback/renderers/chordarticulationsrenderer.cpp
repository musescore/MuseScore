                                          ...
                                          mpe::PlaybackEventList& result)
{
    ...
    for (const auto& type : ctx.commonArticulations) {
        if (muse::contains(GRACE_NOTE_ARTICULATION_TYPES, type.first)) {
            renderGraceChord(toChord(item), type.first, ctx, result);
            return;
        }
    }
...
void ChordArticulationsRenderer::renderGraceChord(const Chord* chord, const muse::mpe::ArticulationType type, const RenderingContext& ctx,
