                                             ...
                                             mpe::PlaybackEventsMap& result) const
{
    ...
    for (int tick = measureStartTick; tick < measureEndTick; tick += step) {
        ...
        result[eventTimestamp].emplace_back(buildMetronomeEvent(timeSignatureFraction, bps.val, beatType, eventTimestamp, profile));
    }
}
                                             ...
                                             const muse::mpe::ArticulationsProfilePtr profile, mpe::PlaybackEventsMap& result) const
{
    ...
    result[actualTimestamp].emplace_back(buildMetronomeEvent(timeSignatureFraction, bps.val, beatType, actualTimestamp, profile));
}
                                           ...
                                           muse::mpe::PlaybackEventsMap& result, muse::mpe::duration_t& totalCountInDuration) const
{
    ...
    // Add extra clicks if...
    int endTick = timeSignatureFraction.ticksPerMeasure() + (startTick - measureStartTick); // ... not starting playback at beginning of measure
    if (measure->isAnacrusis()) {  // ... measure is incomplete (anacrusis)
        endTick += (ticksPerMeasure - measure->ticks().ticks());
    }

    timestamp_t prevEventEndTimestamp = 0;
    ...
    for (int tick = 0; tick < endTick; tick += step) {
        ...
        timestamp_t eventTimestamp = actualTimestamp + prevEventEndTimestamp;

        mpe::NoteEvent event = buildMetronomeEvent(timeSignatureFraction, bps.val, beatType, eventTimestamp, profile);
        totalCountInDuration += event.arrangementCtx().nominalDuration;
        prevEventEndTimestamp = event.arrangementCtx().nominalTimestamp + event.arrangementCtx().nominalDuration;
