...
static void layoutAccidental(const Accidental* item, const LayoutContext& ctx, Accidental::LayoutData* ldata)
{
    ...
    if (singleSym != SymId::noSym && ctx.engravingFont()->isValid(singleSym)) {
        ...
        ldata->bbox.unite(item->symBbox(singleSym));
    }
    // Multi
    else {
        ...
        if (bracketSyms.first != SymId::noSym) {
            ...
            ldata->bbox.unite(item->symBbox(bracketSyms.first));
        ...
        ldata->bbox.unite(item->symBbox(mainSym).translated(x, 0.0));
        ...
        if (bracketSyms.second != SymId::noSym) {
            ...
            ldata->bbox.unite(item->symBbox(bracketSyms.second).translated(x, 0.0));
        }
    }
}
...
void TLayout::layout(ActionIcon* item, LayoutContext&)
{
    ...
    ldata->bbox = fontMetrics.boundingRect(Char(item->icon()));
}
...
void TLayout::layout(Ambitus* item, LayoutContext& ctx)
{
    ...
    //
    {
        ...
        if (item->topPitch() == INVALID_PITCH || item->topTpc() == Tpc::TPC_INVALID) {
            ...
            topAccData->pos.setY(ldata->topPos.y());
        }
        ...
        if (item->bottomPitch() == INVALID_PITCH || item->bottomTpc() == Tpc::TPC_INVALID) {
            ...
            bottomAccData->pos.setY(ldata->bottomPos.y());
        }
    }
    ...
    // BBOX
    {
        ...
        ldata->bbox = headRect.translated(ldata->topPos).united(headRect.translated(ldata->bottomPos))
...
static void layoutArticulation(const Articulation* item, const LayoutContext&, Articulation::LayoutData* ldata)
{
    ...
    ldata->bbox = bRect.translated(-0.5 * bRect.width(), 0.0);
}
...
void TLayout::layout2(BarLine* item, LayoutContext& ctx)
{
    ...
    item->calcY();
    ldata->bbox.setTop(ldata->y1);
    ldata->bbox.setBottom(ldata->y2);

    if (ctx.conf().styleB(Sid::repeatBarTips)) {
        switch (item->barLineType()) {
        case BarLineType::START_REPEAT:
            ldata->bbox.unite(item->symBbox(SymId::bracketTop).translated(0, ldata->y1));
            ldata->bbox.unite(item->symBbox(SymId::bracketBottom).translated(0, ldata->y2));
            break;
        case BarLineType::END_REPEAT:
        {
            double w1 = 0.0;               //symBbox(SymId::reversedBracketTop).width();
            ldata->bbox.unite(item->symBbox(SymId::reversedBracketTop).translated(-w1, ldata->y1));
            ldata->bbox.unite(item->symBbox(SymId::reversedBracketBottom).translated(-w1, ldata->y2));
            break;
        }
        case BarLineType::END_START_REPEAT:
        {
...
void TLayout::layout(HBox* item, LayoutContext& ctx)
{
    ...
    if (item->explicitParent() && item->explicitParent()->isVBox()) {
        ...
        ldata->setPos(x, y);
        ldata->bbox.setRect(0.0, 0.0, w, h);
    } else if (item->system()) {
        ldata->bbox.setRect(0.0, 0.0, item->point(item->boxWidth()), item->system()->height());
    } else {
        ldata->bbox.setRect(0.0, 0.0, 50, 50);
    }
...
void TLayout::layout(VBox* item, LayoutContext& ctx)
{
    ...
    if (item->system()) {
        ldata->bbox.setRect(0.0, 0.0, item->system()->width(), item->point(item->boxHeight()));
    } else {
        ldata->bbox.setRect(0.0, 0.0, 50, 50);
    }
...
void TLayout::adjustLayoutWithoutImages(VBox* item, LayoutContext& ctx)
{
    ...
    for (auto pElement : elementList) {
        if (pElement->isText()) {
            ...
            txt->mutLayoutData()->bbox.moveTop(0);
...
void TLayout::layout(TBox* item, LayoutContext& ctx)
{
    FBox::LayoutData* ldata = item->mutLayoutData();
    ldata->setPos(PointF());        // !?
    ldata->bbox.setRect(0.0, 0.0, item->system()->width(), 0);
    ...
    ldata->bbox.setRect(0.0, 0.0, item->system()->width(), h);
...
static void layoutBreath(const Breath* item, const LayoutContext& ctx, Breath::LayoutData* ldata)
{
    ...
    if (item->isCaesura()) {
        ldata->pos = PointF(ldata->pos.x(), item->spatium() + voiceOffset);
    } else if ((ctx.conf().styleSt(Sid::MusicalSymbolFont) == "Emmentaler")
               && (item->symId() == SymId::breathMarkComma)) {
        ldata->pos = PointF(ldata->pos.x(), 0.5 * item->spatium() + voiceOffset);
    } else {
        ldata->pos = PointF(ldata->pos.x(), -0.5 * item->spatium() + voiceOffset);
    }

    ldata->bbox = item->symBbox(item->symId());
}
...
static void layoutFermata(const Fermata* item, const LayoutContext& ctx, Fermata::LayoutData* ldata)
{
    ...
    if (e) {
        if (e->isChord()) {
            ...
            ldata->movePosX(e->x() - e->shape().left() + e->width() * item->staff()->staffMag(Fraction(0, 1)) * .5);
        }
    }
    ...
    if (item->placeAbove()) {
        ...
        ldata->movePosY(item->staff()->height());
    ...
    ldata->bbox = b.translated(-0.5 * b.width(), 0.0);
...
static void layoutFiguredBass(const FiguredBass* item, const LayoutContext& ctx, FiguredBass::LayoutData* ldata)
{
    ...
    if (item->items().size() > 0) {
        ...
        ldata->bbox.setRect(0, 0, ldata->lineLength(0), 0);
...
static void layoutFingering(const Fingering* item, const LayoutContext& ctx, Fingering::LayoutData* ldata)
{
    ...
    if (item->autoplace() && item->note()) {
        ...
        if (item->layoutType() == ElementType::CHORD) {
            ...
            if (n->mirror()) {
                ldata->movePosX(-n->layoutData()->pos.x());
            }
...
static void layoutFretDiagram(const FretDiagram* item, const LayoutContext& ctx, FretDiagram::LayoutData* ldata)
{
    ...
    if (harmony && harmony->autoplace() && harmony->explicitParent()) {
        ...
        if (d > -minDistance) {
            ...
            yd *= -1.0;
            harmony->mutLayoutData()->movePosY(yd);
...
static void layoutNote(const Note* item, const LayoutContext&, Note::LayoutData* ldata)
{
    ...
    if (useTablature) {
        ...
        ldata->bbox.setRect(0, tab->fretBoxY() * mags, w, tab->fretBoxH() * mags);

        if (item->ghost() && Note::engravingConfiguration()->tablatureParenthesesZIndexWorkaround()) {
            ldata->bbox.setWidth(w + item->symWidth(SymId::noteheadParenthesisLeft) + item->symWidth(SymId::noteheadParenthesisRight));
        } else {
            ldata->bbox.setWidth(w);
        }
    } else {
        if (item->deadNote()) {
...
//                  rypos() = 2 * _spatium;
    ...
    double x2 = x1 + w;
    double y  = item->pos().y();
    ldata->bbox.setRect(x1, -item->lw() * .5 + y, w, (_lines - 1) * dist + item->lw());
...
void TLayout::layoutTextLineBaseSegment(TextLineBaseSegment* item, LayoutContext& ctx)
{
    ...
    if (const StaffType* st = item->staffType()) {
        ldata->movePosY(st->yoffset().val() * item->spatium());
    }
    ...
    if (!item->endText()->empty()) {
        item->endText()->mutLayoutData()->movePosX(ldata->bbox.right());
