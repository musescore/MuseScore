                                ...
                                const std::vector<Note*>& notes, const Staff* staff, LayoutContext& ctx)
{
    ...
    for (Chord* chord : chords) {
        ...
        for (Note* note : chordNotes) {
            ...
            double segmentX = noteX + chord->x();     // segment-relative X position of note
            if (note->ldata()->mirror() && !_up && segmentX < -note->headBodyWidth() / 2) {
...
//            shape.add(_tremolo->shape().translated(_tremolo->pos()));
    for (Note* note : item->notes()) {
        ...
        noteShape.remove_if([&](ShapeElement& s) { return !s.item()->addToSkyline() && !s.item()->isNote(); });
        shape.add(noteShape.translate(note->pos()));
    }
