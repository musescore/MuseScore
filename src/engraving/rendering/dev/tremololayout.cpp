...
void TremoloLayout::layoutTwoNotesTremolo(Tremolo* item, LayoutContext& ctx, double x, double y, double h, double spatium)
{
    ...
    TremoloBeamLayout::setupLData(item->mutldata(), item);
    item->setStartAnchor(TremoloBeamLayout::chordBeamAnchor(item, item->ldata(), item->chord1(), ChordBeamAnchorType::Start));
    item->setEndAnchor(TremoloBeamLayout::chordBeamAnchor(item, item->ldata(), item->chord2(), ChordBeamAnchorType::End));
    ...
    ldata->setMag(mag);
    TremoloBeamLayout::calculateAnchors(item, item->mutldata(), chordRests, notes);
...
void TremoloLayout::createBeamSegments(Tremolo* item, LayoutContext& ctx)
{
    ...
    if (item->chord1()->stem() && item->chord2()->stem()
        ...
        for (ChordRest* cr : { item->chord1(), item->chord2() }) {
            ...
            TremoloBeamLayout::extendStem(item, item->ldata(), chord, addition);
        }
    }
}
