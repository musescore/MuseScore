...
void MarkerLayout::layoutMarker(const Marker* item, TextBase::LayoutData* ldata)
{
    doLayoutMarker(item, ldata);

    double customTextOffset = computeCustomTextOffset(item, ldata);
...
void MarkerLayout::doLayoutMarker(const Marker* item, TextBase::LayoutData* ldata)
{
    Measure* measure = item->parentItem() ? toMeasure(item->parentItem()) : nullptr;
    ...
    // POSITION
    double xAdj = item->isRightMarker() && measure ? measure->width() : 0.0;

    Measure* blMeasure = item->isRightMarker() ? measure : measure->prevMeasure();
    const BarLine* bl = blMeasure ? blMeasure->endBarLine() : nullptr;
    double blWidth = bl ? bl->width() : 0.0;
...
double MarkerLayout::computeCustomTextOffset(const Marker* item, TextBase::LayoutData* ldata)
{
    ...
    doLayoutMarker(&referenceMarker, referenceMarker.mutldata());
