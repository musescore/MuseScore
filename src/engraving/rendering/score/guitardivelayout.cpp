...
void GuitarDiveLayout::updateDiveSequences(const std::vector<GuitarBend*>& bends, const LayoutContext& ctx)
{
    ...
    for (GuitarBend* bend : bends) {
        if ((bend->bendType() == GuitarBendType::DIVE || bend->bendType() == GuitarBendType::PRE_DIVE)
            && bend->staffType()->isTabStaff()) {
    ...
    for (auto riter = dives.rbegin(); riter != dives.rend(); ++riter) {
        ...
        bool layoutAboveStaff = tabPos == DirectionV::UP
                                || (tabPos == DirectionV::AUTO && ctx.conf().styleB(Sid::guitarDivesAboveStaff));
...
void GuitarDiveLayout::layoutScoop(GuitarBendSegment* item)
{
    ...
    double xMove
        = -HorizontalSpacing::minHorizontalDistance(Shape(bbox.translated(0.0, startNote->y())), chordShape, spatium) - leftPad;
