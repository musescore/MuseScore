                                    ...
                                    Transform& toSystemCoordinates, double& slurAngle)
{
    ...
    const double arcClearance = -upSign* computeArcClearance(spatium, slurLength);  // Collision clearance at the center of the slur
...
void SlurTieLayout::addMinClearanceToShapes(Shape& segShapes, double spatium, bool slurUp, const ChordRest* startCR, const ChordRest* endCR)
{
    ...
    for (ShapeElement& shapeEl : segShapes.elements()) {
        ...
        if (item->isNote()) {
            slurUp ? shapeEl.adjust(0.0, -noteClearance, 0.0, 0.0) : shapeEl.adjust(0.0, 0.0, 0.0, noteClearance);
        } else {
            slurUp ? shapeEl.adjust(0.0, -itemClearance, 0.0, 0.0) : shapeEl.adjust(0.0, 0.0, 0.0, itemClearance);
        }
    }
}

double SlurTieLayout::computeArcClearance(double spatium, double slurLength)
{
    static constexpr double CLEARANCE_TO_LENGTH_RATIO = 0.04;
    static constexpr double MAX_CLEARANCE = 0.5;

    double clearance = CLEARANCE_TO_LENGTH_RATIO * slurLength;
    clearance = std::min(clearance, MAX_CLEARANCE);
...
{
    double step = upSign * 0.25 * spatium;
