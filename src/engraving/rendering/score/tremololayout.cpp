...
void TremoloLayout::layoutOneNoteTremolo(TremoloSingleChord* item, const LayoutContext& ctx, double x, double y,
                                         double h, double spatium)
{
...
void TremoloLayout::createBeamSegments(TremoloTwoChord* item, const LayoutContext& ctx)
{
    ...
    PointF beamOffset
        = PointF(0., (item->up() ? 1 : -1) * item->spatium() * (ctx.conf().styleB(Sid::useWideBeams) ? 1. : 0.75));
