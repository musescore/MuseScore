...
void MeasureLayout::placeParentheses(EngravingItem* item, LayoutContext& ctx)
{
    if (!item) {
        return;
    }
...
Parenthesis* MeasureLayout::findOrCreateParenthesis(EngravingItem* item, const DirectionH direction, const track_idx_t track)
{
    if (!item) {
        return nullptr;
    }
    ...
    if (!paren) {
        paren = Factory::createParenthesis(item);
        paren->setParent(item);
...
void MeasureLayout::addRepeatCourtesyParentheses(Measure* m, const bool continuation, LayoutContext& ctx)
{
    ...
    auto createParenthesesForSegments = [&](Segment* leftSeg, Segment* rightSeg, track_idx_t track) -> void {
        ...
        Parenthesis* leftParen = findOrCreateParenthesis(leftItem, DirectionH::LEFT, track);
        Parenthesis* rightParen = findOrCreateParenthesis(rightItem, DirectionH::RIGHT, track);
