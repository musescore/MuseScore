...
TEST_F(TextBaseTests, getFontStyleProperty)
{
    ...
    EXPECT_EQ(staffText->getProperty(Ms::Pid::FONT_STYLE), QVariant::fromValue(0));
    ...
    EXPECT_EQ(staffText->getProperty(Ms::Pid::FONT_STYLE), QVariant::fromValue(static_cast<int>(FontStyle::Bold)));
    staffText->cursor()->movePosition(TextCursor::MoveOperation::NextWord, TextCursor::MoveMode::KeepAnchor);
    EXPECT_EQ(staffText->getProperty(Ms::Pid::FONT_STYLE), QVariant::fromValue(static_cast<int>(FontStyle::Bold)));
