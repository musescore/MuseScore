...
int Chord::minStaffOverlap(bool up, int staffLines, int beamCount, bool hasHook, qreal beamSpacing, bool useWideBeams, bool isFullSize)
{
    int beamOverlap = 8;
    if (isFullSize) {
...
int Chord::maxReduction(int extensionOutsideStaff) const
{
    ...
    if (_relativeMag != 1) {
        return qAbs(maxReductions[beamCount][extensionHalfSpaces]);
    }
...
qreal Chord::calcDefaultStemLength()
{
    ...
    shortestStem *= _relativeMag;
    defaultStemLength *= _relativeMag;
