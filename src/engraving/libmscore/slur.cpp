...
//---------------------------------------------------------
//   computeBezier
//    compute the shape of the slur segment and set grip points
//---------------------------------------------------------
    ...
    if (startSeg && endSeg && autoplace()) {
        ...
        do {
            ...
            if (iter % 2 == 0) {
                ...
                * of the slur.
                * ******************************************************/
                // Also make compensations in the case of very steep slurs
                double steepLimit = M_PI / 4;
                double epStep = adjust * step;
                // Collision in left section
                if (collision[0] || slurAngle < -steepLimit) {
                    // Lift the left end point, i.e. tilt the slur around p2
                    double stepX = sin(slurAngle) * epStep;
                    double stepY = cos(slurAngle) * epStep;
                    PointF pp1delta = PointF(stepX, stepY);
                    pp1 += PointF(0.0, epStep);
