<?xml version="1.0" encoding="UTF-8"?>
<museScore version="4.60">
  <Style>
    <Spatium>4</Spatium>
    <pageOddTopMargin>1.0</pageOddTopMargin>
    <pageOddLeftMargin>0</pageOddLeftMargin>
    <pageHeight>20</pageHeight>
    <pageWidth>15</pageWidth>
    <pagePrintableWidth>15</pagePrintableWidth>
    <rehearsalMarkFrameType>0</rehearsalMarkFrameType>
    
    <staffLowerBorder>10</staffLowerBorder>
    <minSystemSpread>5</minSystemSpread>
    <lyricsMinBottomDistance>2</lyricsMinBottomDistance>
    <lyricsMinDistance>1</lyricsMinDistance>
    <lyricsDashMaxLength>0.8</lyricsDashMaxLength>
    <lyricsMelismaPad>0.1</lyricsMelismaPad>
    <lyricsDashPad>0.2</lyricsDashPad>
    <lyricsOddFontSize>8</lyricsOddFontSize>
    <lyricsEvenFontSize>8</lyricsEvenFontSize>
    <barWidth>0.15</barWidth>
    <doubleBarWidth>0.13</doubleBarWidth>
    <endBarWidth>0.5</endBarWidth>
    <doubleBarDistance>0.32</doubleBarDistance>
    <endBarDistance>0.35</endBarDistance>
    <startBarlineSingle>1</startBarlineSingle>
    <maskBarlinesForText>0</maskBarlinesForText>
    <clefKeyDistance>1</clefKeyDistance>
    <shortestStem>3</shortestStem>
    <minNoteDistance>0.7</minNoteDistance>
    <barNoteDistance>1.7</barNoteDistance>
    <barAccidentalDistance>1.5</barAccidentalDistance>
    <noteBarDistance>1.7</noteBarDistance>
    <mrNumberEveryXMeasures>2</mrNumberEveryXMeasures>
    <staffLineWidth>0.06</staffLineWidth>
    <ledgerLineWidth>0.08</ledgerLineWidth>
    <ledgerLineLength>0.25</ledgerLineLength>
    <accidentalDistance>0.22</accidentalDistance>
    <bracketedAccidentalPadding>0.15</bracketedAccidentalPadding>
    <beamWidth>0.4</beamWidth>
    <hairpinPosAbove x="0" y="-2"/>
    <hairpinPosBelow x="0" y="2"/>
    <hairpinLinePosAbove x="0" y="-1.5"/>
    <hairpinLinePosBelow x="0" y="2.5"/>
    <pedalFontSize>12</pedalFontSize>
    <pedalContinueText>(&lt;sym&gt;keyboardPedalPed&lt;/sym&gt;)</pedalContinueText>
    <chordSymbolAFontSize>8</chordSymbolAFontSize>
    <chordSymbolAColor r="27" g="116" b="205" a="255"/>
    <fretFretSpacing>0.8</fretFretSpacing>
    <barreLineWidth>1</barreLineWidth>
    <showMeasureNumberOne>1</showMeasureNumberOne>
    <measureNumberInterval>1</measureNumberInterval>
    <measureNumberSystem>0</measureNumberSystem>
    <scaleRythmicSpacingForSmallNotes>0</scaleRythmicSpacingForSmallNotes>
    <createMultiMeasureRests>1</createMultiMeasureRests>
    <minEmptyMeasures>2</minEmptyMeasures>
    <mmRestBetweenStaves>0</mmRestBetweenStaves>
    <multiMeasureRestMargin>1.5</multiMeasureRestMargin>
    <enableIndentationOnFirstSystem>0</enableIndentationOnFirstSystem>
    <arpeggioNoteDistance>0.5</arpeggioNoteDistance>
    <arpeggioLineWidth>0.15</arpeggioLineWidth>
    <slurMidWidth>0.15</slurMidWidth>
    <tieMidWidth>0.15</tieMidWidth>
    <tiePlacementChord>inside</tiePlacementChord>
    <voltaLineWidth>0.1</voltaLineWidth>
    <voltaFontSize>10</voltaFontSize>
    <voltaFontStyle>0</voltaFontStyle>
    <voltaOffset x="0.6" y="2"/>
    <tupletStemLeftDistance>0</tupletStemLeftDistance>
    <tupletStemRightDistance>0</tupletStemRightDistance>
    <tupletBracketType>1</tupletBracketType>
    <tupletFontSize>8</tupletFontSize>
    <minVerticalDistance>3</minVerticalDistance>
    <dynamicsSize>0.85</dynamicsSize>
    <dynamicsPosAbove x="0" y="-1.5"/>
    <dynamicsPosBelow x="0" y="2.5"/>
    <titleFontSize>18</titleFontSize>
    <subTitleFontSize>10.8</subTitleFontSize>
    <subTitleOffset x="0" y="8"/>
    <composerFontSize>8</composerFontSize>
    <lyricistFontSize>8</lyricistFontSize>
    <translatorFontSize>8</translatorFontSize>
    <partInstrumentFontSize>10.8</partInstrumentFontSize>
    <tempoFontSize>9</tempoFontSize>
    <tempoPosAbove x="0" y="-1.5"/>
    <tempoChangeFontSize>7</tempoChangeFontSize>
    <tempoChangeFontStyle>0</tempoChangeFontStyle>
    <tempoChangePosAbove x="0" y="-1.5"/>
    <measureNumberFontSize>6</measureNumberFontSize>
    <measureNumberPosAbove x="0" y="-0.5"/>
    <measureNumberPosBelow x="0" y="1"/>
    <measureNumberAlign>center,baseline</measureNumberAlign>
    <mmRestRangeFontStyle>1</mmRestRangeFontStyle>
    <translatorFontSize>9</translatorFontSize>
    <staffTextFontSize>8</staffTextFontSize>
    <rehearsalMarkFontSize>9</rehearsalMarkFontSize>
    <rehearsalMarkAlign>left,baseline</rehearsalMarkAlign>
    <rehearsalMarkFrameWidth>0.1</rehearsalMarkFrameWidth>
    <rehearsalMarkPosAbove x="0" y="-2.5"/>
    <systemTextLineFontSize>12</systemTextLineFontSize>
    <glissandoText></glissandoText>
    <headerFontSize>11</headerFontSize>
    <headerFontStyle>1</headerFontStyle>
    <pageNumberFontSize>10</pageNumberFontSize>
    <instrumentChangeFontStyle>2</instrumentChangeFontStyle>
    <ottavaFontSize>9</ottavaFontSize>
    <longInstrumentFontSize>9</longInstrumentFontSize>
    <shortInstrumentFontSize>9</shortInstrumentFontSize>
    <dynamicsFontSize>9</dynamicsFontSize>
    <expressionFontSize>9</expressionFontSize>
    <metronomeFontSize>11</metronomeFontSize>
    <systemTextFontSize>8</systemTextFontSize>
    <instrumentChangeOffset x="0" y="-3"/>
    <stickingAlign>left,baseline</stickingAlign>
    <stickingPosAbove x="0" y="-2"/>
    <letRingFontSize>7.5</letRingFontSize>
    <palmMuteFontSize>7.5</palmMuteFontSize>
    <bendFontSize>7.2</bendFontSize>
    <slurShowTabSimple>0</slurShowTabSimple>
    <fermataShowTabSimple>0</fermataShowTabSimple>
    <dynamicsShowTabSimple>0</dynamicsShowTabSimple>
    <hairpinShowTabSimple>0</hairpinShowTabSimple>
    <accentShowTabSimple>0</accentShowTabSimple>
    <staccatoShowTabSimple>0</staccatoShowTabSimple>
    <rasgueadoShowTabSimple>0</rasgueadoShowTabSimple>
    <mordentShowTabSimple>0</mordentShowTabSimple>
    <turnShowTabSimple>0</turnShowTabSimple>
    <wahShowTabSimple>0</wahShowTabSimple>
    <golpeShowTabSimple>0</golpeShowTabSimple>
    <tabShowTiedFret>2</tabShowTiedFret>
    <tabParenthesizeTiedFret>1</tabParenthesizeTiedFret>
    <defaultsVersion>420</defaultsVersion>
    <changesBeforeBarlineRepeats>0</changesBeforeBarlineRepeats>
    <changesBeforeBarlineOtherJumps>0</changesBeforeBarlineOtherJumps>
    <showCourtesiesRepeats>0</showCourtesiesRepeats>
    <showCourtesiesOtherJumps>0</showCourtesiesOtherJumps>
    <showCourtesiesAfterCancellingRepeats>0</showCourtesiesAfterCancellingRepeats>
    <showCourtesiesAfterCancellingOtherJumps>0</showCourtesiesAfterCancellingOtherJumps>
    <showFretOnFullBendRelease>0</showFretOnFullBendRelease>
    <hopoShowOnStandardStaves>0</hopoShowOnStandardStaves>
    <hopoShowAll>0</hopoShowAll>
    <hopoAlignLettersStandardStaves>0</hopoAlignLettersStandardStaves>
    <hopoAlignLettersTabStaves>0</hopoAlignLettersTabStaves>
    <lhTappingSymbolNormalStave>circledT</lhTappingSymbolNormalStave>
    <lhTappingSymbolTab>circledT</lhTappingSymbolTab>
    <lhTappingShowItemsNormalStave>0</lhTappingShowItemsNormalStave>
    <lhTappingShowItemsTab>2</lhTappingShowItemsTab>
    <rhTappingSymbolNormalStave>T</rhTappingSymbolNormalStave>
    <partInstrumentFontSpatiumDependent>1</partInstrumentFontSpatiumDependent>
    <metronomeFontSpatiumDependent>1</metronomeFontSpatiumDependent>
    <measureNumberFontSpatiumDependent>1</measureNumberFontSpatiumDependent>
    <mmRestRangeFontSpatiumDependent>1</mmRestRangeFontSpatiumDependent>
    <headerFontSpatiumDependent>1</headerFontSpatiumDependent>
    <footerFontSpatiumDependent>1</footerFontSpatiumDependent>
    <lyricsOddFontSpatiumDependent>1</lyricsOddFontSpatiumDependent>
    <lyricsEvenFontSpatiumDependent>1</lyricsEvenFontSpatiumDependent>
    <frameFontSpatiumDependent>1</frameFontSpatiumDependent>
    <pageNumberFontSpatiumDependent>1</pageNumberFontSpatiumDependent>
    <copyrightFontSpatiumDependent>1</copyrightFontSpatiumDependent>
    <rehearsalMarkFontSpatiumDependent>1</rehearsalMarkFontSpatiumDependent>
    <measureNumberAlternateFontSpatiumDependent>1</measureNumberAlternateFontSpatiumDependent>
    <preferSameStringForTranspose>1</preferSameStringForTranspose>
    <copyrightOffset x="0" y="0"/>
    <footerOffset x="0" y="0"/>
    <pageNumberOffset x="0" y="0"/>
  </Style>
  </museScore>
