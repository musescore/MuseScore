...
void EngravingElementsProvider::dumpTree(const Ms::EngravingObject* e) const
{
    int level = 0;
    dumpTree(e, level);
}

void EngravingElementsProvider::dumpTree(const Ms::EngravingObject* e, int& level) const
{
    if (e->isDummy()) {
        return;
    }
    ...
    gap.fill(' ', level);
    LOGI() << gap << e->name();
    for (const Ms::EngravingObject* ch : e->children()) {
