...
Column {
    ...
    //! NOTE Overridden in instances and specializations of PropertyView
        ...
        Loader {
            ...
            Component {
                ...
                PropertyResetButton {
                    ...
                    navigation.accessible.name: root.accessibleName ? qsTrc("propertiespanel", "Reset “%1” to default value").arg(root.accessibleName)
                                                                    : qsTrc("propertiespanel", "Reset property to default value")
            ...
            Component {
                ...
                MenuButton {
                    ...
                    navigation.accessible.name: qsTrc("propertiespanel", "Menu for “%1”").arg(root.accessibleName)

                    menuModel: {
                        var result = []

                        result.push({ title: qsTrc("propertiespanel", "Reset"), enabled: root.isModified, icon: IconCode.UNDO, id: "reset" })

                        if (root.isModified) {
                            result.push({ title: qsTrc("propertiespanel", "Save as default style for this score"), enabled: !root.propertyItem.isUndefined, icon: IconCode.SAVE, id: "save" })
                        } else {
                            result.push({ title: qsTrc("propertiespanel", "This is set as the default style for this score"), enabled: false, icon: IconCode.TICK_RIGHT_ANGLE })
                        }
