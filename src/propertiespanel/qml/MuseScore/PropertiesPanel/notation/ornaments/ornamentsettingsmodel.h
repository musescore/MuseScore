...
class OrnamentSettingsModel : public PropertiesPanelAbstractModel
{
    Q_OBJECT
    QML_ELEMENT;
    ...
    Q_PROPERTY(mu::propertiespanel::PropertyItem * intervalAbove READ intervalAbove CONSTANT)
    Q_PROPERTY(bool isIntervalAboveAvailable READ isIntervalAboveAvailable NOTIFY isIntervalAboveAvailableChanged)

    Q_PROPERTY(mu::propertiespanel::PropertyItem * intervalBelow READ intervalBelow CONSTANT)
    Q_PROPERTY(bool isIntervalBelowAvailable READ isIntervalBelowAvailable NOTIFY isIntervalBelowAvailableChanged)

    Q_PROPERTY(mu::propertiespanel::PropertyItem * intervalStep READ intervalStep CONSTANT)
    Q_PROPERTY(mu::propertiespanel::PropertyItem * intervalType READ intervalType CONSTANT)
    ...
    Q_PROPERTY(mu::propertiespanel::PropertyItem * showAccidental READ showAccidental CONSTANT)
    Q_PROPERTY(mu::propertiespanel::PropertyItem * startOnUpperNote READ startOnUpperNote CONSTANT)

    Q_PROPERTY(mu::propertiespanel::PropertyItem * showCueNote READ showCueNote CONSTANT)
