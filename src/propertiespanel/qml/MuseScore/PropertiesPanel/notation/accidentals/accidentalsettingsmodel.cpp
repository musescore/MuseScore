...
{
    setModelType(PropertiesPanelModelType::TYPE_ACCIDENTAL);
    setTitle(muse::qtrc("propertiespanel", "Accidental"));
...
void AccidentalSettingsModel::updateIsSmallAvailable()
{
    ...
    for (mu::engraving::EngravingItem* item : m_elementList) {
        mu::engraving::EngravingItem* parent = item ? item->ownershipParentItem() : nullptr;
