...
{
    setModelType(PropertiesPanelModelType::TYPE_BEAM);
    setTitle(muse::qtrc("propertiespanel", "Beam"));

    m_beamModesModel = new BeamModesModel(this, iocCtx, repository);
    m_beamModesModel->init();

    connect(m_beamModesModel->mode(), &PropertyItem::propertyModified, this, &PropertiesPanelAbstractModel::requestReloadPropertyItems);
}
