...
BracketSettingsModel::BracketSettingsModel(QObject* parent, const muse::modularity::ContextPtr& iocCtx,
    ...
    : PropertiesPanelAbstractModel(parent, iocCtx, repository, mu::engraving::ElementType::BRACKET)
{
    setModelType(PropertiesPanelModelType::TYPE_BRACKET);
    setTitle(muse::qtrc("propertiespanel", "Bracket"));
...
{
    return m_elementList.count() == 1; // Brackets propertiespanel doesn't support multiple selection
}
...
int BracketSettingsModel::maxBracketColumnPosition() const
{
    ...
    int bracketStartIndex = static_cast<int>(bracketItem->startStaff()->idx());
...
int BracketSettingsModel::maxBracketSpanStaves() const
{
    ...
    const BracketItem* bracketItem = toBracketItem(m_elementList.front());
    return static_cast<int>(bracketItem->score()->nstaves()) - static_cast<int>(bracketItem->startStaff()->idx());
}
