    ...
    : TextLineSettingsModel(parent, iocCtx, repository)
{
    if (lineType == Diminuendo) {
        setModelType(PropertiesPanelModelType::TYPE_DIMINUENDO);
        setTitle(muse::qtrc("propertiespanel", "Diminuendo"));
        setIcon(muse::ui::IconCode::Code::DIMINUENDO);
    } else {
        setModelType(PropertiesPanelModelType::TYPE_CRESCENDO);
        setTitle(muse::qtrc("propertiespanel", "Crescendo"));
