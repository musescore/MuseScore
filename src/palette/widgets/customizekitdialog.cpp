...
struct SymbolIcon {
    ...
    static SymbolIcon generateIcon(const SymId& id, double w, double h, double defaultScale)
    {
        ...
        painter.setPen(Color(CustomizeKitDialog::uiConfiguration()->currentTheme().values[muse::ui::FONT_PRIMARY_COLOR].
                             toString()));
...
void CustomizeKitDialog::fillCustomNoteheadsDataFromComboboxes(int pitch)
{
    ...
    m_editedDrumset.drum(
        pitch).noteheads[int(NoteHeadType::HEAD_QUARTER)] = static_cast<SymId>(quarterCmb->currentData().toInt());
