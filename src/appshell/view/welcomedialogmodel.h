...
class WelcomeDialogModel : public QObject, public muse::Injectable
{
    ...
    Q_PROPERTY(bool hasPrev READ hasPrev NOTIFY currentItemChanged)
    Q_PROPERTY(bool hasNext READ hasNext NOTIFY currentItemChanged)
...
public:
    ...
    bool hasPrev() const { return !m_items.empty() && m_currentIndex > 0; }
    bool hasNext() const { return !m_items.empty() && m_currentIndex < count() - 1; }
