...
Loader {
    ...
    sourceComponent: FlatButton {
        ...
        mouseArea.enabled: view.model.canProcessOnlineSounds
        ...
        toolTipTitle: {
            if (view.model.status === OnlineSoundsStatusModel.Error) {
                return qsTrc("playback", "Some online sounds aren’t ready yet")
            }
        ...
        toolTipDescription: {
            if (view.model.status === OnlineSoundsStatusModel.Error) {
                return qsTrc("global", "Please check your internet connection or try again later.")
            }
