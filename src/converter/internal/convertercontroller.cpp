...
Ret ConverterController::batchConvert(const muse::io::path_t& batchJobFile, const OpenParams& openParams,
                                      const String& soundProfile, const muse::UriQuery& extensionUri, const size_t pageNum,
                                      muse::ProgressPtr progress)
{
    ...
    for (const Job& job : batchJob.val) {
        ...
        Ret ret = fileConvert(job.in, job.out, openParams, soundProfile, extensionUri, job.transposeOptions, pageNum);
        if (!ret) {
                                     ...
                                     const size_t pageNum)
{
    ...
    // standart convert
        ...
        const bool pageNumValid = pageNum != muse::nidx;
        ...
        if (pageNumValid || isConvertPageByPage(suffix)) {
            if (pageNumValid) {
