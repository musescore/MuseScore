...
TEST_F(Global_Ser_XML, testNoChildren) {
    ByteArray data;
    Buffer buf(&data);
    ...
    // Write
    {
        ...
        XmlStreamWriter xml(&buf);
        xml.startDocument();
        xml.startElement("tagWithChildren");
        xml.element("tagNoChildren");
        xml.endElement();
    ...
    // Read
    {
        ...
        EXPECT_TRUE(xml.readNextStartElement());
        EXPECT_EQ(xml.name(), "tagWithChildren");
        EXPECT_FALSE(xml.noChildren());

        EXPECT_TRUE(xml.readNextStartElement());
        EXPECT_EQ(xml.name(), "tagNoChildren");
        EXPECT_TRUE(xml.noChildren());
    }
}
