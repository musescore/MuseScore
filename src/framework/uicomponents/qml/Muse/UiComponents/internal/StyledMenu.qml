...
MenuView {
    ...
    function navigateWithSymbolRequested(symbol, requestingMenuModel) {
        ...
        // Find the menu items that match the given underlined symbol (letter).
        ...
        for (let j = 0; j < view.count; ++j) {
            ...
            if (!item.enabled || !requestingMenuModel.menuItemMatchesSymbol(item, symbol)) {
                continue;
            }
        ...
        // Focus the first matching menu item. If it is the only match, also click it.
        // Otherwise let the user click it or navigate further to the other matching menu items.
        if (firstMatchingIndex !== -1) {
