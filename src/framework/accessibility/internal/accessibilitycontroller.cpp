    ...
    : muse::Injectable(iocCtx)
{
    ...
    m_pretendFocusTimer.callOnTimeout(lambda);
}
...
void AccessibilityController::announce(const QString& announcement)
{
    ...
    QAccessible::Event eventType = QAccessible::NameChanged;
...
void AccessibilityController::triggerRevoicing(const Item& current)
{
    ...
    swapExternalFocus(current, pretend);
...
void AccessibilityController::restoreFocus()
{
    ...
    swapExternalFocus(pretend, restore);
}

void AccessibilityController::swapExternalFocus(const Item& from, const Item& to)
{
    // We can't tell external APIs the new value of a state; only that it has changed.
    ...
    // VoiceOver: Use QObject not QAccessibleInterface in QAccessible…Event() constructors.
    QAccessibleStateChangeEvent fromChanged(from.object, focusChanged);
    QAccessibleStateChangeEvent toChanged(to.object, focusChanged);
    QAccessibleEvent toFocused(to.object, QAccessible::Focus);
    sendEvent(&fromChanged);
    sendEvent(&toFocused);
    sendEvent(&toFocused);
}
