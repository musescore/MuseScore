...
void StartAudioController::startAudioProcessing(const IApplication::RunMode& mode)
{
    ...
    requiredSpec.callback = [this](void* /*userdata*/, uint8_t* stream, int byteCount) {
...
#ifdef MUSE_MODULE_AUDIO_WORKER
        m_engineController->popAudioData(reinterpret_cast<float*>(stream), samplesPerChannel);
#else
        static bool once = false;
        if (!once) {
            th_setupEngine();
        ...
        m_rpcChannel->process();
        m_engineController->process(reinterpret_cast<float*>(stream), samplesPerChannel);
#endif
    };
