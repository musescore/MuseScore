...
#define IROM_FCC                                                               \
    FLUID_FOURCC('i', 'r', 'o', 'm') /* info ids (1st byte of info strings) */
...
static const uint32_t idlist[] = {
    RIFF_FCC, LIST_FCC, SFBK_FCC, INFO_FCC, SDTA_FCC, PDTA_FCC,

    IFIL_FCC, ISNG_FCC, INAM_FCC, IROM_FCC, IVER_FCC, ICRD_FCC,
    IENG_FCC, IPRD_FCC, ICOP_FCC, ICMT_FCC, ISFT_FCC,

    SNAM_FCC, SMPL_FCC, PHDR_FCC, PBAG_FCC, PMOD_FCC, PGEN_FCC,
    IHDR_FCC, IBAG_FCC, IMOD_FCC, IGEN_FCC, SHDR_FCC, SM24_FCC };

static const unsigned short invalid_inst_gen[] = {
    GEN_UNUSED1,   GEN_UNUSED2,   GEN_UNUSED3,   GEN_UNUSED4,
    GEN_RESERVED1, GEN_RESERVED2, GEN_RESERVED3, GEN_INSTRUMENT,
};
 ...
 *
 * If fluidsynth was built with DLS support, this function will also identify
 * DLS files.
 *
 * @note This function only checks whether header(s) in the RIFF chunk are
 * present. A call to fluid_synth_sfload() might still fail.
 */
int fluid_is_soundfont(const char* filename)
{
    ...
    do {
        if ((fp = fluid_file_open(filename, &err_msg)) == NULL) {
            FLUID_LOG(FLUID_ERR, "fluid_is_soundfont(): fopen() failed: '%s'",
                      err_msg);
            return retcode;
        }
        ...
        if (FLUID_FREAD(&fcc, sizeof(fcc), 1, fp) != 1) {
            FLUID_LOG(FLUID_ERR,
                      "fluid_is_soundfont(): failed to read RIFF chunk id.");
            break;
        }

        if (fcc != RIFF_FCC) {
            FLUID_LOG(FLUID_ERR,
                      "fluid_is_soundfont(): expected RIFF chunk id '0x%04X' but got "
                      "'0x%04X'.",
                      (unsigned int)RIFF_FCC, (unsigned int)fcc);
            break;
        }
        ...
        if (FLUID_FREAD(&fcc, sizeof(fcc), 1, fp) != 1) {
            FLUID_LOG(FLUID_ERR,
                      "fluid_is_soundfont(): failed to read SFBK chunk id.");
            break;
        }
        ...
        if (retcode) {
            ...
            if (fhandle != NULL) {
                retcode
                    =(ipatch_file_identify(fhandle->file, NULL) == IPATCH_TYPE_DLS_FILE);
 ...
 */
SFData* fluid_sffile_open(const char* fname,
 ...
 *
 * This function will always return the sample data in WAV format. If the
 * sample_type specifies an Ogg Vorbis compressed sample, it will be
 * decompressed automatically before returning.
 *
 ...
 * @param data pointer to sample data pointer, will point to loaded sample data
 * on success
 * @param data24 pointer to 24-bit sample data pointer if 24-bit data present,
 * will point to loaded 24-bit sample data on success or NULL if no 24-bit data
 * is present in file
 *
 * @return The number of sample words in returned buffers or -1 on failure
 */
int fluid_sffile_read_sample_data(SFData* sf, unsigned int sample_start,
                                  unsigned int sample_end, int sample_type,
                                  short** data, char** data24)
{
    ...
    if (sample_type & FLUID_SAMPLETYPE_OGG_VORBIS) {
        ...
        num_samples
            =fluid_sffile_read_wav(sf, sample_start, sample_end, data, data24);
    }
...
static int process_info(SFData* sf, int size)
{
    ...
    while (size > 0) {
        ...
        if (chunk.id == IFIL_FCC) {
            ...
            if ((chunk.id != ICMT_FCC && chunk.size > 256) || (chunk.size > 65536)
                || (chunk.size % 2)) {
                FLUID_LOG(FLUID_ERR,
...
static int pdtahelper(SFData* sf, unsigned int expid, unsigned int reclen,
                      SFChunk* chunk, int* size)
{
    ...
    *size -= 8;
    ...
    if (chunk->id != expid) {
        FLUID_LOG(FLUID_ERR,
                  "Expected PDTA sub-chunk '%.4s' found invalid id instead",
                  (char*)&expid);
        return FALSE;
    }
...
static int load_phdr(SFData* sf, unsigned int size)
{
    ...
    for (; i > 0; i--) {
        ...
        preset->zone
            =NULL; /* In case of failure, fluid_sffile_close can cleanup */
        ...
        if (prev_preset) {
            ...
            FLUID_LOG(FLUID_WARN, "%d preset zones not referenced, discarding",
                      pbag_idx);
        }
    ...
    while (preset_list) {
        ...
        while (zone_list) {
            ...
            if (level < 3 && (zone_list != preset->zone)) {
                ...
                /* we have already advanced the zone_list pointer, so continue with next
                 * zone */
                continue;
            }
...
static int load_ihdr(SFData* sf, unsigned int size)
{
    ...
    for (i = 0; i < size; i++) {
        ...
        if (prev_inst) {
            ...
            FLUID_LOG(FLUID_WARN, "%d instrument zones not referenced, discarding",
                      zndx);
        }
...
static int preset_compare_func(const void* a, const void* b)
{
    ...
    aval
        =(int)(((const SFPreset*)a)->bank) << 16 | ((const SFPreset*)a)->prenum;
    bval
        =(int)(((const SFPreset*)b)->bank) << 16 | ((const SFPreset*)b)->prenum;
...
static int fluid_sffile_read_wav(SFData* sf, unsigned int start,
                                 unsigned int end, short** data,
                                 char** data24)
{
    ...
    if ((start * sizeof(short) > sf->samplesize)
        || (end * sizeof(short) > sf->samplesize)) {
        FLUID_LOG(FLUID_ERR, "Sample offsets exceed sample data chunk");
        goto error_exit;
    }

    /* Load 16-bit sample data */
    if (sf->fcbs->fseek(sf->sffd, sf->samplepos + (start * sizeof(short)),
                        SEEK_SET) == FLUID_FAILED) {
    ...
    if (sf->fcbs->fread(loaded_data, num_samples * sizeof(short), sf->sffd)
        ...
        FLUID_VERSION_CHECK(2, 2, 0)
        if ((int)(num_samples * sizeof(short)) < 0) {
            FLUID_LOG(FLUID_INFO, "This SoundFont seems to be bigger than 2GB, which "
                                  "is not supported in this version of fluidsynth. "
    ...
    *data = loaded_data;
    ...
    /* Optionally load additional 8 bit sample data for 24-bit support. Any
     * failures while loading the 24-bit sample data will be logged as errors but
     * won't prevent the sample reading to fail, as sound output is still possible
     * with the 16-bit sample data. */
    if (sf->sample24pos) {
        ...
        if (sf->fcbs->fseek(sf->sffd, sf->sample24pos + start, SEEK_SET)
            == FLUID_FAILED) {
            FLUID_LOG(FLUID_ERR,
                      "Failed to seek position for 24-bit sample data in data file");
            goto error24_exit;
        }
...
error24_exit:
    FLUID_LOG(FLUID_WARN,
              "Ignoring 24-bit sample data, sound quality might suffer");
...
static sf_count_t sfvio_read(void* ptr, sf_count_t count, void* user_data)
{
    ...
    fluid_rec_mutex_lock(sf->mtx);
    if (sf->fcbs->fseek(sf->sffd, data->start + data->offset, SEEK_SET)
        == FLUID_FAILED) {
        FLUID_LOG(FLUID_ERR,
                  "This should never happen: fseek failed in sfvoid_read()");
        count = 0;
    } else {
...
/**
 * Read Ogg Vorbis compressed data from the Soundfont and decompress it,
 * returning the number of samples in the decompressed WAV. Only 16-bit mono
 * samples are supported.
 *
 * Note that this function takes byte indices for start and end source data. The
 * sample headers in SF3 files use byte indices, so those pointers can be passed
 ...
 */
static int fluid_sffile_read_vorbis(SFData* sf, unsigned int start_byte,
                                    unsigned int end_byte, short** data)
{
    SNDFILE* sndfile;
    SF_INFO sfinfo;
    SF_VIRTUAL_IO sfvio = { sfvio_get_filelen, sfvio_seek, sfvio_read, NULL,
                            sfvio_tell };
    sfvio_data_t sfdata;
    short* wav_data = NULL;
    ...
    if (sfinfo.channels != 1) {
        FLUID_LOG(FLUID_DBG, "Unsupported channel count %d in ogg sample",
                  sfinfo.channels);
        goto error_exit;
    }
    ...
    if ((sfinfo.format & SF_FORMAT_OGG) == 0) {
        FLUID_LOG(
            FLUID_WARN,
            "OGG sample is not OGG compressed, this is not officially supported");
    }
...
extern int vorbis_decode_memory(const unsigned char* mem, unsigned int len, short** output, unsigned int* channels,
