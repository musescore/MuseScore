    ...
    for (const auto& pair : changes) {
        for (const mpe::PlaybackEvent& event : pair.second) {
            ...
            for (const auto& pair : noteEvent.expressionCtx().articulations) {
                ...
                } else if (muse::contains(SUSTAIN_PEDAL_CC_SUPPORTED_TYPES, meta.type)) {
                ...
                } else if (muse::contains(SOSTENUTO_PEDAL_CC_SUPPORTED_TYPES, meta.type)) {
                    mpe::timestamp_t timestamp = timestampFrom + noteEvent.arrangementCtx().actualDuration * 0.1; // add offset for Sostenuto to take effect
