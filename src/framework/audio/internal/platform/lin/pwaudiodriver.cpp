...
void PwRegistry::unregisterGlobal(uint32_t id)
{
    ...
    m_pwDevices.erase(std::remove_if(m_pwDevices.begin(), m_pwDevices.end(), [id](const Device& device) {
        return device.id == id;
    }), m_pwDevices.end());

    m_pwNodes.erase(std::remove_if(m_pwNodes.begin(), m_pwNodes.end(), [id](const SinkNode& node) {
        return node.id == id;
    }), m_pwNodes.end());
