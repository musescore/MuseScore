...
void ThemeApi::initUiFonts()
{
    ...
    configuration()->defaultFontChanged().onNotify(this, [this]() {
        // We get the change notification immediately after the user changes
        // the text scaling setting, but for some reason it takes time
        // (70-100ms on my machine) for MuseScore to read the updated font size.
        // So we must poll until we see the change or give up after some time.
        if (m_pollTimer) {
            return;
        }
        ...
        m_pollTimer->callOnTimeout(this, [this, oldDefaultFont, pollCount]() mutable {
