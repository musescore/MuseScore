...
static const QString GUITARPRO_DIR("data/");
...
{
    initMTest(QString(iex_guitarpro_tests_DATA_ROOT));
...
{
    MasterScore* score = readScore(GUITARPRO_DIR + file + "." + ext);
    QVERIFY(score);

    QVERIFY(saveCompareScore(score, QString("%1.%2.mscx").arg(file).arg(ext),
                             GUITARPRO_DIR + QString("%1.%2-ref.mscx").arg(file).arg(ext)));
    delete score;
}
