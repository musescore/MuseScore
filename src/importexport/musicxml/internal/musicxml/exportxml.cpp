...
static void fermata(const Fermata* const a, XmlWriter& xml)
{
    ...
    tagName += String(" type=\"%1\"").arg(a->placement() == PlacementV::ABOVE ? u"upright" : u"inverted");
    ...
    if (isLineType) {
        if (hp->lineVisible()) {
            if (hp->tick() == tick) {
                ...
                m_xml.startElement("direction-type");
                m_xml.tagRaw(String("dashes type=\"stop\" number=\"%1\"").arg(n + 1));
                m_xml.endElement();
            }
        }
    } else {
...
static void segmentHarmonies(ExportMusicXml* exp, track_idx_t track, Segment* seg, size_t offset)
{
    ...
    for (const EngravingItem* e : diagrams) {
        ...
        for (const EngravingItem* e : harmonies) {
...
void ExportMusicXml::writeParts()
{
    ...
    for (size_t partIndex = 0; partIndex < parts.size(); ++partIndex) {
        ...
        m_tick = { 0, 1 };
        m_xml.startElementRaw(String("part id=\"P%1\"").arg(partIndex + 1));
