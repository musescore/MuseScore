...
ChordRest* MeiImporter::addChordRest(pugi::xml_node node, Measure* measure, int track,
                                     const libmei::Element& meiElement, Fraction& ticks,
                                     bool isRest)
{
...
ControlElementPosition MeiImporter::findStart(const libmei::Element& meiElement, Measure* measure)
{
    ...
    if (startIdAtt->HasStartid()) {
        ...
        const int layer = (layerIdentAtt && layerIdentAtt->HasLayer()) ? this->getVoiceIndex(staffIdx,
...
ControlElementPosition MeiImporter::findEnd(pugi::xml_node controlNode, Spanner* spanner)
{
    ...
    if (startEndIdAtt.HasEndid()) {
        ...
        if (!m_endIdChordRests.count(endId) || !m_endIdChordRests.at(endId)) {
            Convert::logs.push_back(String("Could not find element for @endid '%1'").arg(String::fromStdString(
                                                                                             startEndIdAtt.GetEndid())));
            return pos;
        }
...
Note* MeiImporter::findStartNote(const libmei::Element& meiElement)
{
    ...
    if (!m_startIdNotes.count(startId) || !m_startIdNotes.at(startId)) {
        Convert::logs.push_back(String("Could not find note for @startid '%1'").arg(String::fromStdString(startIdAtt->
                                                                                                          GetStartid())));
        return nullptr;
    }
...
Note* MeiImporter::findEndNote(pugi::xml_node controlNode)
{
    ...
    if (!m_endIdNotes.count(endId) || !m_endIdNotes.at(endId)) {
        Convert::logs.push_back(String("Could not find note for @endid '%1'").arg(String::fromStdString(startEndIdAtt.
                                                                                                        GetEndid())));
        return nullptr;
    }
...
bool MeiImporter::readStaffDefs(pugi::xml_node parentNode)
{
    ...
    for (pugi::xpath_node staffDefXpathNode : staffDefs) {
        ...
        if (meterSigNode) {
            ...
            meiStaffDef.SetMeterSym(meiStaffDef.AttMeterSigDefaultLog::StrToMetersign(meterSigNode.attribute("sym").
                                                                                      value()));
        }
...
bool MeiImporter::readSectionElements(pugi::xml_node parentNode)
{
    ...
    if (m_score->measures()->last() && m_score->measures()->last()->isMeasure()) {
        Measure* measure = toMeasure(m_score->measures()->last());
...
bool MeiImporter::readStaves(pugi::xml_node parentNode, Measure* measure, Fraction& measureTicks)
{
    ...
    for (pugi::xpath_node xpathNode : staves) {
        ...
        int timeSigIdx
            = m_timeSigs.count(staffIdx) ? staffIdx : m_timeSigs.count(SCOREDEF_IDX) ? SCOREDEF_IDX : MEI_UNSET;
...
bool MeiImporter::readChord(pugi::xml_node chordNode, Measure* measure, int track, Fraction& ticks)
{
    ...
    Chord* chord = toChord(addChordRest(chordNode, measure, track, meiChord, ticks, false));
...
bool MeiImporter::readNote(pugi::xml_node noteNode, Measure* measure, int track, Fraction& ticks, Chord* chord)
{
    ...
    if (!chord) {
        chord = toChord(addChordRest(noteNode, measure, track, meiNote, ticks, false));
...
bool MeiImporter::readSyl(pugi::xml_node sylNode, Lyrics* lyrics, Convert::textWithSmufl& textBlocks,
                          ElisionType elision)
{
...
bool MeiImporter::readArpeg(pugi::xml_node arpegNode, Measure* measure)
{
    ...
    Arpeggio* arpeggio = toArpeggio(this->addToChordRest(meiArpeg, measure));
    if (!arpeggio) {
        // Warning message given in MeiImporter::addToChordRest
        return true;
    }
...
bool MeiImporter::readBreath(pugi::xml_node breathNode, Measure* measure)
{
    ...
    Breath* breath = toBreath(this->addAnnotation(meiBreath, measure));
    if (!breath) {
...
bool MeiImporter::readCaesura(pugi::xml_node caesuraNode, Measure* measure)
{
    ...
    Breath* breath = toBreath(this->addAnnotation(meiCaesura, measure));
    if (!breath) {
        // Warning message given in MeiImporter::addAnnotation
        return true;
    }
...
bool MeiImporter::readDir(pugi::xml_node dirNode, Measure* measure)
{
    ...
    if (Convert::isDirWithExt(meiDir)) {
        TextLineBase* textLineBase = toTextLineBase(this->addSpanner(meiDir, measure, dirNode));
        if (!textLineBase) {
        ...
        TextBase* textBase = toTextBase(this->addAnnotation(meiDir, measure));
        if (!textBase) {
            // Warning message given in MeiImporter::addAnnotation
            return true;
        }
...
bool MeiImporter::readDynam(pugi::xml_node dynamNode, Measure* measure)
{
    ...
    Dynamic* dynamic = toDynamic(this->addAnnotation(meiDynam, measure));
    if (!dynamic) {
        // Warning message given in MeiImporter::addAnnotation
        return true;
    }
...
bool MeiImporter::readFb(pugi::xml_node harmNode, Measure* measure)
{
    ...
    FiguredBass* figuredBass = toFiguredBass(this->addAnnotation(meiHarm, measure));
    if (!figuredBass) {
        // Warning message given in MeiImporter::addAnnotation
        return true;
    }
...
bool MeiImporter::readFermata(pugi::xml_node fermataNode, Measure* measure)
{
    ...
    if (meiFermata.HasTstamp()) {
        ...
        if (fermataPos == measure->ticks()) {
            ...
            const int staffIdx
                = (meiFermata.HasStaff() && meiFermata.GetStaff().size() > 0) ? this->getStaffIndex(
                      meiFermata.GetStaff().at(0)) : 0;
    ...
    if (!fermata) {
        fermata = toFermata(this->addAnnotation(meiFermata, measure));
    }
...
bool MeiImporter::readHairpin(pugi::xml_node hairpinNode, Measure* measure)
{
    ...
    Hairpin* hairpin = toHairpin(this->addSpanner(meiHairpin, measure, hairpinNode));
    if (!hairpin) {
        // Warning message given in MeiImporter::addSpanner
        return true;
    }
...
bool MeiImporter::readHarm(pugi::xml_node harmNode, Measure* measure)
{
    ...
    Harmony* harmony = toHarmony(this->addAnnotation(meiHarm, measure));
    if (!harmony) {
        // Warning message given in MeiImporter::addAnnotation
        return true;
    }
...
bool MeiImporter::readHarpPedal(pugi::xml_node harpPedalNode, Measure* measure)
{
    ...
    HarpPedalDiagram* harpPedalDiagram = toHarpPedalDiagram(this->addAnnotation(meiHarpPedal, measure));
    if (!harpPedalDiagram) {
        // Warning message given in MeiImporter::addAnnotation
        return true;
    }
...
bool MeiImporter::readMordent(pugi::xml_node mordentNode, Measure* measure)
{
    ...
    Ornament* ornament = toOrnament(this->addToChordRest(meiMordent, measure));
    if (!ornament) {
        // Warning message given in MeiImporter::addToChordRest
        return true;
    }
...
bool MeiImporter::readOctave(pugi::xml_node octaveNode, Measure* measure)
{
    ...
    Ottava* ottava = toOttava(this->addSpanner(meiOctave, measure, octaveNode));
    if (!ottava) {
        // Warning message given in MeiImporter::addSpanner
        return true;
    }
...
bool MeiImporter::readOrnam(pugi::xml_node ornamNode, Measure* measure)
{
    ...
    Ornament* ornament = toOrnament(this->addToChordRest(meiOrnam, measure));
    if (!ornament) {
        // Warning message given in MeiImporter::addToChordRest
        return true;
    }
...
bool MeiImporter::readPedal(pugi::xml_node pedalNode, Measure* measure)
{
    ...
    Pedal* pedal = toPedal(this->addSpanner(meiPedal, measure, pedalNode));
    if (!pedal) {
        // Warning message given in MeiImporter::addSpanner
        return true;
    }
...
bool MeiImporter::readReh(pugi::xml_node rehNode, Measure* measure)
{
    ...
    RehearsalMark* rehearsalMark = toRehearsalMark(this->addAnnotation(meiReh, measure));
    if (!rehearsalMark) {
        // Warning message given in MeiImporter::addAnnotation
        return true;
    }
...
bool MeiImporter::readRepeatMark(pugi::xml_node repeatMarkNode, Measure* measure)
{
    ...
    if (Convert::elementTypeFor(meiRepeatMark) == ElementType::JUMP) {
        ...
        Convert::jumpFromMEI(toJump(item), meiRepeatMark, warning);
    } else {
        item = Factory::createMarker(measure);
        Convert::markerFromMEI(toMarker(item), meiRepeatMark, warning);
    }
...
bool MeiImporter::readSlur(pugi::xml_node slurNode, Measure* measure)
{
    ...
    Slur* slur = toSlur(this->addSpanner(meiSlur, measure, slurNode));
    if (!slur) {
        // Warning message given in MeiImporter::addSpanner
        return true;
    }
...
bool MeiImporter::readTempo(pugi::xml_node tempoNode, Measure* measure)
{
    ...
    TempoText* tempoText = toTempoText(this->addAnnotation(meiTempo, measure));
    if (!tempoText) {
        // Warning message given in MeiImporter::addAnnotation
        return true;
    }
...
bool MeiImporter::readTrill(pugi::xml_node trillNode, Measure* measure)
{
    ...
    Ornament* ornament = toOrnament(this->addToChordRest(meiTrill, measure));
    if (!ornament) {
        // Warning message given in MeiImporter::addToChordRest
        return true;
    }
...
bool MeiImporter::readTurn(pugi::xml_node turnNode, Measure* measure)
{
    ...
    Ornament* ornament = toOrnament(this->addToChordRest(meiTurn, measure));
    if (!ornament) {
        // Warning message given in MeiImporter::addToChordRest
        return true;
    }
...
bool MeiImporter::readGracedAtt(libmei::AttGraced& gracedAtt)
{
    ...
    if (gracedAtt.HasGrace()) {
        ...
        bool warning = false;
        auto [isAfter, noteType] = Convert::gracegrpFromMEI(libmei::graceGrpLog_ATTACH_post,
                                                            gracedAtt.GetGrace(), warning);
        m_graceNoteType = noteType;
    } else {
        m_readingGraceNotes = GraceNone;
    }
