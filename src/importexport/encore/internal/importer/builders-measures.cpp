...
// Number of MuseScore measures to create for a single EncMeasure (its multi-rest count, else 1).
static int encMeasDisplayCount(const EncMeasure& m)
{
    const int cnt = encMeasMultiRestCount(m);
    return cnt > 1 ? cnt : 1;
}
...
void buildMeasures(BuildCtx& ctx)
{
    ...
    for (size_t mi = 0; mi < enc.measures.size(); ++mi) {
        ...
        const int displayCount = encMeasDisplayCount(encMeas);
