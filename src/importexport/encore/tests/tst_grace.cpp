        ...
        for (Segment* s = toMeasure(mb)->first(SegmentType::ChordRest);
            ...
            for (EngravingItem* e : s->elist()) {
                if (e && e->isChord() && toChord(e)->noteType() == NoteType::NORMAL
                    && !toChord(e)->notes().empty() && toChord(e)->isSmall()) {
                    ++cueChords;
                }
            }
        }
    ...
    for (MeasureBase* mb = score->first(); mb; mb = mb->next()) {
        ...
        for (Segment* s = toMeasure(mb)->first(SegmentType::ChordRest); s; s = s->next(SegmentType::ChordRest)) {
            ...
            if (e && e->isChord() && !toChord(e)->notes().empty()) {
                ...
                perMeasure.push_back({ toChord(e)->isSmall(), n->play() });
                break;
            }
        }
    }
...
TEST_F(Tst_Grace, beamed_grace_group_stays_separate)
{
    MasterScore* score = readEncoreScore("importer_grace_beamed_group.enc");
    ASSERT_NE(score, nullptr);
    muse::Ret ret = score->sanityCheck();
    EXPECT_TRUE(ret) << ret.text();

    auto firstGraceHost = [&](Measure* m) -> Chord* {
        for (Segment* s = m->first(SegmentType::ChordRest); s; s = s->next(SegmentType::ChordRest)) {
            EngravingItem* e = s->element(0);
            if (e && e->isChord() && !toChord(e)->graceNotes().empty()) {
                return toChord(e);
            }
        }
        return nullptr;
    ...
    Chord* g1 = firstGraceHost(m1);
    ASSERT_NE(g1, nullptr);
    EXPECT_EQ(g1->graceNotes().size(), 2u)
        << "a beamed grace pair must be two separate grace chords, not one merged chord";
    for (Chord* gc : g1->graceNotes()) {
        EXPECT_EQ(gc->notes().size(), 1u) << "each beamed grace is a single-note chord";
    }

    Measure* m2 = m1->nextMeasure();
    ASSERT_NE(m2, nullptr);
    Chord* g2 = firstGraceHost(m2);
    ASSERT_NE(g2, nullptr);
    EXPECT_EQ(g2->graceNotes().size(), 1u)
        << "a non-beamed same-tick grace pair stays one stacked grace chord";
    if (g2->graceNotes().size() == 1) {
        EXPECT_EQ(g2->graceNotes().front()->notes().size(), 2u)
            << "the stacked grace chord holds both notes";
    }
    delete score;
}
