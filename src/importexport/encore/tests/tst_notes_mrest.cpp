...
// Regression: two consecutive multi-rest groups with different counts, the first also carrying a key
// change. The old anti-cascade collapsed the second group after the first, and the all-REST check
// rejected the first group because of its KEYCHANGE companion; together they dropped measures.
// Fixture [note][mrest=3 + keychange][mrest=2][note] must expand to 1+3+2+1 + 2 pad = 9.
TEST_F(Tst_NotesMrest, mrest_consecutive_groups_all_expand)
{
    MasterScore* score = readEncoreScore("importer_mrest_consecutive_groups.enc");
    ASSERT_NE(score, nullptr) << "Failed to load importer_mrest_consecutive_groups.enc";
    EXPECT_EQ(score->nmeasures(), 9)
        << "both consecutive mrest groups (3 then 2) must expand, and a key change must not "
        "suppress the group; without the fix only 6 measures";
    delete score;
}
