...
static void exportAndCompareWithRef(const std::string& name)
{
    ...
    const String midiRefPath = ScoreRW::rootPath() + u'/' + MIDI_EXPORT_DATA_DIR + u'/' + String::fromUtf8(
        midiRefFileName);
...
protected:
    ...
    static void testTimeStretchFermataTempoEdit(MasterScore* score, const String& file, const String& testName)
    {
        ...
        EngravingItem* tempo
            = score->firstSegment(SegmentType::ChordRest)->findAnnotation(ElementType::TEMPO_TEXT, 0, 3);
