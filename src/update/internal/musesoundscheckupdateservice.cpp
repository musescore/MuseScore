...
static const mu::Uri MUSEHUB_APP_URI("musehub://launch?from=musescore");

using namespace mu::update;
using namespace mu::network;

mu::Ret MuseSoundsCheckUpdateService::needCheckForUpdate() const
{
#ifdef Q_OS_WIN
    return true;
#elif defined(Q_OS_MAC)
    if (systemInfo()->productVersion() < framework::Version("10.15")) {
        return false;
    }
...
mu::RetVal<ReleaseInfo> MuseSoundsCheckUpdateService::checkForUpdate()
{
...
mu::RetVal<ReleaseInfo> MuseSoundsCheckUpdateService::lastCheckResult()
{
    return m_lastCheckResult;
}

mu::framework::Progress MuseSoundsCheckUpdateService::updateProgress()
{
...
void MuseSoundsCheckUpdateService::openMuseHub()
{
    ...
    interactive()->openApp(MUSEHUB_APP_URI).onReject(this, [=](int, const std::string&) {
        static const mu::Uri MUSEHUB_APP_V1_URI("muse-hub://launch?from=musescore");
...
mu::RetVal<ReleaseInfo> MuseSoundsCheckUpdateService::parseRelease(const QByteArray& json) const
{
    RetVal<ReleaseInfo> result;
