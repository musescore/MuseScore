            ...
            for (auto currentBeat = currentBeatList.begin(); currentBeat != currentBeatList.end(); currentBeat++) {
                  ...
                  while (!currentNode.isNull()) {
                        if (currentNode.nodeName() == "GraceNotes") {
                              ...
                              for (auto iter = notesList.begin(); iter != notesList.end(); ++iter) {
                                    ...
                                    while (!currentNote.isNull()) {
                                          if (currentNote.nodeName() == "Properties") {
                                                ...
                                                if (ottavaFound[track] == 2 || (ottavaFound[track] == 1 && currentNode.parentNode().firstChildElement("Ottavia").isNull())) {
                                                      createOttava(false, track, cr, ottavaValue);
                                                      if (ottavaFound[track] == 2)
                                                            ottavaFound[track] = 1;
                                                      else
                                                            ottavaFound[track] = 0;
                                                      }
                                                if (ottavaFound[track]) {
                              ...
                              if (ottavaFound[track] == 1 && ottavaValue.compare(currentNode.toElement().text()))
                                    ottavaFound[track] = 2;
                              else
                                    ottavaFound[track] = 1;
...
void GuitarPro6::readGpif(QByteArray* data)
      ...
      ottava = new Ottava*[staves * VOICES];
      ottavaFound = new int[staves * VOICES];
      for (int i = 0; i < staves; ++i)
            slurs[i] = 0;
