...
void ChordEditor::clicked(QAbstractButton* b)
      ...
      if (bb->standardButton(b) == QDialogButtonBox::Reset) {
            QList<NoteEventList> el = _chord->score()->renderChord(_chord);
            int n = _chord->notes().size();
            for (int i = 0; i < n; ++i) {
                  _chord->notes()[i]->setPlayEvents(el[i]);
                  printf("====reset %d events\n", _chord->notes()[i]->playEvents().size());
                  }
            chordView->setChord(_chord);
            dirty = true;
            }
