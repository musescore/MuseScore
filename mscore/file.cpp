...
QPainter * getSvgPainter(QString saveName, qreal width, qreal height, qreal scale) 
   {
...
bool MuseScore::saveSvgCollection(Score* score, const QString& saveName)
      ...
      double mag = converterDpi / MScore::DPI;
      const QFileInfo fi(saveName);

      QFile metafile(QDir(fi.absolutePath()).filePath(fi.baseName()+".svg-c"));

      if (!metafile.open(QIODevice::WriteOnly))
            return false;

      QTextStream qts(&metafile);
      ...
      foreach(const Element* e, page->elements()) {
            ...
            if (e->type() == Element::Type::MEASURE) {
               ...
               if (measure->system()!=sys) {
                  ...
                  QString svgname = fi.baseName()+QString::number(count++)+".svg";
                  qts << "# " << svgname << endl;
                  //qts << ticksFromBeg << ',' << 0.0 << '\n';

                  p = getSvgPainter(QDir(fi.absolutePath()).filePath(svgname),w,h,mag);
