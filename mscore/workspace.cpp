...
static QString editedWorkspaceTranslatableName(const QString& oldWorkspaceTranslatableName)
{
    ...
    const auto it = std::find(WorkspacesManager::defaultWorkspaces.begin(),
                              WorkspacesManager::defaultWorkspaces.end(), oldWorkspaceTranslatableName);
...
QString WorkspacesManager::defaultWorkspaceTranslatableName(const QString& editedWorkspaceName)
{
    const auto it = std::find(
        WorkspacesManager::defaultEditedWorkspaces.begin(),
        WorkspacesManager::defaultEditedWorkspaces.end(), editedWorkspaceName);
    ...
    QMessageBox::StandardButton reply
        =(MScore::noGui && MScore::testMode)
          ...
          : QMessageBox::question(0,
                                  ...
                                  QWidget::tr("Do you really want to delete the '%1' workspace?").arg(
                                      workspace->name()),
...
void MuseScore::changeWorkspace(Workspace* p, bool first)
{
    ...
    if (WorkspacesManager::currentWorkspace()) {
        disconnect(getPaletteWorkspace(), &PaletteWorkspace::userPaletteChanged,
                   WorkspacesManager::currentWorkspace(), QOverload<>::of(&Workspace::setDirty));
    }
...
{
    setIconSize(QSize(preferences.getInt(PREF_UI_THEME_ICONWIDTH) * guiScaling,
...
bool WorkspacesManager::isDefaultWorkspace(Workspace* workspace)
{
    ...
    return std::find(defaultWorkspaces.begin(), defaultWorkspaces.end(),
                     workspace->translatableName()) != defaultWorkspaces.end();
}
...
bool WorkspacesManager::isDefaultEditedWorkspace(Workspace* workspace)
{
    ...
    return std::find(WorkspacesManager::defaultEditedWorkspaces.begin(),
                     WorkspacesManager::defaultEditedWorkspaces.end(),
                     workspace->translatableName()) != WorkspacesManager::defaultEditedWorkspaces.end();
}
...
void WorkspacesManager::initWorkspaces()
{
    ...
    for (const QString& path : findWorkspaceFiles()) {
        ...
        const bool isDefault = std::find(
            WorkspacesManager::defaultWorkspaces.begin(), WorkspacesManager::defaultWorkspaces.end(),
            name) != WorkspacesManager::defaultWorkspaces.end();
        const bool isEditedDefault = std::find(
            WorkspacesManager::defaultEditedWorkspaces.begin(),
            WorkspacesManager::defaultEditedWorkspaces.end(),
            name) != WorkspacesManager::defaultEditedWorkspaces.end();
    ...
    for (int i = 0; i < m_workspaces.size(); i++) {
        ...
        if (trName == WorkspacesManager::defaultWorkspaces[0]
            || trName == WorkspacesManager::defaultEditedWorkspaces[0]) {
...
const Workspace* Workspace::sourceWorkspace() const
{
    const QString sourceName
        = _sourceWorkspaceName.isEmpty() ? WorkspacesManager::defaultWorkspaces[0] : _sourceWorkspaceName;
