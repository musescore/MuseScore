...
//      preview->setPreviewOnly(true);
      ...
      // These variables are SPATIUM20 converted to each unit and rounded
      // to 3 decimals. The dialog box rounds the spatium to 3 decimal places.
      // These help force the default value of SPATIUM20 and avoid rounding errors.
      sp20_mm = rint((SPATIUM20 / DPMM) * 1000) / 1000; // millimeters
      sp20_in = rint((SPATIUM20 / DPI)  * 1000) / 1000; // inches
      sp20_p  = rint((SPATIUM20 / DPI_F / CICERO) * 1000) / 1000; // picas
      sp20_dd = rint((SPATIUM20 / DPI_F / DIDOT)  * 1000) / 1000; // didot
      sp20_c  = rint((SPATIUM20 / DPI_F / DIDOT / CICERO) * 1000) / 1000; // cicero
      ...
      if (score->styleB(Sid::pageTwosided) == twosided->isChecked() &&
          ...
          score->style().pageSize().id()   == prev->style().pageSize().id())
            ...
            if (sOdd.units()               == odd.units() &&
                ...
                sOdd.widthPoints()         == odd.widthPoints() &&
                sOdd.heightPoints()        == odd.heightPoints() &&
                sOdd.leftMarginPoints()    == odd.leftMarginPoints() &&
                sOdd.rightMarginPoints()   == odd.rightMarginPoints() &&
                sOdd.topMarginPoints()     == odd.topMarginPoints() &&
                sOdd.bottomMarginPoints()  == odd.bottomMarginPoints() &&
                sEven.topMarginPoints()    == even.topMarginPoints() &&
                sEven.bottomMarginPoints() == even.bottomMarginPoints())
                  {
