...
EditStyle::EditStyle(Score* s, QWidget* parent)
   : QDialog(parent), cs(s)
      ...
      connect(textFromToButton, &QPushButton::pressed, this, &EditStyle::changeChosenTextFonts);
...
void EditStyle::changeChosenTextFonts()
      {
      ...
      for(int i = 0; i < textStyles->count(); i++){
            ...
            if(textStyleFontFace->currentFont().family() == textFromComboBox->currentFont().family() && textStyles->item(i)->data(Qt::UserRole) != static_cast<int>(Tid::HARMONY_ROMAN))
                  textStyleFontFace->setCurrentFont(textToComboBox->currentFont());
            }
...
void EditStyle::setFromFontToMostUsed()
      {
      std::map<QFont, int> fontOccurences;
      for (int i = 0; i < textStyles->count(); i++) {
            textStyles->setCurrentRow(i);
      ...
      auto comp = [](std::pair<QFont, int> x, std::pair<QFont, int> y) { return x.second < y.second; };
