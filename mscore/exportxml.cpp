...
static void midiInstrument(XmlWriter& xml, const int partNr, const int instrNr,
      ...
      if (!qFuzzyCompare(instr->channel(0)->volume(), 90))
            xml.tag("volume", instr->channel(0)->volume());  //percent
      if (!qFuzzyIsNull(instr->channel(0)->pan()))
            xml.tag("pan", instr->channel(0)->pan()); //-180 min, -90 hard left, 90 hard right, +180 max
      if (!qFuzzyIsNull(instr->channel(0)->reverb()))
            xml.tag("reverb", instr->channel(0)->reverb());  //percent
      if (!qFuzzyIsNull(instr->channel(0)->chorus()))
            xml.tag("chorus", instr->channel(0)->chorus());  //percent
      xml.etag();
      }
