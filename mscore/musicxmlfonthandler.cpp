...
//    deffs: default font size for this type of text (as specified in word-font or lyric-font)
//    curfs: current font according to the style
//    if curfs != deffs, an initial font-size attribute is emitted
//---------------------------------------------------------

MScoreTextToMXML::MScoreTextToMXML(const QString& tag, const QString& attr, const QString& t, const int deffs, const int curfs)
      : attribs(attr), tagname(tag)
      {
      //qDebug("MScoreTextToMXML('%s')", qPrintable(t));
      // font size update iff fs != 0
      oldFormat.setFontSize(deffs);
      newFormat.setFontSize(curfs);
