    ...
    : QDialog(parent), ui(new Ui::PaletteProperties), palette(p)
{
    ...
    connect(ui->elementOffset, QOverload<double>::of(
                &QDoubleSpinBox::valueChanged), this, &PalettePropertiesDialog::offsetChanged);
...
{
    return !isGridCheckBoxChanged && !isNameChanged && !isHeightChanged && !isWidthChanged && !isOffsetChanged
           && !isScaleChanged;
}
