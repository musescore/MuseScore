...
Ms::NotePattern* SelectNoteDialog::pattern() const
{
    Ms::NotePattern* pattern = new Ms::NotePattern();
    if (sameNotehead->isChecked()) {
        pattern->notehead = m_note->headGroup();
    }
    if (samePitch->isChecked()) {
        pattern->pitch = m_note->pitch();
    }
    if (sameString->isChecked()) {
        pattern->string = m_note->string();
    }
    if (sameName->isChecked()) {
        pattern->tpc = m_note->tpc();
    }
    if (sameType->isChecked()) {
        pattern->noteType = m_note->noteType();
    }
    if (sameDurationType->isChecked()) {
        pattern->durationType = m_note->chord()->actualDurationType();
    }

    if (sameDurationTicks->isChecked()) {
        pattern->durationTicks = m_note->chord()->actualTicks();
    } else {
        pattern->durationTicks = Ms::Fraction(-1, 1);
    }

    if (sameStaff->isChecked()) {
        pattern->staffStart = m_note->staffIdx();
        pattern->staffEnd = m_note->staffIdx() + 1;
    } else if (inSelection->isChecked()) {
        pattern->staffStart = m_note->score()->selection().staffStart();
        pattern->staffEnd = m_note->score()->selection().staffEnd();
    } else {
        pattern->staffStart = -1;
        pattern->staffEnd = -1;
    }

    pattern->voice = sameVoice->isChecked() ? m_note->voice() : -1;
    pattern->system = 0;
    if (sameSystem->isChecked()) {
        pattern->system = m_note->chord()->segment()->system();
    }
...
void SelectNoteDialog::apply() const
{
    ...
    Ms::NotePattern* pattern = this->pattern();
