...
ScorePropertiesDialog::ScorePropertiesDialog(QWidget* parent)
   : QDialog(parent),
     ...
     m_dirty(false)
      ...
      // FIXME
      QString currentFileName;//  = m_score->masterScore()->fileInfo()->absoluteFilePath();
      QString previousFileName = m_score->importedFilePath();
      ...
      QMapIterator<QString, QString> iterator(m_score->metaTags());
      ...
      if (!QFileInfo::exists(m_score->importedFilePath()))
...
QPair<QLineEdit*, QLineEdit*> ScorePropertiesDialog::addTag(const QString& key, const QString& value, const bool builtinTag)
      {
...
bool ScorePropertiesDialog::isBuiltinTag(const QString& tag) const
      {
