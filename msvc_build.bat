...
IF NOT "%MSCORE_STABLE_BUILD%" == "" (
    IF NOT "%CRASH_LOG_SERVER_URL%" == "" (
        SET CRASH_REPORT_URL_OPT=-DMUSE_MODULE_DIAGNOSTICS_CRASHREPORT_URL=%CRASH_LOG_SERVER_URL% -DBUILD_CRASH_REPORTER=ON
    )
)

IF "%MUSE_APP_BUILD_MODE%" == "" (
...
REM -DCMAKE_BUILD_NUMBER=%BUILD_NUMBER% -DCMAKE_BUILD_AUTOUPDATE=%BUILD_AUTOUPDATE% %CRASH_REPORT_URL_OPT% are used for CI only
   ...
   IF EXIST "CMakeCache.txt" (
      ...
      cmake -G "%GENERATOR_NAME%" -A "%PLATFORM_NAME%" -DCMAKE_INSTALL_PREFIX=../%INSTALL_FOLDER% -DCMAKE_BUILD_TYPE=%CONFIGURATION_STR% -DMUSE_APP_BUILD_MODE=%MUSE_APP_BUILD_MODE% -DMUSESCORE_REVISION=%MUSESCORE_REVISION% -DMUSE_COMPILE_BUILD_64=%BUILD_64% -DCMAKE_BUILD_NUMBER=%BUILD_NUMBER% -DBUILD_AUTOUPDATE=%BUILD_AUTOUPDATE% %BUILD_VST_OPT% %VST3_SDK_PATH_OPT% %CRASH_REPORT_URL_OPT% %WIN_PORTABLE_OPT% ..
      IF !ERRORLEVEL! NEQ 0 (
